...
The following optional configuration parameters enable external QR scanner providers before the existing Java/Python fallback chain:

    images.qr.native.enabled=true
    images.qr.native.timeout.ms=2500
...
`images.qr.chromium.enabled` enables an optional Chromium/Chrome BarcodeDetector provider (disabled by default). Set `program.chromium.path` when browser detection should use a specific executable.

If optional providers are unavailable or return no usable result, DataShot falls back to the current barcode detection behavior.

For macOS users, native helper usage is:

1. Download the correct artifact for your CPU architecture from Releases:
   - Apple Silicon: `DataShot-<version>-macos-arm64.zip`
   - Intel: `DataShot-<version>-macos-x64.zip`
2. Ensure the helper is executable:

   chmod +x /path/to/qr-native-macos

3. Point DataShot to the helper in `imagecapture.properties`:

   images.qr.native.enabled=true
   program.qr.native.macos=/absolute/path/to/qr-native-macos

4. Optional quick check in Terminal:

   /path/to/qr-native-macos /path/to/image.jpg

If macOS blocks execution of a trusted downloaded helper, remove quarantine metadata from that file:

    xattr -d com.apple.quarantine /path/to/qr-native-macos

For maintainers distributing macOS release artifacts, recommended signing/notarization is:

1. Sign helper binaries (Intel + Apple Silicon) with Developer ID Application cert:

   codesign --force --options runtime --timestamp --sign "Developer ID Application: <Team Name> (<TEAMID>)" native/bin/qr-native-macos

2. Verify signature and Gatekeeper assessment:

   codesign --verify --strict --verbose=2 native/bin/qr-native-macos
   spctl -a -vv -t execute native/bin/qr-native-macos

3. Notarize the distributable archive:

   xcrun notarytool submit dist/DataShot-<version>-macos-<arch>.zip --keychain-profile "<NOTARY_PROFILE>" --wait

4. If distributing a `.app` or `.dmg`, staple the notarization ticket to that container (zip archives are not stapled).

The GitHub workflows can do this automatically when these repository secrets are set; otherwise the signing/notarization steps are skipped:
