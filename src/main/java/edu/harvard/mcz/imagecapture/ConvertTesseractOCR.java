...
public class ConvertTesseractOCR implements OCR {
    ...
    private static final Logger log =
            LoggerFactory.getLogger(ConvertTesseractOCR.class);
    ...
    public static void main(String[] args) {
        ...
        String target =
                "/home/mole/stuff/MCZ/mcz/insects/testImages/base/photo_plate_test_good_4_template.JPG";
        if (args.length == 1) {
            target = args[0];
        }
        ...
        try {
            ...
            ConvertTesseractOCR converter =
                    new ConvertTesseractOCR(imagefile.getSubimage(x, y, w, h));
            ...
            System.out.println("Couldn't read imagefile " + target + "." +
                    e1.getMessage());
        }
    }
    ...
    public String getOCRText() throws OCRReadException {
        ...
        if (iter.hasNext()) { // it is entering here and never reaches the else
            // below. when it executes the else below, it can't
            // find the convert program because there are
            ...
            try {
                ...
                throw new OCRReadException("Native tiff file creation failed." +
                        e.getMessage());
            }
        } else {
            ...
            if (debug) {
                ...
                for (int i = 0; i < formatNames.length; i++) {
                    ...
                    log.debug(
                            "in ConvertTesseractOCR.getOCRText() 8 Write: + formatNames[i]");
                }
            }
            ...
            try {
                ...
                log.debug("in ConvertTesseractOCR.getOCRText() 14 exitVal is " +
                        exitVal);
                if (exitVal > 0) {
                    throw new OCRReadException(
                            "ImageMagick temporary TIFF file creation failed.");
                }
            } catch (IOException e) {
                System.out.println(e.getMessage());
                log.debug("in ConvertTesseractOCR.getOCRText() 15 IOException");
                throw new OCRReadException(
                        "Native temporary jpeg file creation failed. " + e.getMessage());
            } catch (InterruptedException e) {
                System.out.println(e.getMessage());
                log.debug(
                        "in ConvertTesseractOCR.getOCRText() 16 InterruptedException");
                throw new OCRReadException("Call to ImageMagick failed." +
                        e.getMessage());
            }
            ...
            try {
                ...
                log.debug("in ConvertTesseractOCR.getOCRText() 19 OCRReadException " +
                        e1.getMessage());
            ...
            try {
                ...
                log.debug("in ConvertTesseractOCR.getOCRText() 21 OCRReadException " +
                        e.getMessage());
                System.out.println(e.getMessage());
                e.printStackTrace();
            }
        }
        log.debug("in ConvertTesseractOCR.getOCRText() 22 returnValue is " +
                returnValue);
        return returnValue;
    }
}
