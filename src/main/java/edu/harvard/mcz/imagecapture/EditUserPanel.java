	...
	private void save() {
		...
		if (!okToSave) {
			...
			JOptionPane.showMessageDialog(Singleton.getSingletonInstance().getMainFrame(), message,
					"Error. Can't save.", JOptionPane.OK_OPTION);
		} else {
			// Save
		...
		if (jButtonSetPassword == null) {
			...
			jButtonSetPassword.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					String pw = (String) JOptionPane.showInputDialog(Singleton.getSingletonInstance().getMainFrame(),
							"Set a new password for " + user.getFullname() + " (at least 10 characters)",
							"Change password for " + user.getUsername(), JOptionPane.QUESTION_MESSAGE, null, null, "");
					if (pw != null && !pw.isEmpty()) {
						...
						if (Users.testProposedPassword(pw, user.getUsername())) {
							...
							JOptionPane.showMessageDialog(Singleton.getSingletonInstance().getMainFrame(),
									"Password is not complex enough" + Users.PASSWORD_RULES_MESSAGE,
