 ...
 *
 * <p>
 * File last changed on $Date: 2012-01-06 18:51:55 -0500 (Fri, 06 Jan 2012) $ by
 * $Author: mole $ in $Rev$. $Id: ImageCaptureApp.java 305 2012-01-06 23:51:55Z
 * mole $
 */
...
public class ImageCaptureApp {
  ...
  public static final String APP_DESCRIPTION =
      "Rapid capture of data from images of pin Labels and pinned insect \nspecimens developed for the MCZ Lepidoptera collection";
  ...
  public static final String APP_LIBRARIES =
      "Hibernate, Tesseract, ZXing, Log4J, drew.metadata.exif, iText, event_date_qc";
  public static final String APP_REV =
      "$Rev$"; // ImageCapture.jar file built before commit will be one revision
  /**
   ...
   *
   * @see
   *     edu.harvard.mcz.imagecapture.ImageCaptureProperties#KEY_REGEX_DRAWERNUMBER
   */
  public static final String REGEX_DRAWERNUMBER = "[0-9]{3}\\Q.\\E[0-9]+";
  /**
   * Default regular expression for recognizing image filenames in pattern
   * decided on for project. Used to set the default value of property
   * ImageCaptureProperties.KEY_IMAGEREGEX, use that property instead of this
   * hard coded constant.
   *
  ...
  public static final String REGEX_IMAGEFILE =
      "^ETHZ_ENT[0-9]{2}_[0-9]{4}_[0-9]{2}_[0-9]{2}_[0-9]{6}\\.JPG$";
  /**
  ...
  private static final Logger log =
      LoggerFactory.getLogger(ImageCaptureApp.class);
  ...
  public static void main(String[] args) {
    ...
    /*
    URL resource = ImageCaptureApp.class.getResource("/log4j.properties");
    ...
    if (Singleton.getSingletonInstance()
      ...
      Singleton.getSingletonInstance().setBarcodeBuilder(
          barcodeTextBuilderMatcher);
      Singleton.getSingletonInstance().setBarcodeMatcher(
          barcodeTextBuilderMatcher);
    } else if (Singleton.getSingletonInstance()
                   .getProperties()
                   .getProperties()
                   .getProperty(ImageCaptureProperties.KEY_COLLECTION)
     ...
     * "com.sun.corba.ee.impl.presentation.rmi.JNDIStateFactoryImpl"); try {
     * context = new InitialContext(contextProperties); } catch (NamingException
     * ex) { ex.printStackTrace(); } if (context!=null) { ConnectionFactory
     ...
     * connection.start(); while (true) { Message m = subscriber.receive(1); if
     * (m
     * != null) { if (m instanceof TextMessage) { TextMessage message =
     * (TextMessage) m; String originator =
     * message.getStringProperty("Originator"); String text = message.getText();
     * System.out.println("Message: " + originator
     * + ": " + text); } else { break; } } } } catch (NamingException e) { //
    ...
    if (APP_VERSION == null) {
      ...
      try {
        ...
        privateProperties.load(
            ImageCaptureApp.class.getClassLoader().getResourceAsStream(
                "imagecapture.private.properties"));
        APP_VERSION = privateProperties.getProperty("project.version");
        try {
          String buildTimestamp =
              privateProperties.getProperty("buildTimestamp");
          if (buildTimestamp != null) {
            APP_VERSION += " " + buildTimestamp;
          }
        } catch (Exception e) {
          log.warn("Exception when getting App Version", e);
        }
  ...
  public static void doStartUp() {
    ...
    try {
      ...
      if (allowed == null || allowed.trim().length() == 0) {
        Singleton.getSingletonInstance().getMainFrame().setStatusMessage(
            "Database does not support this version, schema update needed.");
        JOptionPane.showMessageDialog(
            Singleton.getSingletonInstance().getMainFrame(),
            "The database does not support" + APP_NAME + " version " +
                APP_VERSION +
                ".  A database schema update to version 1.3 is required.",
            "Schema Update Required", JOptionPane.OK_OPTION);
        log.error(
            "Database does not test for versioning. Added in 1.3.0 Schema, and required by " +
    ...
    if (isCurrentAllowed) {
      Singleton.getSingletonInstance().getMainFrame().setStatusMessage(
          ImageCaptureApp.APP_VERSION + " OK");
    } else {
      String allowed = AllowedVersionLifeCycle.listAllowedVersions();
      Singleton.getSingletonInstance().getMainFrame().setStatusMessage(
          "Database does not support version, update needed.");
      int response = JOptionPane.showConfirmDialog(
          Singleton.getSingletonInstance().getMainFrame(),
          "The database does not support" + APP_NAME + " version " +
              APP_VERSION + "." + System.lineSeparator() +
              "  A software (or database) update from " + allowed +
      ...
      log.error("Database does not allow version " +
                ImageCaptureApp.APP_VERSION + ". Upgrade will " +
                (response == JOptionPane.YES_OPTION ? "" : "not happen") +
                "happen.");
      ...
      if (response == JOptionPane.YES_OPTION) {
        ...
        try {
          ...
          JOptionPane.showMessageDialog(
              Singleton.getSingletonInstance().getMainFrame(),
              "Upgrade failed with error: " + e.getMessage(),
              "DB Upgrade failed.", JOptionPane.ERROR_MESSAGE);
        }
      }
      // ImageCaptureApp.exit(EXIT_ERROR);
    }
  ...
  public static void cleanUp() {
    try {
      ...
      System.out.println("Properties file save failed.  " +
