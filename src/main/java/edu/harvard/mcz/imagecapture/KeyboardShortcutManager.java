...
/**
 * Filesystem persistence and retrieval of keyboard shortcuts. This class
 * provides the properties for the keyboard shortcuts as well as an easy way to
 * access/get/add them
 * <p>
 * Syntax for a shortcut: see
 * https://javadoc.scijava.org/Java7/javax/swing/KeyStroke.html#getKeyStroke(java.lang.String)
 */
public class KeyboardShortcutManager extends AbstractTableModel {

	private static final Logger log = LoggerFactory.getLogger(KeyboardShortcutManager.class);
	 ...
	 *
	 * @param name
	 *            a unique ID for the action, used as key in the properties file
	 * @param defaultStroke
	 *            the KeyStroke to use otherwise
	...
	public KeyStroke getShortcut(String name, String defaultStroke) {
		...
		try {
			...
			KeyboardShortcutManager.log.error(
					"Failed to return keyboard shortcut. Format them as described in https://javadoc.scijava.org/Java7/javax/swing/KeyStroke.html#getKeyStroke(java.lang.String)",
					e);
			System.out.println("...!");
			return null;
		}
		...
		try {
			...
			if (propertiesStream != null) {
				try {
					...
					// keyboard shortcuts are not that important to require all cases
					// handled
				}
			}
		}
