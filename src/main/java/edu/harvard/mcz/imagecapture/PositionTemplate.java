...
/**
 * Description of the coordinates within an image where parts of the image to
 * pass to barcode reading software, OCR software, and which parts are to be
 * displayed as containing a specimen image or specimen labels.   Wrapper for
 * persistent Template class.  Provides special case template
 * TEMPLATE_NO_COMPONENT_PARTS for handling any size image that isn't templated.
 * Provides hard coded default template.  Provides methods to get/set
 * coordinates as Dimension objects as a convenience over Template's get/set of
 * individual heights and widths as integers. <p>
 * DefaultPositionTemplateDetector makes the assumption that a template can be
 * uniquely identified by the location of the barcode in the image. <p> Each
...
public class PositionTemplate {
   ...
   *
   * @see
   *     edu.harvard.mcz.imagecapture.exceptions.NoComponentPartsTemplateException
   */
  ...
  private static final Logger log =
      LoggerFactory.getLogger(PositionTemplate.class);
  private Dimension imageSize;       // Pixel dimensions of the image
  private Dimension barcodePosition; // Location of upper left corner of area to
                                     // scan for barcode in image.
  private Dimension barcodeSize; // Size of area in image to scan for barcode.
  private Dimension specimenPosition; // Location of upper left corner of part
                                      // of image containing specimen.
  ...
  private ICImage
      referenceImage;          // Filename of reference image for this template.
  private String templateName; // free text description of the template
  private String templateId;   // the identifying string for this template =
                               // TEMPLATE_ constants
  private boolean isEditable;  // false for TEMPLATE_s, true for DB records.

  /**
   * Use a (currently hardcoded) default template for which pixel coordinates on
   * the image contain which information.
   */
  public PositionTemplate() { useDefaultValues(); }

  /**
   * Create a new (probably editable) template based on the default template for
   * which pixel coordinates on the image contain which information.
   *
  ...
  /**
   * Use a template defined by a PositionTemplate.TEMPLATE_* constant or
   * potentially from another valid source.  A template defines which pixel
   * coordinates on the image contain which information.   The template cannot
   * be changed for an instance of PositionTemplate once it has been
   * instantiated.  This constructor is the only means of setting the template
   * to use. Create a new instance of PositionTemplate if you wish to use a
   * different template. <p> The list of available templateIds can be retrieved
   * with PositionTemplate.getTemplates().
   *
   * @param templateToUse the templateID of the template to use in this instance
   *     of PositionTemplate.
  ...
  public PositionTemplate(Template templateInstance) {
    ...
    imageSize = new Dimension(templateInstance.getImageSizeX(),
                              templateInstance.getImageSizeY());
    barcodePosition = new Dimension(templateInstance.getBarcodePositionX(),
                                    templateInstance.getBarcodePositionY());
    barcodeSize = new Dimension(templateInstance.getBarcodeSizeX(),
                                templateInstance.getBarcodeSizeY());
    specimenPosition = new Dimension(templateInstance.getSpecimenPositionX(),
                                     templateInstance.getSpecimenPositionY());
    specimenSize = new Dimension(templateInstance.getSpecimenSizeX(),
                                 templateInstance.getSpecimenSizeY());
    labelPosition = new Dimension(templateInstance.getLabelPositionX(),
                                  templateInstance.getLabelPositionY());
    labelSize = new Dimension(templateInstance.getLabelSizeX(),
                              templateInstance.getLabelSizeY());
    utLabelPosition = new Dimension(templateInstance.getUtLabelPositionX(),
                                    templateInstance.getUtLabelPositionY());
    utLabelSize = new Dimension(templateInstance.getUtLabelSizeX(),
                                templateInstance.getUtLabelSizey());
    utBarcodePosition = new Dimension(templateInstance.getUtBarcodePositionX(),
                                      templateInstance.getUtBarcodePositionY());
    utBarcodeSize = new Dimension(templateInstance.getUtBarcodeSizeX(),
                                  templateInstance.getUtBarcodeSizeY());
    textPosition = new Dimension(templateInstance.getTextPositionX(),
                                 templateInstance.getTextPositionY());
    textSize = new Dimension(templateInstance.getTextSizeX(),
                             templateInstance.getTextSizeY());
  }

  /**
   * Fetch the list of valid template names (including the no component parts
   * template). Use these names in the constructor PositionTemplate(String
   * templateToUse);
   *
  ...
  public static List<String> getTemplateIds() {
    ...
    String[] templates = {TEMPLATE_TEST_1, TEMPLATE_DEFAULT,
                          TEMPLATE_NO_COMPONENT_PARTS};

    List<String> temp = Arrays.asList(templates);
    ArrayList<String> templateIdList = new ArrayList<String>();

    String defaultTemplatesPropertiesVal =
        Singleton.getSingletonInstance()
            .getProperties()
            .getProperties()
            .getProperty(ImageCaptureProperties.KEY_DEFAULT_TEMPLATES);

    // allie: here load the props file: leave the default and config the other
    // var. template.default=Default template template.default2=ETHZ_template1
    // template.default2=ETHZ_template1,ETHZ_template2
    if (!("").equals(defaultTemplatesPropertiesVal)) {
      StringTokenizer st2 =
          new StringTokenizer(defaultTemplatesPropertiesVal, ",");
      ...
      // templateIdList.add(defaultTemplatesPropertiesVal); //todo string
      // tokenizer
      log.debug("defaultTemplatesPropertiesVal::::: " +
                defaultTemplatesPropertiesVal);
    } else { // here it cycles through all the templates (if you leave
             // template.default=Default template and template.default2=)
      log.debug("it will cycle through all templates::::: " +
                defaultTemplatesPropertiesVal);
  ...
  /**
   * Given an ICImage, look up the template for that image in the database, if
   * none, run a template detector to determine the template for the image.
   *
   * @param image the ICImage for which the PositionTemplate is to be returned.
   * @return the PositionTemplate for this image indicating what information is
   *     where in the image.
   * @throws ImageLoadException
   */
  public static PositionTemplate findTemplateForImage(ICImage image)
      throws ImageLoadException {
    ...
    try {
      ...
      log.error("Image database record includes an unknown template. " +
                e.getMessage());
      ...
      try {
        ...
        log.error(
            "TEMPLATE_NO_COMPONENT_PARTS produced a NoSuchTemplateException.",
            e1);
        log.trace("Trace", e1);
        ;
  ...
  private void useDefaultValues() {
    ...
    // Approximately 300 x 300 pixel area in upper right corner of 12 mega-pixel
    // image.
    ...
    utLabelPosition = new Dimension(
        0, 850); // not defined as different from labels in test image
  ...
  /**
   * Get the identifying name of this position template.  This name is fixed for
   * an instance during its construction.   This name corresponds to one of the
   * strings returned by PositionTemplate.getTemplates();   Redundant with
   * getTemplateId()
   *
   * @return the identifier of the template in use in this instance of
   *     PositionTemplate.
   * @see edu.harvard.mcz.imagecapture.PositionTemplate#getTemplateIds()
   * @see edu.harvard.mcz.imagecapture.PositionTemplate#getTemplateId()
   * @deprecated
   */
  public String getTemplateIdentifier() { return getTemplateId(); }

  /**
   * Get the free text descriptive name of the position template for potential
   * display to a person. Use getTemplateIdentifier() to identify templates in
   * code.
   *
   ...
   */
  public void setImageSize(Dimension imageSize) { this.imageSize = imageSize; }

  // TODO: add define/save templates

  // TODO: add retrieval of template from file.

  public Dimension getBarcodeULPosition() { return barcodePosition; }

  public Dimension getBarcodeSize() { return barcodeSize; }
   ...
   */
  public Dimension getBarcodePosition() { return barcodePosition; }
   ...
   */
  public Dimension getSpecimenPosition() { return specimenPosition; }
   ...
   */
  public Dimension getSpecimenSize() { return specimenSize; }
   ...
   */
  public Dimension getTextPosition() { return textPosition; }
   ...
   * @return the textSize
   */
  public Dimension getTextSize() { return textSize; }

  /**
   * @param textSize the textSize to set
   */
  public void setTextSize(Dimension textSize) { this.textSize = textSize; }
   ...
   */
  public void setLabelSize(Dimension labelSize) { this.labelSize = labelSize; }

  /**
   * @return the labelPosition
   */
  public Dimension getLabelPosition() { return labelPosition; }
   ...
   * @return the labelSize
   */
  public Dimension getUTLabelsSize() { return utLabelSize; }
   ...
   * @return the labelPosition
   */
  public Dimension getUTLabelsPosition() { return utLabelPosition; }
  ...
  /**
   * private method, to use a different template, instantiate a new
   * PositionTemplate. Contains hardcoded templates other than the default <p>
  ...
  private boolean loadTemplateValues(String templateToUse) {
    ...
    if (templateToUse.equals(PositionTemplate.TEMPLATE_TEST_1)) {
      ...
      // Set default values
      imageSize = new Dimension(2848, 4272);
      // Approximately 300 x 300 pixel area in upper right corner of 12
      // mega-pixel image.
  ...
  public boolean loadByTemplateId(String aTemplateId) {
    ...
    if (t_result != null) {
      ...
      imageSize =
          new Dimension(t_result.getImageSizeX(), t_result.getImageSizeY());
      barcodePosition = new Dimension(t_result.getBarcodePositionX(),
                                      t_result.getBarcodePositionY());
      barcodeSize =
          new Dimension(t_result.getBarcodeSizeX(), t_result.getBarcodeSizeY());
      specimenPosition = new Dimension(t_result.getSpecimenPositionX(),
                                       t_result.getSpecimenPositionY());
      specimenSize = new Dimension(t_result.getSpecimenSizeX(),
                                   t_result.getSpecimenSizeY());
      textPosition = new Dimension(t_result.getTextPositionX(),
                                   t_result.getTextPositionY());
      textSize =
          new Dimension(t_result.getTextSizeX(), t_result.getTextSizeY());
      labelPosition = new Dimension(t_result.getLabelPositionX(),
                                    t_result.getLabelPositionY());
      labelSize =
          new Dimension(t_result.getLabelSizeX(), t_result.getLabelSizeY());
      utLabelPosition = new Dimension(t_result.getUtLabelPositionX(),
                                      t_result.getUtLabelPositionY());
      utLabelSize =
          new Dimension(t_result.getUtLabelSizeX(), t_result.getUtLabelSizey());
      utBarcodePosition = new Dimension(t_result.getUtBarcodePositionX(),
                                        t_result.getUtBarcodePositionY());
      utBarcodeSize = new Dimension(t_result.getUtBarcodeSizeX(),
                                    t_result.getUtBarcodeSizeY());
      isEditable = true;
   ...
   * PoistionTemplate.
   */
  public ICImage getReferenceImage() { return referenceImage; }

  public void setReferenceImage(ICImage anImage) { referenceImage = anImage; }
  ...
  public String getReferenceImageFilePath() {
    ...
    if (referenceImage != null && referenceImage.getPath() != null &&
        referenceImage.getFilename() != null) {
  ...
  public Dimension getUtBarcodeSize() { return utBarcodeSize; }

  public void setUtBarcodeSize(Dimension utBarcodeSize) {
    this.utBarcodeSize = utBarcodeSize;
  }

  public Dimension getUtBarcodePosition() { return utBarcodePosition; }
