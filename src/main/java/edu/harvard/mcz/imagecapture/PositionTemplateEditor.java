...
/**
 * PositionTemplateEditor allows viewing and creation of position templates
 * indicating which portions of an image file contain a barcode, OCR text, a
 * specimen, labels from the unit tray, and labels from the specimen's pin.
 */
public class PositionTemplateEditor extends JFrame {
    ...
    private static final Logger log =
            LoggerFactory.getLogger(PositionTemplateEditor.class);
    ...
    private String referenceImageFilename; // name of the currently loaded image
    // file.  //  @jve:decl-index=0:
    ...
    /**
     * Constructor called from main method when running as stand alone
     * application.
     *
    ...
    public void setBlankBackgroundImage() {
        try {
            // Can't retrieve resource as a file from Jar file, unless giving File to
            // ImageIcon....
            URL url = this.getClass().getResource(
                    "/edu/harvard/mcz/imagecapture/resources/images/blank2848x4272.jpg");
            log.error("Error", url);
    ...
    public void setTemplate(String aTemplateId) throws NoSuchTemplateException {
        ...
        if (template.getImageSize() == null) {
            ...
            jTextField2.setText("Width=" + template.getImageSize().width +
                    " Height=" + template.getImageSize().height);
            setButtonTexts();
    ...
    /**
     * Set the image displayed in the editor given an URL (needed to load from
     * resource inside jar).
     *
                ...
                public void actionPerformed(java.awt.event.ActionEvent e) {
                    ...
                    if (template.isEditable()) {
                        ...
                        try {
                            ...
                            // TODO: Check that template is valid, not overlapping with
                            // existing template. Test images IMG_00005.jpg and IMG_00001.jpg
                            // suggest that overlapping templates might be needed - where all
                            // parameters except extent of barcode are the same.
                            ...
                            JOptionPane.showMessageDialog(
                                    thisFrame,
                                    "Error. Unable to save template, invalid data. " +
                                            e1.getMessage(),
                            ...
                            JOptionPane.showMessageDialog(
                                    ...
                                    "Error:SaveFailed", JOptionPane.ERROR_MESSAGE);
                            log.debug("Debug", e2);
                        }
                    }
                }
    ...
    /**
     * Set the text for each button as the position and size of the relevant
     * portion of the template.
     */
    private void setButtonTexts() {
        controlBarcode.setText("UL=" + template.getBarcodePosition().width + "," +
                template.getBarcodePosition().height +
                " W/H=" + template.getBarcodeSize().width + "," +
                template.getBarcodeSize().height);
        controlText.setText("UL=" + template.getTextPosition().width + "," +
                template.getTextPosition().height +
                " W/H=" + template.getTextSize().width + "," +
                template.getTextSize().height);
        controlLabel.setText("UL=" + template.getLabelPosition().width + "," +
                template.getLabelPosition().height +
                " W/H=" + template.getLabelSize().width + "," +
                template.getLabelSize().height);
        controlUTLabels.setText("UL=" + template.getUTLabelsPosition().width + "," +
                template.getUTLabelsPosition().height +
                " W/H=" + template.getUTLabelsSize().width + "," +
                template.getUTLabelsSize().height);
        controlSpecimen.setText("UL=" + template.getSpecimenPosition().width + "," +
                template.getSpecimenPosition().height +
                " W/H=" + template.getSpecimenSize().width + "," +
                template.getSpecimenSize().height);
        controlUTBarcode.setText("UL=" + template.getUtBarcodePosition().width +
                "," + template.getUtBarcodePosition().height +
                " W/H=" + template.getUtBarcodeSize().width + "," +
                template.getUtBarcodeSize().height);
    }
    ...
    private JMenuItem getJMenuItemLoadImage() {
        if (jMenuItem1 == null) {
            ...
            jMenuItem1.addActionListener(new java.awt.event.ActionListener() {
                public void actionPerformed(java.awt.event.ActionEvent e) {
                    jLabelFeedback.setText("");
                    File imageFile = FileUtility.askForImageFile(
    ...
    private JButton getJTextField3() {
        if (controlBarcode == null) {
            ...
            controlBarcode.addActionListener(new java.awt.event.ActionListener() {
                public void actionPerformed(java.awt.event.ActionEvent e) {
                    try {
                        PositionTemplateBoxDialog dialog = new PositionTemplateBoxDialog(
                                thisFrame, template.getImageSize(),
                                template.getBarcodeULPosition(), template.getBarcodeSize(),
                        ...
                        JOptionPane.showMessageDialog(
                                thisFrame,
                                "Error. Unable to edit, invalid template data. " +
                                        e1.getMessage(),
                                "Error:BadTemplate", JOptionPane.ERROR_MESSAGE);
                        log.error("Error", e1);
                    }
                    drawLayers();
                }
        ...
        if (controlText == null) {
            ...
            controlText.addActionListener(new java.awt.event.ActionListener() {
                public void actionPerformed(java.awt.event.ActionEvent e) {
                    try {
                        PositionTemplateBoxDialog dialog = new PositionTemplateBoxDialog(
                                thisFrame, template.getImageSize(), template.getTextPosition(),
        ...
        if (controlLabel == null) {
            ...
            controlLabel.addActionListener(new java.awt.event.ActionListener() {
                public void actionPerformed(java.awt.event.ActionEvent e) {
                    try {
                        PositionTemplateBoxDialog dialog = new PositionTemplateBoxDialog(
                                thisFrame, template.getImageSize(), template.getLabelPosition(),
            ...
            controlUTLabels.addActionListener(new java.awt.event.ActionListener() {
                public void actionPerformed(java.awt.event.ActionEvent e) {
                    try {
                        PositionTemplateBoxDialog dialog = new PositionTemplateBoxDialog(
                                thisFrame, template.getImageSize(),
                                template.getUTLabelsPosition(), template.getUTLabelsSize(),
            ...
            controlSpecimen.addActionListener(new java.awt.event.ActionListener() {
                public void actionPerformed(java.awt.event.ActionEvent e) {
                    try {
                        PositionTemplateBoxDialog dialog = new PositionTemplateBoxDialog(
                                thisFrame, template.getImageSize(),
                                template.getSpecimenPosition(), template.getSpecimenSize(),
    ...
    private JTable getJTable() {
        if (jTable == null) {
            ...
            jTable.getColumn("").setCellEditor(
                    new ButtonEditor(ButtonEditor.OPEN_TEMPLATE, thisFrame));
        }
        return jTable;
    }
    ...
    /**
     * Draw boxes delimiting the various parts of the template on the current
     * image. Public so that it can be invoked from ButtonEditor().
     */
    public void drawLayers() {
        // draw the image bounds in black.
        if (template != null) {
            imagePanelForDrawing.clearOverlay();
            imagePanelForDrawing.drawBox(new Dimension(0, 0), template.getImageSize(),
                    Color.BLACK, new BasicStroke(2F));
            // draw each template layer in a distinct color (keyed to UI text).
            imagePanelForDrawing.drawBox(template.getBarcodeULPosition(),
                    template.getBarcodeSize(), Color.RED);
            imagePanelForDrawing.drawBox(template.getTextPosition(),
                    template.getTextSize(), Color.BLUE);
            imagePanelForDrawing.drawBox(template.getSpecimenPosition(),
                    template.getSpecimenSize(), Color.ORANGE);
            imagePanelForDrawing.drawBox(template.getUTLabelsPosition(),
                    template.getUTLabelsSize(), Color.CYAN);
            imagePanelForDrawing.drawBox(template.getLabelPosition(),
                    template.getLabelSize(), Color.MAGENTA);
            imagePanelForDrawing.drawBox(template.getUtBarcodePosition(),
            ...
            jButton.addActionListener(new java.awt.event.ActionListener() {
                public void actionPerformed(java.awt.event.ActionEvent e) {
                    if (template != null && imagePanelForDrawing.getImage() != null) {
                        ...
                        jTextFieldBarcodeScan.setText(
                                candidateImageFile.getBarcodeTextFromImage(
                                        (BufferedImage) imagePanelForDrawing.getImage(), template,
                                        false));
                    }
                }
            });
    ...
    private JButton getJButton1() {
        if (jButtonUnitTrayBarcode == null) {
            ...
            jButtonUnitTrayBarcode.addActionListener(
                    new java.awt.event.ActionListener() {
                        public void actionPerformed(java.awt.event.ActionEvent e) {
                            CandidateImageFile candidateImageFile = new CandidateImageFile();
                            jTextFieldUnitTrayBarcode.setText(
                                    candidateImageFile.getBarcodeUnitTrayTextFromImage(
                                            (BufferedImage) imagePanelForDrawing.getImage(),
                                            template));
                        }
                    });
        }
            ...
            controlUTBarcode.addActionListener(new java.awt.event.ActionListener() {
                public void actionPerformed(java.awt.event.ActionEvent e) {
                    try {
                        PositionTemplateBoxDialog dialog = new PositionTemplateBoxDialog(
                                thisFrame, template.getImageSize(),
                                template.getUtBarcodePosition(), template.getUtBarcodeSize(),
