...
public class ThumbnailBuilder implements Runnable {
	private static final Logger log = LoggerFactory.getLogger(ThumbnailBuilder.class);
	...
	public void run() {
				...
				while (i.hasNext()) {
					...
					if (!output.exists()) {
						...
						try {
							...
							BufferedImage thumbnail = Scalr.resize(img, Scalr.Method.BALANCED, Scalr.Mode.FIT_TO_WIDTH,
		...
		JOptionPane.showMessageDialog(null, "Done building " + thumbnailCounter + " thumbnails in ./thumbs/" + exists,
				"Thumbnails Built.", JOptionPane.INFORMATION_MESSAGE);
	}
}
