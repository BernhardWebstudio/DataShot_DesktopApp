...
public class UnitTrayLabelParser implements TaxonNameReturner, DrawerNameReturner, CollectionReturner {
	...
	private static final Logger log = LoggerFactory.getLogger(UnitTrayLabelParser.class);
	...
	/**
	 * Create a unit tray label parser and parse the text. Call the get methods of
	 * the instance to return the parsed text.
	 *
	 * Usage: <code>
	...
	/**
	 * Identify atomic database field elements from their position in aStringToParse
	 * as given to the constructor (called by the constructor). Invokes the
	 * protected set_ methods.
	 */
	protected void parse() {
		...
		if (text != null) {
			...
			try {
				...
				if (lines.length > 4) {
					int st = 1;
					if (lines[st].trim().contains(" ") && lines[st + 1].trim().matches(".*[A-Za-z]+.*")
			...
			if (higherbits.length == nameStartsAt + 1 && higherbits.length > 0) {
				...
				try {
					...
					if (lines[lines.length - 2].trim().matches(Singleton.getSingletonInstance().getProperties()
				...
				if (lines.length > 1) {
					try {
						String[] speciesGroupName = lines[lines.length - 3].trim().split(" +"); // split on one or more
																								// spaces
						parseSpeciesGroupName(speciesGroupName);
					} catch (ArrayIndexOutOfBoundsException e) {
						// unexpected, but possible if elements are missing
			...
			if (lines != null) {
				...
				for (int i = 0; i < lines.length; i++) {
					if (lines[i].trim().matches(Singleton.getSingletonInstance().getProperties().getProperties()
	 ...
	 *            the species group name to parse.
	 */
			...
			if (speciesGroupName.length > 2) {
				if (speciesGroupName[2].matches("^var.|forma|f.|form")) {
					try {
						...
						// following it.
						log.error("Parsing error: found infrasubspecific rank with no epithet in: "
