...
public class LocationInCollection {
	 ...
	 * of collections.
	 *
	 * @return a string array of values for the location in collection suitable for
	 *         populating a picklist.
	 */
	public static String[] getLocationInCollectionValues() {

		// TODO: make list available as configuration (or from a database table)

		String configuredcollection = Singleton.getSingletonInstance().getProperties().getProperties()
				.getProperty(ImageCaptureProperties.KEY_SPECIFIC_COLLECTION);
		String coll = Singleton.getSingletonInstance().getProperties().getProperties()
		...
		if (configuredcollection == null || configuredcollection.trim().length() == 0) {
			...
			if (coll.equals(ImageCaptureProperties.COLLECTION_ETHZENT)) {
				...
				String[] values = {configuredcollection, GENERAL, GENERALANT, "Type Collection", "Nabokov Collection",
						"Slide Collection"};
				return values;
			}
		}
	 ...
	 * specific value obtained from ImageCaptureProperties.KEY_SPECIFIC_COLLECTION.
	 *
	 * @return string value for the default location in collection.
	 */
	public static String getDefaultLocation() {
		String value = Singleton.getSingletonInstance().getProperties().getProperties()
				.getProperty(ImageCaptureProperties.KEY_SPECIFIC_COLLECTION);
