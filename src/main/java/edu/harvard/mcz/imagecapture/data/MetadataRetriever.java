...
/**
 * MetadataRetriever produces metadata (field lengths, tooltip texts, input
 * masks, input verifiers) for fields in tables in database.
 */
public class MetadataRetriever {
	...
	/**
	 * Generates a MaskFormatter for a JFormattedTextField based upon the length
	 * (and potentially the type) of the underlying text field. Doesn't work well
	 * for normal varchar() fields, as the JTextField appears to be full of spaces.
	 * <p>
	 * Usage:
	 *
	 * <pre>
	 * JFormattedTextField jtext_for_fieldname = new JFormattedTextField(
	 * 		MetadataRetriever(tablename.class, "fieldname"));
	 * </pre>
	 *
	 * @param aTableClass
	 * @param fieldname
	 ...
	 *         <p>
	 *         TODO: add field type recognition, currently returns only "****"
	 *         masks.
	 */
	...
	public static boolean isValidDateISOString(final String dateString) {
		...
		if (Objects.equals(split[1], "00") || Objects.equals(split[2], "00")) {
			return (Integer.parseInt(split[1]) <= 12 && Integer.parseInt(split[2]) <= 31);
		}
	 ...
	 * jText_for_fieldname
	 * 		.addInputVerifier(MetadataRetriever.getInputVerifier(tablename.class, "fieldname", jText_for_fieldname));
	 * </pre>
	 *
	 * @param aTableClass
	 *            table proxy object for fieldname
	 * @param fieldname
	 *            field for which to check the fieldlength
	 * @param field
	 *            JTextField to which the InputVerifier is being added.
	 * @return an InputVerifier for the JTextField TODO: implement tests for more
	 *         than just length.
	 */
	public static InputVerifier getInputVerifier(final Class aTableClass, final String fieldname,
			final JTextField field) {
		InputVerifier result = null;
		if ((aTableClass == Specimen.class
				&& (fieldname.equalsIgnoreCase("ISODate") || fieldname.equalsIgnoreCase("DateIdentified")))
				...
				public boolean verify(JComponent comp) {
					...
					if (content.length() > getFieldLength(aTableClass, fieldname)) {
						...
						if (content.contains("-")) {
							...
							if (!MetadataRetriever.isValidDateISOString(split[0])
									|| !MetadataRetriever.isValidDateISOString(split[1])) {
								returnValue = false;
							}
						} else {
	...
	/**
	 * Determine the length of a field from the class of the proxy object for the
	 ...
	 *
	 * @param aTableClass
	 *            the class of the proxy object over the table.
	 * @param fieldname
	 *            the name of the field in that table (case insensitive).
	...
	public static int getFieldLength(Class aTableClass, String fieldname) {
		...
		if (aTableClass == Specimen.class) {
			...
			if (fieldname.equalsIgnoreCase("DrawerNumber")) {
				length = 10;
			}
			if (fieldname.equalsIgnoreCase("TypeStatus")) {
				length = 50;
			}
			if (fieldname.equalsIgnoreCase("TypeNumber")) {
				length = 255;
			}
			if (fieldname.equalsIgnoreCase("CitedInPublication")) {
				length = 900;
			}
			...
			if (fieldname.equalsIgnoreCase("Tribe")) {
				length = 40;
			}
			length = getDetFieldLength(fieldname, length);
			if (fieldname.equalsIgnoreCase("NatureOfID")) {
				length = 255;
			}
			...
			if (fieldname.equalsIgnoreCase("SpecificLocality")) {
				length = 65535;
			}
			if (fieldname.equalsIgnoreCase("VerbatimLocality")) {
				length = 65535;
			}
			if (fieldname.equalsIgnoreCase("VerbatimElevation")) {
				length = 255;
			}
			if (fieldname.equalsIgnoreCase("CollectingMethod")) {
				length = 255;
			}
			if (fieldname.equalsIgnoreCase("ISODate")) {
				length = 21;
			}
			if (fieldname.equalsIgnoreCase("DateNOS")) {
				length = 32;
			}
			if (fieldname.equalsIgnoreCase("DateEmerged")) {
				length = 32;
			}
			if (fieldname.equalsIgnoreCase("DateEmergedIndicator")) {
				length = 50;
			}
			if (fieldname.equalsIgnoreCase("DateCollected")) {
				length = 32;
			}
			if (fieldname.equalsIgnoreCase("DateCollectedIndicator")) {
				length = 50;
			}
			if (fieldname.equalsIgnoreCase("Collection")) {
				length = 255;
			}
			if (fieldname.equalsIgnoreCase("SpecimenNotes")) {
				length = 65535;
			}
			if (fieldname.equalsIgnoreCase("LifeStage")) {
				length = 50;
			}
			if (fieldname.equalsIgnoreCase("Sex")) {
				length = 50;
			}
			if (fieldname.equalsIgnoreCase("PreparationType")) {
				length = 50;
			}
			if (fieldname.equalsIgnoreCase("Habitat")) {
				length = 900;
			}
			if (fieldname.equalsIgnoreCase("AssociatedTaxon")) {
				length = 900;
			}
			if (fieldname.equalsIgnoreCase("Questions")) {
				length = 900;
			}
			if (fieldname.equalsIgnoreCase("Inferences")) {
				length = 900;
			}
			if (fieldname.equalsIgnoreCase("LocationInCollection")) {
				length = 50;
			}
			...
			if (fieldname.equalsIgnoreCase("DateLastUpdated")) {
				length = 0;
			}
			if (fieldname.equalsIgnoreCase("LastUpdatedBy")) {
				length = 255;
			}
			if (fieldname.equalsIgnoreCase("ValidDistributionFlag")) {
				length = 1;
			}
		}
		if (aTableClass == Number.class) {
			...
			if (fieldname.equalsIgnoreCase("NumberType")) {
				length = 50;
			}
		}
		if (aTableClass == Collector.class) {
			if (fieldname.equalsIgnoreCase("CollectorName")) {
				length = 255;
			}
		}
		...
		if (aTableClass == ICImage.class) {
			if (fieldname.equalsIgnoreCase("rawBarcode")) {
				length = 50;
			}
			if (fieldname.equalsIgnoreCase("rawExifBarcode")) {
				length = 50;
			}
			if (fieldname.equalsIgnoreCase("filename")) {
				length = 50;
			}
			if (fieldname.equalsIgnoreCase("rawOcr")) {
				length = 65535;
			}
			if (fieldname.equalsIgnoreCase("path")) {
				length = 900;
			}
			if (fieldname.equalsIgnoreCase("uri")) {
				length = 50;
			}
			if (fieldname.equalsIgnoreCase("templateId")) {
				length = 50;
			}
			if (fieldname.equalsIgnoreCase("drawerNumber")) {
				length = 10;
	...
	private static int getDetFieldLength(String fieldname, int length) {
		...
		if (fieldname.equalsIgnoreCase("SubspecificEpithet")) {
			length = 255;
		}
		if (fieldname.equalsIgnoreCase("InfraspecificEpithet")) {
			length = 40;
		}
		if (fieldname.equalsIgnoreCase("InfraspecificRank")) {
			length = 40;
		}
		if (fieldname.equalsIgnoreCase("Authorship")) {
			length = 255;
		}
		if (fieldname.equalsIgnoreCase("UnNamedForm")) {
			length = 50;
		}
		if (fieldname.equalsIgnoreCase("IdentificationQualifier")) {
			length = 50;
		}
		if (fieldname.equalsIgnoreCase("IdentifiedBy")) {
			length = 255;
		}
		if (fieldname.equalsIgnoreCase("DateIdentified")) {
			length = 21;
		}
		return length;
	}
	...
	/**
	 * Given a proxy class for a table and the name of a field return a help text
	 * for that field.
	 *
	 * @param aTableClass
	 * @param fieldname
	 * @return a String containing a help text suitable for use as a tooltip or
	 *         other context help display.
	 */
	...
	public static String getFieldHelp(Class aTableClass, String fieldname) {
		...
		if (aTableClass == Specimen.class) {
			...
			if (fieldname.equalsIgnoreCase("IdentificationRemarks")) {
	 ...
	 *
	 * @param aTableClass
	 *            the class for the table in which the field occurs.
	 * @param fieldname
	 *            the name of the field (case insensitive).
	 * @return true if the field is allowed to be updated by an external process,
	 *         false otherwise.
	 */
	public static boolean isFieldExternallyUpdatable(Class aTableClass, String fieldname) {
		...
		if (aTableClass == Specimen.class) {
			...
			if (fieldname.equalsIgnoreCase("HigherGeography")) {
				result = true;
			}
			if (fieldname.equalsIgnoreCase("SpecificLocality")) {
				result = true;
			...
			if (fieldname.equalsIgnoreCase("VerbatimCollector")) {
				result = true;
			}
			if (fieldname.equalsIgnoreCase("VerbatimCollection")) {
				result = true;
			}
			if (fieldname.equalsIgnoreCase("VerbatimNumbers")) {
				result = true;
			}
			if (fieldname.equalsIgnoreCase("VerbatimUnclassifiedText")) {
				result = true;
			}
			if (fieldname.equalsIgnoreCase("VerbatimClusterIdentifier")) {
				result = true;
			}
			if (fieldname.equalsIgnoreCase("ExternalWorkflowProcess")) {
				result = true;
			}
			if (fieldname.equalsIgnoreCase("ExternalWorkflowDate")) {
				result = true;
			}
			if (fieldname.equalsIgnoreCase("Minimum_Elevation")) {
				result = true;
			}
			if (fieldname.equalsIgnoreCase("Maximum_Elevation")) {
				result = true;
			}
			if (fieldname.equalsIgnoreCase("MinimumElevationSt")) {
				result = true;
			}
			if (fieldname.equalsIgnoreCase("MaximumElevationSt")) {
				result = true;
			}
			if (fieldname.equalsIgnoreCase("Elev_Units")) {
				result = true;
			}
			if (fieldname.equalsIgnoreCase("CollectingMethod")) {
				result = true;
			...
			if (fieldname.equalsIgnoreCase("Microhabitat")) {
				result = true;
			}
			if (fieldname.equalsIgnoreCase("AssociatedTaxon")) {
				result = true;
		...
		if (aTableClass == Determination.class) {
			if (fieldname.equalsIgnoreCase("VerbatimText")) {
				result = true;
			}
		}
		if (aTableClass == Number.class) {
	 ...
	 * @param fieldname
	 *            the field to check (not case sensitive)
	...
	/**
	 * Identify whether a field in a table contains values that are process
	 * metadata.
	 *
	 * @param aTableClass
	 *            table to check
	 * @param fieldname
	 *            field in aTableClass
