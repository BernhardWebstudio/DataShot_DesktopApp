...
public class LabelEncoder {
  ...
  private static final Logger log = LoggerFactory.getLogger(LabelEncoder.class);

  private final UnitTrayLabel label;

  public LabelEncoder(UnitTrayLabel aLabel) { label = aLabel; }

  @SuppressWarnings("hiding")
  public static boolean printList(List<UnitTrayLabel> taxa)
    ...
    try {
      ...
      while (i.hasNext()) {
        ...
        for (int toPrint = 0; toPrint < label.getNumberToPrint(); toPrint++) {
          ...
          // Generate a text and a barcode cell for each, and add to array for
          // page
          ...
          Chunk normal = null; // normal font prefix to preceed specific epithet
                               // (nr. <i>epithet</i>)
          if (label.getSpecificEpithet().contains(".") ||
              label.getSpecificEpithet().contains("[")) {
            if (label.getSpecificEpithet().startsWith("nr. ")) {
              normal = new Chunk("nr. ");
              normal.setFont(
                  new Font(Font.FontFamily.TIMES_ROMAN, 11, Font.NORMAL));
              species =
                  new Chunk(label.getSpecificEpithet().trim().substring(4));
              species.setFont(
                  new Font(Font.FontFamily.TIMES_ROMAN, 11, Font.ITALIC));
            } else {
              species.setFont(
                  new Font(Font.FontFamily.TIMES_ROMAN, 11, Font.NORMAL));
            }
          ...
          Chunk subspecies =
              new Chunk(s + label.getSubspecificEpithet().trim());
      ...
      throw new PrintFailedException(
          "Ran out of memory, too many labels at once.");
    }
    return result;
  }

  private static void addPageWithColCells(PdfPTable table,
                                          ArrayList<PdfPCell> cells,
                                          ArrayList<PdfPCell> cells_barcode) {
    for (int x = 0; x < 6; x++) {
      if (cells.get(x) == null) {
