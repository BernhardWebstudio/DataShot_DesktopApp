...
public class LatLong implements Serializable, Cloneable {
	...
	/*
	 * Valid values for origLatLongUnits: decimal degrees deg. min. sec. degrees
	 * dec. minutes unknown
	 */
	...
	/**
	 * Check whether this object has the same properties as a similar object. Note
	 * that due to historical reasons, not every property is compared. If need to
	 * check for strict equality, compare the id's! Examples of uncompaired
	 * properties:
	 *
	 * @param coord
	 * @return whether these few properties are equal.
	 * @see LatLong#equalsOneD(LatLong, LatLong) TODO: not used, but if, please
	 *      refactor.
	 */
	@Override
	 ...
	 * Examples of uncompaired properties: - georefmethod: had different defaults
	 * over time, so we could get false positives - fieldVerifiedFg: had undefined
	 * behaviour in some cases - latLongForNnpFg: had undefined behaviour in some
	 * cases
	 *
	 * @param subject
	 *            this object may have null/""/0.0 values to return true
	 * @param defaultVal
	 *            this object has the acceptable default values to return true
	 * @return
	 */
	private boolean equalsOneD(LatLong subject, LatLong defaultVal) {
		return subject.acceptedLatLongFg == defaultVal.acceptedLatLongFg
				...
				&& emptyOrEqual(subject.extent, defaultVal.extent) && emptyOrEqual(subject.latDeg, defaultVal.latDeg)
				&& emptyOrEqual(subject.latDir, defaultVal.latDir) && emptyOrEqual(subject.latMin, defaultVal.latMin)
