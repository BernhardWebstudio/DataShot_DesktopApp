...
public class Users implements Serializable {
	...
	public static final String[] ROLES = {ROLE_ADMINISTRATOR, ROLE_CHIEF_EDITOR, ROLE_EDITOR, ROLE_FULL,
			ROLE_DATAENTRY};
	private static final long serialVersionUID = -5258394845638721810L;
	/**
	 * A text description suitable for error messages that describes the rules
	 * implemented in testProposedPassword()
	 *
	...
	public Users(String username, String fullname, String description, String role) {
		this.username = username;
		this.fullname = fullname;
	 ...
	 * rules implemented here should match the rules described in
	 * PASSWORD_RULES_MESSAGE
	 *
	 * @param proposal
	 *            a string that has been proposed as a new password for user.
	...
	/**
	 * Test whether a user has rights under a role. Use
	 * UsersLifeCycle.isUserAdministrator() or UsersLifeCycle.isUserChiefEditor() to
	 * test if a particular user has administrative rights instead of this method,
	 * as it validates the rights against an underlying Users database record, while
	 * this method only validates against an instance of the Users class. This
	 * isUserRole() test returns true if the user is a member of a more privileged
	 * class of users, e.g. isUserRole(Users.ROLE_DATAENTRY) returns true for a user
	 * who is in role ROLE_FULL.
	 *
	 * @param aUserRole
	 *            the role to test, one of Users.ROLE_*
	...
	public boolean isUserRole(String aUserRole) {
		...
		if (this.role.equals(aUserRole)) {
			...
			if (this.role.equals(Users.ROLE_CHIEF_EDITOR)
					&& (aUserRole.equals(Users.ROLE_DATAENTRY) || aUserRole.equals(Users.ROLE_FULL)
							|| aUserRole.equals(Users.ROLE_CHIEF_EDITOR) || aUserRole.equals(Users.ROLE_EDITOR))) {
				// Role chief editor includes roles full access, editor and data entry.
				result = true;
			}
			if (this.role.equals(Users.ROLE_EDITOR) && (aUserRole.equals(Users.ROLE_DATAENTRY)
					|| aUserRole.equals(Users.ROLE_FULL) || aUserRole.equals(Users.ROLE_EDITOR))) {
