...
public class WorkFlowStatus {
	 ...
	 * populated from taxon label data encoded in a barcode on the unit tray label.
	 */
	public static final String STAGE_1 = "Taxon Entered";

	/**
	 * Specimen record where the text from the pin labels has been transcribed into
	 * verbatim database fields by a data entry person (or in an external
	...
	/**
	 * Specimen record where the text from the pin labels has been transcribed into
	 * database fields by a data entry person.
	 */
	public static final String STAGE_2 = "Text Entered";

	/**
	 * State change for a specimen record where a human or an automated process has
	 * identified a quality control issue with the record.
	 */
	 ...
	 * has been reviewed by a taxonomist. Record is now only editable by Admins.
	 */
	...
	private static final String[] VALUES = {STAGE_0, STAGE_1, STAGE_VERBATIM, STAGE_CLASSIFIED, STAGE_2, STAGE_QC_FAIL,
			STAGE_QC_PASS, STAGE_CLEAN, STAGE_DONE, STAGE_DELETE};
	...
	/**
	 * Obtain the list of all workflow status values that can be used when a record
	 * may be put into a verbatim captured state.
	 *
	...
	/**
	 * Obtain the list of all workflow status values that can be used when a record
	 * may be put into a verbatim classified state.
	 *
	 * @return array of string constants.
	 */
	 ...
	 *            a current workflow state to check
	 * @return true if the record can be placed into state verbatim captured from
	 *         its current (other) state, false otherwise. False if the current
	 *         state is already verbatim captured.
	 */
	...
	public static boolean allowsVerbatimUpdateOverwrite(String workflowStatus) {
		...
		if (workflowStatus.equals(STAGE_VERBATIM)) {
			result = true;
		}
	...
	public static boolean allowsClassifiedUpdate(String workflowStatus) {
		...
		if (workflowStatus.equals(STAGE_CLASSIFIED)) {
			result = true;
		}
