...
abstract public class AbstractFileScanJob implements RunnableJob, Runnable {
	...
	private static final Logger log = LoggerFactory.getLogger(AbstractFileScanJob.class);
	 ...
	 *
	 * @param counter
	 *            to apply the error to
	 * @param filename
	 *            the errored file
	 * @param barcode
	 *            the barcode as detected
	 * @param exifComment
	 *            the comment, exif as read from file
	 * @param parser
	 *            the parser used
	 * @param barcodeInImageMetadata
	 *            whether the barcode was extracted from metadata
	 * @param log
	 */
	static void logMismatch(ScanCounterInterface counter, String filename, String barcode, String exifComment,
			TaxonNameReturner parser, boolean barcodeInImageMetadata, Logger log) {
	 ...
	 *
	 * @param parser
	 *            source of family, subfamily
	 * @param s
	 *            the specimen to set the values on
	 */
	...
	/**
	 * Do actual processing of one file. Static to enable threaded, state-less
	 * execution
	 *
	 * @param containedFile
	 *            the file to process
	 * @param counter
	 *            the counter to keep track of success/failure
	 * @param locks
	 *            the locks to use to prevent concurrency issues
	 */
	protected static void checkFile(File containedFile, ScanCounterInterface counter, Lock[] locks) {
		AbstractFileScanJob.checkFile(containedFile, counter, locks, false);
	}
	 ...
	 *            the locks to use to prevent concurrency issues
	 * @param ask
	 *            whether to show the "What's this image" dialog on fail
	 */
	protected static void checkFile(File containedFile, ScanCounterInterface counter, Lock[] locks, boolean ask) {
		Singleton.getSingletonInstance().getProperties().getProperties()
				.setProperty(ImageCaptureProperties.KEY_LASTPATH, containedFile.getPath());
		...
		try {
			...
			if (matches != null && matches.size() == 1 && matches.get(0).getRawBarcode() == null
					&& matches.get(0).getRawExifBarcode() == null && (matches.get(0).getDrawerNumber() == null
			...
			if (matches == null || matches.size() == 0) {
				AbstractFileScanJob.createDatabaseRecordForFile(containedFile, counter, reattach, imageLifeCycle,
						existingTemplate, locks, ask);
			} else {
			...
			counter.appendError(new RunnableJobError(containedFile.getName(), "", "Could not read file",
	...
	/**
	 * Create a new image database record. Static to enable threaded, state-less
	 * execution
	 *
	 * @param containedFile
	 *            the file path relative to the start
	 * @param counter
	 *            to count errors
	 * @param reattach
	 *            whether there is already a database record existing, to be
	 *            overridden
	 * @param imageLifeCycle
	 *            the repository to sage to
	 * @param image
	 *            providing access to path & filename
	 * @param ask
	 *            whether to ask what type of file this is
	 * @throws UnreadableFileException
	 *             if the file could not be read
	 */
	protected static void createDatabaseRecordForFile(File containedFile, ScanCounterInterface counter,
			boolean reattach, ICImageLifeCycle imageLifeCycle, ICImage image, Lock[] locks, boolean ask)
			throws UnreadableFileException {
		...
		log.debug("Detected Template: " + templateId);
		log.debug("Candidate image file, catalog nr barcode status: {}",
				candidateImageFile.getCatalogNumberBarcodeStatus());
		...
		if (labelRead != null) {
			...
			try {
				...
				RunnableJobError error = new RunnableJobError(image.getFilename(), "OCR Failed", barcode, exifComment,
						"Couldn't find text to OCR for taxon label.", null, null, e, RunnableJobError.TYPE_NO_TEMPLATE);
				counter.appendError(error);
			}
			if (rawOCR == null) {
				rawOCR = "";
			...
			if (((UnitTrayLabelParser) parser).isParsedFromJSON()) {
				RunnableJobError error = new RunnableJobError(image.getFilename(), "OCR Failover found barcode.",
						barcode, exifComment,
						"Couldn't read Taxon barcode, failed over to OCR, but OCR found taxon barcode.", parser, null,
						null, RunnableJobError.TYPE_FAILOVER_TO_OCR);
				counter.appendError(error);
			} else {
				RunnableJobError error = new RunnableJobError(image.getFilename(), "TaxonLabel read failed.", barcode,
						exifComment, "Couldn't read Taxon barcode, failed over to OCR only.", parser, null, null,
		...
		if (matcher.matchesPattern(exifComment) || matcher.matchesPattern(barcode)) {
			isSpecimenImage = true;
			System.out.println(containedFile.getName() + " is a Specimen Image");
		} else {
			if (exifComment != null && exifComment.matches(Singleton.getSingletonInstance().getProperties()
				...
				// ask
				if (ask) {
					log.debug("Need to ask what type this file is");
					ImageDisplayFrame resultFrame = new ImageDisplayFrame();
					resultFrame.setBarcode("QR=" + barcode + " Comment=" + exifComment);
					try {
						resultFrame.loadImagesFromFileSingle(containedFile, template, null);
						...
						System.out.println(e2.getMessage());
						// TODO: possibly detect
					}
					WhatsThisImageDialog askDialog = new WhatsThisImageDialog(resultFrame, containedFile);
					askDialog.setVisible(true);
					if (askDialog.isSpecimen()) {
						isSpecimenImage = true;
						barcode = askDialog.getBarcode();
					}
					...
					if (templateId.equals(PositionTemplate.TEMPLATE_NO_COMPONENT_PARTS)) {
						error = new RunnableJobError(image.getFilename(), barcode, barcode, exifComment,
								"Image does not appear to contain a barcode in a templated position", null, null, null,
								RunnableJobError.TYPE_NO_TEMPLATE);
					} else {
						error = new RunnableJobError(
								image.getFilename(), barcode, barcode, exifComment, "Neither comment '" + exifComment
										+ "' nor barcode '" + barcode + "' did " + "match known patterns.",
					...
					if (reattach) {
						counter.incrementFilesExisting();
						log.debug("Existing image record for file {} still has problems, counted as existing",
		...
		if (isSpecimenImage) {
			AbstractFileScanJob.createDatabaseRecordForSpecimen(containedFile, counter, image, barcode, exifComment,
					parser, labelRead, state, locks);
		...
		try {
			...
			log.error(e.getMessage(), e);
			counter.incrementFilesFailed();
			String failureMessage = "Failed to save image record for image " + containedFile.getName() + ".  "
					+ e.getMessage();
			RunnableJobError error = new RunnableJobError(image.getFilename(), "Save Failed", image.getFilename(),
					image.getPath(), failureMessage, null, null, null, RunnableJobError.TYPE_SAVE_FAILED);
			counter.appendError(error);
		}
		if (isSpecimenImage) {
	 ...
	 * ICImage. Static to enable threaded, state-less execution. BUT, since multiple
	 * images could reference the same specimen, we have to lock stuff
	 *
	 * @param containedFile
	 *            the file of the image
	 * @param counter
	 *            counter to add errors
	 * @param image
	 *            the image to associate with the specimen
	 * @param barcode
	 *            the barcode associated with the specimen
	 * @param exifComment
	 *            the exif metadata comment to enable additional data extraction
	 * @param unitTrayLabel
	 *            the unit tray label of the Specimen
	 * @param state
	 *            the workflow state we are in with the specimen/image
	 */
	protected static void createDatabaseRecordForSpecimen(File containedFile, ScanCounterInterface counter,
			ICImage image, String barcode, final String exifComment, TaxonNameReturner parser,
			UnitTrayLabel unitTrayLabel, String state, Lock[] locks) {
		BarcodeMatcher matcher = Singleton.getSingletonInstance().getBarcodeMatcher();
		final String rawBarcode = barcode;
		if (!rawBarcode.equals(exifComment)) {
			// Log the mismatch
			logMismatch(counter, image.getFilename(), barcode, exifComment, parser, matcher.matchesPattern(exifComment),
					log);
		}

		Singleton.getSingletonInstance().getMainFrame()
				.setStatusMessage("Creating new specimen record for file " + containedFile.getName() + ".");
		Specimen s = new Specimen();
		if ((!matcher.matchesPattern(barcode)) && matcher.matchesPattern(exifComment)) {
		...
		if (lock == null) {
			log.warn("Lock null, fetched from " + barcode.hashCode() + ", " + locks.length + " and "
		...
		try {
			...
			if (existingSpecimens != null && existingSpecimens.size() > 0) {
				...
				for (Specimen specimen : existingSpecimens) {
					...
					try {
						...
						counter.appendError(new RunnableJobError(containedFile.getName(), barcode,
								specimen.getSpecimenId().toString(), "Failed adding image to existing Specimen", e,
			...
			if (unitTrayLabel != null) {
				...
				if (sex.equalsIgnoreCase("worker")) {
					...
					SpecimenPartAttribute workerCaste = new SpecimenPartAttribute(worker, "caste", "worker", "", "", "",
							new Date());
					...
					SpecimenPartAttribute gyneCaste = new SpecimenPartAttribute(gyne, "caste", "gyne", "", "", "",
							new Date());
			...
			try {
				...
				try {
					...
					RunnableJobError error = new RunnableJobError(image.getFilename(), barcode, rawBarcode, exifComment,
							errorMessage, parser, (DrawerNameReturner) parser, e2, RunnableJobError.TYPE_SAVE_FAILED);
					counter.appendError(error);
				}
			} catch (SaveFailedException e) {
				...
				try {
					...
					if (((DrawerNameReturner) parser).getDrawerNumber().length() > MetadataRetriever
						...
						RunnableJobError error = new RunnableJobError(image.getFilename(), barcode, rawBarcode,
								exifComment, badParse, parser, (DrawerNameReturner) parser, e,
								RunnableJobError.TYPE_BAD_PARSE);
						counter.appendError(error);
					} else {
						RunnableJobError error = new RunnableJobError(image.getFilename(), barcode, rawBarcode,
								exifComment, e.getMessage(), parser, (DrawerNameReturner) parser, e,
								RunnableJobError.TYPE_SAVE_FAILED);
						counter.appendError(error);
					}
				} catch (Exception err) {
					...
					if (s.getDrawerNumber() == null) {
						...
						if (s.getDrawerNumber().length() > MetadataRetriever.getFieldLength(Specimen.class,
								"DrawerNumber")) {
					...
					RunnableJobError error = new RunnableJobError(image.getFilename(), barcode, rawBarcode, exifComment,
							badParse, parser, (DrawerNameReturner) parser, err, RunnableJobError.TYPE_SAVE_FAILED);
					counter.appendError(error);
					counter.incrementFilesFailed();
					s = null;
				}
				...
				log.error("Error", ex);
				RunnableJobError error = new RunnableJobError(image.getFilename(), barcode, rawBarcode, exifComment,
						ex.getMessage(), parser, (DrawerNameReturner) parser, ex, RunnableJobError.TYPE_SAVE_FAILED);
				counter.appendError(error);
			}
		} finally {
			if (lock != null) {
