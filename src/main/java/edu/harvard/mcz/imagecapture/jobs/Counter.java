...
public class Counter implements ScanCounterInterface {
	...
	public String getBarcodeSequenceDescription() {
		if (barcodes.size() > 2) {
			...
			if (barcodesMissing.size() == 0 && barcodeSeqWhereMoreThanOneMisses.size() == 0) {
				return "No missing barcodes detected between " + barcodes.get(0) + " and "
						+ barcodes.get(barcodes.size() - 1) + ".\n";
			}
			if (barcodesMissing.size() + barcodeSeqWhereMoreThanOneMisses.size() / 2 > 5) {
				return "Too many missing barcodes in sequence, " + barcodesMissing.size() + " single and "
						+ (barcodeSeqWhereMoreThanOneMisses.size() / 2) + " bigger distance.";
			} else {
				...
				if (barcodeSeqWhereMoreThanOneMisses.size() > 0) {
					for (int i = 0; i < barcodeSeqWhereMoreThanOneMisses.size() / 2; ++i) {
						report.append("between ").append(barcodeSeqWhereMoreThanOneMisses.get(2 * i)).append(" & ")
								.append(barcodeSeqWhereMoreThanOneMisses.get(2 * i + 1));
	...
	public String getNumbersToCopy() {
		...
		Map<String, Integer> propertyKeyAndValue = Stream
				.of(new Object[][]{{ImageCaptureProperties.KEY_EXCEL_COL_NR_FILES, getTotal()},
