...
public class JobCleanDirectory implements RunnableJob, Runnable {
	...
	private static final Logger log = LoggerFactory.getLogger(JobCleanDirectory.class);
	...
	/**
	 * Default constructor, launch a dialog. JobCleanDirectory only works on a
	 * directory within the mount path, and must find the target directory (as
	 * readable) before running, otherwise image records for files that do exist
	 * could be removed when the file system is not mounted (at the configured
	 * location).
	 */
	...
	/**
	 * Create a clean images job to bring up dialog to pick a specific directory on
	 * which to clean up image records.
	 * <p>
	 * Behavior:
	 * <p>
	 ...
	 *
	 * @param whatToScan
	 *            one of SCAN_SPECIFIC, SCAN_SELECT
	 * @param startAt
	...
	private List<File> getFileList() {
		...
		if (Singleton.getSingletonInstance().getProperties().getProperties()
			...
			if (!ImageCaptureProperties.isInPathBelowBase(startPoint)) {
				...
				String message = "Can only scan and cleanup files in a selected "
						+ "directory within the base directory  (" + base
	...
	private void cleanupFile(File file) {
		log.debug("File to cleanup: {}", file);
		String filename = file.getName();

		if (file.exists()) {
			log.debug("Existing file: {}", file);
			...
			while (iter.hasNext()) {
				ICImage image = iter.next();
				log.debug("Image path: {}", image.getPath());
				log.debug("Image filename: {}", image.getFilename());
				try {
					RunnableJobError error = new RunnableJobError(image.getPath(), image.getRawBarcode(),
							"Deleted while cleaning up.", new Exception(), RunnableJobError.TYPE_UNKNOWN);
					ils.delete(image);
