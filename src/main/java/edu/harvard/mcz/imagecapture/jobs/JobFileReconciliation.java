...
/**
 * JobFileReconciliation, walk through the image directories and find all files
 * that have a filename pattern that matches that of an image file that should
 * be in the database, check if they are, and report if they are not present in
 * the database.
 */
public class JobFileReconciliation implements RunnableJob, Runnable {

	private static final Logger log = LoggerFactory.getLogger(JobFileReconciliation.class);
			...
			for (int i = 0; i < containedFiles.length; i++) {
				if (runStatus != RunStatus.STATUS_TERMINATED) {
					...
					Singleton.getSingletonInstance().getMainFrame()
							.setStatusMessage("Reconciling: " + fileToCheck.getName());
					// Check to see if this is a directory
					if (fileToCheck.isDirectory()) {
						if (fileToCheck.getName().equals("thumbs")) {
							log.debug("Skipping thumbnail directory: " + fileToCheck.getPath() + File.separator
									+ fileToCheck.getName());
						} else {
						...
						if (fileToCheck.getName().matches(Singleton.getSingletonInstance().getProperties()
							...
							if (matches != null && matches.size() == 1) {
								...
								RunnableJobError error = new RunnableJobError(fileToCheck.getName(),
										barcode.toString().trim(), ImageCaptureProperties.getPathBelowBase(fileToCheck),
										"", "More than one database (Image) record for this file.", null, null, null,
										RunnableJobError.TYPE_DUPLICATE);
								...
								RunnableJobError error = new RunnableJobError(fileToCheck.getName(),
										barcode.toString().trim(), ImageCaptureProperties.getPathBelowBase(fileToCheck),
										"", "No database (Image) record for this file.", null, null, null,
	...
	private void report(Counter counter) {
		...
		RunnableJobReportDialog errorReportDialog = new RunnableJobReportDialog(
				...
				RunnableJobErrorTableModel.TYPE_FILE_RECONCILIATION, "Image File/Image Record reconciliation Results");
