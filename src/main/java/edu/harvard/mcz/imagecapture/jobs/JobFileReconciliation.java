...
public class JobFileReconciliation implements RunnableJob, Runnable {
    ...
    private static final Logger log =
            LoggerFactory.getLogger(JobFileReconciliation.class);
            ...
            for (int i = 0; i < containedFiles.length; i++) {
                if (runStatus != RunStatus.STATUS_TERMINATED) {
                    ...
                    Singleton.getSingletonInstance().getMainFrame().setStatusMessage(
                            "Reconciling: " + fileToCheck.getName());
                    // Check to see if this is a directory
                    if (fileToCheck.isDirectory()) {
                        if (fileToCheck.getName().equals("thumbs")) {
                            log.debug(
                                    "Skipping thumbnail directory: " + fileToCheck.getPath() +
                                            File.separator + fileToCheck.getName());
                        } else {
                        ...
                        if (fileToCheck.getName().matches(
                            ...
                            dbImage.setPath(
                                    ImageCaptureProperties.getPathBelowBase(fileToCheck));
                            ...
                            if (matches != null && matches.size() == 1) {
                                ...
                                counter.incrementFilesDatabased();
                                log.error(
                                        "File with more than one database match by name and path");
                                RunnableJobError error = new RunnableJobError(
                                        fileToCheck.getName(), barcode.toString().trim(),
                                        ImageCaptureProperties.getPathBelowBase(fileToCheck), "",
                                        "More than one database (Image) record for this file.",
                                        null, null, null, RunnableJobError.TYPE_DUPLICATE);
                                ...
                                if (matches != null) {
                                    ...
                                    while (ri.hasNext()) {
                                        ICImage match = ri.next();
                                        barcode.append(match.getSpecimen().getBarcode())
                                                .append(" ");
                                    }
                                }
                                ...
                                RunnableJobError error = new RunnableJobError(
                                        fileToCheck.getName(), barcode.toString().trim(),
                                        ImageCaptureProperties.getPathBelowBase(fileToCheck), "",
                                        "No database (Image) record for this file.", null, null,
