...
public class JobRecheckForTemplates implements RunnableJob, Runnable {
	...
	private static final Logger log = LoggerFactory.getLogger(JobRecheckForTemplates.class);
	 ...
	 * directory on which to recheck image records for templates.
	 * <p>
	 * Behavior:
	 * <p>
	 * whatToScan=SCAN_ALL, all records having no template and a linked specimen are
	 * rechecked. whatToScan=SCAN_SELECT, startAt is used as starting point for
	 * directory chooser dialog. whatToScan=SCAN_SPECIFIC, startAt is used as
	...
	private void recheckForTemplates(ICImage image) {
		if (image.getSpecimen() != null) {
			...
			try {
				...
				log.error(e.getMessage());
				RunnableJobError error = new RunnableJobError(image.getFilename(), image.getRawBarcode(), "",
						image.getRawExifBarcode(), "Unreadable File Exception checking for template.", null, null, null,
						RunnableJobError.TYPE_NO_TEMPLATE);
				counter.appendError(error);
