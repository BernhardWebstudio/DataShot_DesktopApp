...
public class JobRepeatOCR implements RunnableJob, Runnable {
	...
	/**
	 * Default constructor. Creates an OCR job to repeat the OCR on all images for
	 * all specimens in state OCR.
	 */
	public JobRepeatOCR() {
		scan = SCAN_ALL;
	...
	/**
	 * Create a repeat OCR job to bring up dialog to pick a specific directory on
	 * which to repeat OCR for specimens in state OCR or to repeat OCR for a
	 * specific directory specified by startAt, again for specimens in state OCR.
	 * <p>
	 * Behavior:
	 * <p>
	...
	public boolean registerListener(RunnerListener jobListener) {
		...
		log.debug("Job listener: {}", jobListener);
		...
		try {
			...
			try {
				...
				if (!rawBarcode.equals(exifComment)) {
					...
					// Log the mismatch
					JobAllImageFilesScan.logMismatch(counter, filename, barcode, exifComment, parser,
				...
				if (isSpecimenImage
					...
					if (specimens.size() == 1) {
						...
						if (s.getWorkFlowStatus().equals(WorkFlowStatus.STAGE_0)) {
							...
							log.debug("Specimen collection: {}", s.getCollection());
							...
							try {
								...
								if (foundQRText) {
									...
									log.debug("Workflow status: {}", s.getWorkFlowStatus());
								}
								sh.attachDirty(s);
								...
								try {
									...
									RunnableJobError error = new RunnableJobError(filename, barcode, rawBarcode,
											exifComment, badParse, parser, (DrawerNameReturner) parser, null,
									...
									RunnableJobError error = new RunnableJobError(filename, barcode, rawBarcode,
											exifComment, badParse, parser, (DrawerNameReturner) parser, err,
							...
							RunnableJobError error = new RunnableJobError(filename, barcode, rawBarcode, exifComment,
									...
									(DrawerNameReturner) parser, null, RunnableJobError.TYPE_SAVE_FAILED);
							counter.appendError(error);
						}
					}
				} else {
					...
					RunnableJobError error = new RunnableJobError(filename, barcode, rawBarcode, exifComment,
							"Didn't try to save, not a specimen image, or rawBarcode doesn't match pattern", parser,
							(DrawerNameReturner) parser, null, RunnableJobError.TYPE_SAVE_FAILED);
					counter.appendError(error);
				}
			} catch (Exception ex) {
