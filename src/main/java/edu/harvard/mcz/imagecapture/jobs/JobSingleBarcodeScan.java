...
public class JobSingleBarcodeScan extends AbstractFileScanJob {
	...
	private static final Logger log = LoggerFactory.getLogger(JobSingleBarcodeScan.class);
	 ...
	 * examination of image extracted barcode, and OCR of label data without making
	 ...
	 *
	 * @param persistResult
	 *            if true enables connection to database to persist changes, and
	 *            adds editable form showing the current specimen record in the
	 *            database that matches the barcode extracted from the image,
	 *            creating the image and specimen records if needed.
	 */
	...
	private void init() {
		...
		fileToCheck = FileUtility.askForImageFile(new File(Singleton.getSingletonInstance().getProperties()
	 ...
	 * (non-Javadoc)
	 *
	 * @see edu.harvard.mcz.imagecapture.Runnable#registerListener(edu.harvard.mcz.
	 * imagecapture.RunnerListener)
	 */
	@Override
