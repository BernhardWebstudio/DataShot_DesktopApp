...
class ThumbnailBuilderJob implements Runnable, RunnableJob {
	...
	private static final Logger log = LoggerFactory.getLogger(ThumbnailBuilderJob.class);
	File startPoint;
	...
	public ThumbnailBuilderJob(File aStartPoint, AtomicInteger counter) {
		...
		thumbHeight = Singleton.getSingletonInstance().getProperties().getProperties()
				.getProperty(ImageCaptureProperties.KEY_THUMBNAIL_HEIGHT);
		thumbWidth = Singleton.getSingletonInstance().getProperties().getProperties()
	...
	public void run() {
		...
		if (startPoint.isDirectory() && (!startPoint.getName().equals("thumbs"))) {
			...
			if (filesToThumbCount > 0) {
				...
				String mogrify = Singleton.getSingletonInstance().getProperties().getProperties()
				...
				if (mogrify == null || mogrify.trim().length() > 0) {
					...
					List<String> runCommand = new ArrayList<>(
							Arrays.asList(mogrify, "-path", "thumbs", "-resize", thumbWidth + "x" + thumbHeight));
					...
					try {
						...
						Singleton.getSingletonInstance().getMainFrame()
								.setStatusMessage("Error creating thumbnails " + e.getMessage());
						makeWithJava = true;
				...
				if (makeWithJava) {
					for (int i = 0; i < potentialFilesToThumb.length; i++) {
						try {
							log.debug("Attempting thumbnail generation with java in " + startPoint.getPath() + " to "
									+ thumbsDir.getPath());
							...
							while (it.hasNext() && thumbRunStatus != RunStatus.STATUS_CANCEL_REQUESTED) {
								...
								if (!file.isDirectory() && file.exists() && file.canRead()) {
									...
									if (file.getName().matches(Singleton.getSingletonInstance().getProperties()
										...
										File target = new File(
												thumbsDir.getPath() + File.separatorChar + file.getName());
										// log.debug("Debug {}", target.getPath());
										if (!target.exists()) {
											BufferedImage img = new BufferedImage(Integer.parseInt(thumbWidth),
													Integer.parseInt(thumbHeight), BufferedImage.TYPE_INT_RGB);
											img.createGraphics().drawImage(
													ImageIO.read(file).getScaledInstance(80, 120, Image.SCALE_SMOOTH),
													0, 0, null);
											ImageIO.write(img, "jpg", target);
											creationCounter++;
										}
										log.debug(
												"Created thumbnail from " + file.getPath() + " to " + target.getPath());
									}
									setThumbPercentComplete((int) (((float) creationCounter / totalFiles) * 100));
								}
							}
							if (creationCounter > 0) {
								String message = "Finished creating thumbnails (" + creationCounter + ") in: "
										+ startPoint.getPath();
								Singleton.getSingletonInstance().getMainFrame().setStatusMessage(message);
								log.debug(message);
							}
