...
public class CollectorLifeCycle extends GenericLifeCycle {
	...
	private static final Logger log = LoggerFactory.getLogger(CollectorLifeCycle.class);
	...
	/**
	 * Delete orphaned collector records (collectors without an associated
	 * specimen). This can happen due to data inconsistencies or if specimen
	 * deletion doesn't properly cascade.
	 *
	...
	public int deleteOrphanedCollectors() {
		...
		try {
			...
			int deletedCount = 0;
			try {
				Query query = session.createQuery("DELETE FROM Collector c WHERE c.specimen IS NULL");
	...
	public List<Collector> findByIds(List ids) {
		try {
			...
			try {
				...
				Query<Collector> query = session.createQuery("SELECT c FROM Collector c " + "WHERE c.id IN (?1)");
