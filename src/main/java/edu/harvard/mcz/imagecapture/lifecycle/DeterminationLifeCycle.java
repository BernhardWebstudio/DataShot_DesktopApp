...
public class DeterminationLifeCycle {
	...
	private static final Logger log = LoggerFactory.getLogger(DeterminationLifeCycle.class);
	...
	public Determination findById(java.lang.Long id) {
		...
		try {
			...
			try {
				session.beginTransaction();
				instance = (Determination) session.get("edu.harvard.mcz.imagecapture.data.Determination", id);
				if (instance == null) {
