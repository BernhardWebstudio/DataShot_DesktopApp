...
public abstract class GenericLifeCycle<T> {
  ...
  public T findOneBy(String propertyPath, Object value) {
    log.debug("getting one " + this.tCLass.toGenericString() + " by " +
              propertyPath);
    try {
      T instance = null;
  ...
  public List<T> findBy(Map<String, Object> propertyValueMap, int maxResults,
                        int offset, boolean like) {
    log.debug("finding " + this.tCLass.toGenericString() +
              " instance by hashmap: " + propertyValueMap.toString());
    try {
      Session session = this.getSession();
      try {
        session.beginTransaction();
        ...
        for (Map.Entry<String, Object> entry : propertyValueMap.entrySet()) {
          ...
          if (entry.getValue() instanceof Specification) {
            p = ((Specification<T, Long>)entry.getValue())
                    .toPredicate(root, cr, cb);
            ...
            if (value instanceof Boolean || value instanceof String ||
                value instanceof Date || value instanceof java.lang.Number) {
              p = cb.equal(propertyToMatch, entry.getValue());
            } else {
              log.warn("Will not handle property: " + entry.getKey() +
                       " with value type "
        ...
        cr.where(cb.and(propertyValueRelations.toArray(
            new Predicate[propertyValueRelations.size()])));
  ...
  https://stackoverflow.com/questions/19418500/get-table-column-names-in-hibernate
      MetamodelImplementor metamodel = (MetamodelImplementor)
  ...
  public List<T> findByExample(T instance, int maxResults, int offset) {
    ...
    try {
      ...
      List<String> persistentAttributes =
          this.getEntityAttributes(instance, session);
      log.debug("found entity attributes: " + persistentAttributes);
      int attributesSize = persistentAttributes.size();
      HashMap<String, Object> propertyValueRelations =
      ...
      for (int i = 0; i < attributesSize; i++) {
        ...
        try {
          getter = instance.getClass().getMethod(
              "get" + attrName.substring(0, 1).toUpperCase() +
              attrName.substring(1));
          propertyValue = getter.invoke(instance);
        } catch (Exception e) {
          log.warn("Failed to get getter method or value of '" + attrName +
  ...
  public String[] runQueryToGetStrings(ArrayList<String> collections,
                                       String sql, Logger log) {
    Session session = this.getSession();
    try {
      session.beginTransaction();
