...
public class HigherTaxonLifeCycle {
	...
	public boolean isMatched(String aFamily, String aSubfamily) {
		...
		try {
			String sql = "Select distinct family, subfamily from HigherTaxon ht where soundex(ht.family) = soundex(:family) and soundex(ht.subfamily) = soundex(:subfamily)";
			Session session = HibernateUtil.getSessionFactory().getCurrentSession();
			try {
				session.beginTransaction();
				Query q = session.createQuery(sql);
	...
	public boolean isMatched(String aFamily, String aSubfamily, String aTribe) {
		boolean result = false;
		try {
			String sql = "Select distinct family, subfamily from HigherTaxon ht where soundex(ht.family) = soundex(:family) and soundex(ht.subfamily) = soundex(:subfamily) and soundex(ht.tribe) = soundex(:tribe)";
			Session session = HibernateUtil.getSessionFactory().getCurrentSession();
			try {
				session.beginTransaction();
				Query q = session.createQuery(sql);
	 ...
	 *
	 * @param aFamily
	 * @return a String containing the matched family name, null if no match or a
	 *         connection problem.
	 */
	public String findMatch(String aFamily) {
		String result = null;
		try {
			String sql = "SELECT DISTINCT family FROM HigherTaxon ht WHERE soundex(ht.family) = soundex(:family)";
			Session session = HibernateUtil.getSessionFactory().getCurrentSession();
			try {
				session.beginTransaction();
				Query q = session.createQuery(sql);
	...
	/**
	 * Find the first soundex match to both family and subfamily in the Higher Taxon
	 * authority file.
	 *
	 * @param aFamily
	 * @param aSubfamily
	 * @return a String array with the 0th element being the family name and the 1st
	 *         element being the subfamily name, null if no match or a connection
	 *         problem.
	 */
	 ...
	 *
	 * @param aFamily
	 *            the text string to check for a matching family name.
	 * @param aSubfamily
	 *            the text string to check for a matching subfamily name.
	 * @param aTribe
	 *            the text string to check for a matching tribe name.
	 * @return a string array of {family,subfamily,tribe} from the database, or null
	...
	public String[] findMatch(String aFamily, String aSubfamily, String aTribe) {
		...
		try {
			String sql = "Select distinct family, subfamily, tribe from HigherTaxon ht where "
					+ "soundex(ht.family) = soundex(:family) and " + "soundex(ht.subfamily) = soundex(:subfamily) and "
