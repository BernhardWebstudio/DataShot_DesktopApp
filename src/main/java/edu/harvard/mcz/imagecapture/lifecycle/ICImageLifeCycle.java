...
/**
 * Home object for domain model class ICImage. Refactored from generated EJB
 * home class to use with local singleton HibernateUtil for sessions instead of
 * EJB. Renamed from Home to LifeCycle to prevent overwriting by Hibernate Tools
 *
 * @see ICImage
 */
public class ICImageLifeCycle extends GenericLifeCycle<ICImage> {

	private static final Logger log = LoggerFactory.getLogger(ICImageLifeCycle.class);
	...
	public static List<ICImage> findMismatchedImages() {
		...
		try {
			...
			try {
				results = (List<ICImage>) session.createQuery(
		...
		try {
			...
			try {
				results = (List<ICImage>) session
						.createQuery("From ICImage i where specimen is null and drawerNumber = '" + aDrawerNumber.trim()
								+ "' order by i.filename")
						.list();
	...
	public static List<ICImage> findNotDrawerNoTemplateImages() {
		...
		try {
			...
			try {
				results = (List<ICImage>) session
						.createQuery("From ICImage i where specimen is not null and templateId = '"
								+ PositionTemplate.TEMPLATE_NO_COMPONENT_PARTS.trim() + "' order by i.filename")
						.list();
				log.debug(
						"find all whole image only records with specimens successful, result size: " + results.size());
				session.getTransaction().commit();
			} catch (HibernateException e) {
				session.getTransaction().rollback();
	...
	public List<ICImage> findAllInDir(String path) {
		...
		try {
			...
			List<ICImage> results = null;
			try {
				results = session.createQuery("From ICImage i where path = :path order by i.filename", ICImage.class)
						.setParameter("path", path).list();
	...
	public List<ICImage> findByIds(List<Long> ids) {
		...
		try {
			...
			try {
				...
				Query<ICImage> query = session.createQuery("SELECT i FROM ICImage i WHERE i.imageId IN (?1)");
