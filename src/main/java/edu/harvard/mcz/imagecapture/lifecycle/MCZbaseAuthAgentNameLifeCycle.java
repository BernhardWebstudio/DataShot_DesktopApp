...
public class MCZbaseAuthAgentNameLifeCycle {
	private static final Logger log = LoggerFactory.getLogger(MCZbaseAuthAgentNameLifeCycle.class);
		...
		try {
			...
			try {
				results = (List<MCZbaseAuthAgentName>) session
						.createQuery("from MCZbaseAuthAgentName h order by h.agent_name").setReadOnly(true).list();
