...
public class MCZbaseGeogAuthRecLifeCycle {
	private static final Logger log = LoggerFactory.getLogger(MCZbaseGeogAuthRecLifeCycle.class);
	...
	public List<MCZbaseGeogAuthRec> findAll() {
		...
		try {
			...
			try {
				results = (List<MCZbaseGeogAuthRec>) session
						.createQuery("from MCZbaseGeogAuthRec h order by h.higher_geog").setReadOnly(true).list();
