...
public class NumberLifeCycle {
	...
	/**
	 * Retrieve, as a string array suitable for populating a pick list, the distinct
	 * values of Number.numberTypes. Example usage: <code>
	...
	public Number findById(java.lang.Long id) {
		...
		try {
			...
			session.beginTransaction();
			try {
				instance = session.get(Number.class, id);
				if (instance == null) {
