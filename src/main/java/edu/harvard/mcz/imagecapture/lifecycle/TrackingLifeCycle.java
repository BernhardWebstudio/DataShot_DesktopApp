...
public class TrackingLifeCycle extends GenericLifeCycle {
	...
	private static final Logger log = LoggerFactory.getLogger(TrackingLifeCycle.class);
	...
	public List<Specimen> findSpecimensByUser(String user) {
		...
		try {
			...
			session.beginTransaction();
			try {
				results = session.createQuery("select Specimen From Tracking t join Specimen where Users = " + user
						+ " order by t.eventDateTime ").list();
				log.debug("find all successful, result size: " + results.size());
				System.out.println("find all successful, result size: " + results.size());
				session.getTransaction().commit();
			} catch (HibernateException e) {
				session.getTransaction().rollback();
	...
	/**
	 * Find the event tracking records for a specimen by its SpecimenId. Given a
	 * Specimen.SpecimenId = Tracking.SpecimenId, return a list of Trackings with
	 * that SpecimenId.
	 *
	 * @param specimenId
	 *            the SpecimenId for the specimen to find tracking records for.
	...
	public List<Tracking> findBySpecimen(Specimen specimen) {
		...
		try {
			...
			try {
				...
				System.out.println("find by specimen successful, result size: " + results.size());
				session.getTransaction().commit();
			} catch (HibernateException e) {
				session.getTransaction().rollback();
				System.out.println(e.getMessage());
	 ...
	 * tracking table. This list may or may not correspond to the list of users in
	 * the Users table.
	 *
		...
		try {
			...
			try {
				...
				Query<Tracking> query = session.createQuery(
						"SELECT t FROM Tracking t " + "LEFT JOIN FETCH t.specimen " + "WHERE t.id IN (?1)");
