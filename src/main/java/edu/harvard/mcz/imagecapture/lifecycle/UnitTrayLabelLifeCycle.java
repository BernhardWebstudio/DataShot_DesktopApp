...
public class UnitTrayLabelLifeCycle {
  ...
  private static final Logger log =
      LoggerFactory.getLogger(UnitTrayLabelLifeCycle.class);

  public void persist(UnitTrayLabel transientInstance)
    ...
    try {
      ...
      try {
        ...
        throw new SaveFailedException("Save to UnitTrayLabel table failed. " +
                                      e.getMessage());
      }
      try {
        session.close();
  ...
  public void delete(UnitTrayLabel persistentInstance)
      throws SaveFailedException {
    ...
    try {
      ...
      try {
        ...
        throw new SaveFailedException(
            "Delete from UnitTrayLabel table failed. " + e.getMessage());
      }
      try {
        session.close();
      } catch (SessionException e) {
  ...
  public UnitTrayLabel merge(UnitTrayLabel detachedInstance)
      throws SaveFailedException {
  ...
  public List<UnitTrayLabel> findAll() {
    ...
    try {
      ...
      try {
        results =
            (List<UnitTrayLabel>)session
                .createQuery(
                    "from UnitTrayLabel u order by u.ordinal, u.family, u.subfamily, u.tribe, u.genus, u.specificEpithet ")
                .list();
        session.getTransaction().commit();
    ...
    try {
      ...
      try {
        ...
        if (!queryResult.isEmpty()) {
          ...
          // NOTE: This will fail if maximum value of ordinal exceeds the size
          // of Integer.
