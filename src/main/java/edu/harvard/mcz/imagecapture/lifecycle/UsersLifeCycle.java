...
public class UsersLifeCycle {
	...
	private static final Logger log = LoggerFactory.getLogger(UsersLifeCycle.class);
	...
	/**
	 * A somewhat more secure version of #isUserAdministrator Why should this be
	 * more secure? Check is directly on the DB, meaning the time frame to change
	 * memory is very small. Also, the roles are guaranteed to be current
	 *
	 * @param aUserID
	...
	public static boolean isUserAdministrator(int aUserID) {
		...
		try {
			List<Users> results = session
					.createQuery("from Users as u where role = :role and u.userid = :userid", Users.class)
					.setParameter("role", Users.ROLE_ADMINISTRATOR).setParameter("userid", aUserID).list();
	...
	/**
	 * A somewhat more secure version of #isUserChiefEditor Why should this be more
	 * secure? Check is directly on the DB, meaning the time frame to change memory
	 * is very small. Also, the roles are guaranteed to be current
	 *
	 * @param aUserID
	 *            the id of the user to check
	 * @return whether the user is admin or not
	...
	public List<Users> findByNames(String username, String fullName) {
		...
		try {
			...
			try {
				...
				log.debug("find by name successful, result size: " + results.size());
			} catch (HibernateException e) {
				session.getTransaction().rollback();
				log.error(e.getMessage());
			}
	...
	public List<Users> findByCredentials(String username, String password) {
		try {
			...
			try {
				...
				log.debug("find by credentials successful, result size: " + results.size());
			} catch (HibernateException e) {
				session.getTransaction().rollback();
				log.error(e.getMessage());
			}
