...
public class FieldLoader {
  ...
  public FieldLoader() { init(); }

  /**
   * Setup initial conditions, construct list of known fields into which data
   * can be put.
   */
  protected void init() {
    ...
    for (int j = 0; j < specimenMethods.length; j++) {
      if (specimenMethods[j].getName().startsWith("set") &&
          ...
          specimenMethods[j].getParameterTypes()[0].getName().equals(
              String.class.getName())) {
        knownFields.put(
            specimenMethods[j].getName().replaceAll("^set", "").toLowerCase(),
   ...
   * fields, false if not.  Throws a null pointer exception if possibleField is
   * null.
   */
  ...
  public boolean load(String barcode, String verbatimUnclassifiedText,
                      String questions, boolean overwriteExisting)
      throws LoadException {
    return this.load(barcode, verbatimUnclassifiedText, null, questions,
                     overwriteExisting);
  }

  /**
   * Given a barcode number and a value for verbatimUnclassifiedText, update the
   * verbatim value for the matching Specimen record, leaves the Specimen record
   * in workflow state WorkFlowStatus.STAGE_VERBATIM.
   *
   ...
   * @param overwriteExisting        if true, overwrite any value of
   *     verbatimUnclassifiedText in the matching Specimen record.
   * @return if the new value was saved
   * @throws LoadException on an error
   */
  public boolean load(String barcode, String verbatimUnclassifiedText,
                      String verbatimClusterIdentifier, String questions,
                      boolean overwriteExisting) throws LoadException {
    ...
    if (matches != null && matches.size() == 1) {
      ...
      if ((!overwriteExisting &&
          ...
          (overwriteExisting && !WorkFlowStatus.allowsVerbatimUpdateOverwrite(
                                    match.getWorkFlowStatus()))) {
        throw new LoadTargetMovedOnException();
      } else {

        if (match.getVerbatimUnclassifiedText() == null ||
            match.getVerbatimUnclassifiedText().trim().length() == 0 ||
            overwriteExisting) {
        ...
        try {
          ...
          result = true;
          logHistory(match,
                     "Load:" + WorkFlowStatus.STAGE_VERBATIM +
                         ":VerbatimUnclassifiedText",
                     new Date());
        } catch (SaveFailedException e) {
          log.error(e.getMessage(), e);
          throw new LoadTargetSaveException(
              "Error saving updated target record: " + e.getMessage());
        }
      }
    } else {
  ...
  /**
   * Give a barcode number and the set of verbatim fields, attempt to set the
   * values for those verbatim fields for a record. Does not overwrite existing
   * non-empty values, does not modify record if any verbatim field contains a
   * value.
   *
   * @param barcode                  field, must match on exactly one Specimen
   *     record.
   ...
   * @throws LoadException on an error, including any existing value for any of
   *     the verbatim fields.
   */
  public boolean load(String barcode, String verbatimLocality,
                      String verbatimDate, String verbatimCollector,
                      String verbatimCollection, String verbatimNumbers,
                      String verbatimUnclassifiedText, String questions)
      throws LoadException {
    boolean result = false;
    ...
    if (matches != null && matches.size() == 1) {
      ...
      if (WorkFlowStatus.allowsVerbatimUpdate(match.getWorkFlowStatus())) {
        if (match.getVerbatimLocality() == null ||
            match.getVerbatimLocality().trim().length() == 0) {
          match.setVerbatimLocality(verbatimLocality);
        } else {
          throw new LoadTargetPopulatedException();
        }

        if (match.getDateNos() == null ||
            match.getDateNos().trim().length() == 0) {
          match.setDateNos(verbatimDate);
        } else {
          throw new LoadTargetPopulatedException();
        }

        if (match.getVerbatimCollector() == null ||
            match.getVerbatimCollector().trim().length() == 0) {
          match.setVerbatimCollector(verbatimCollector);
        } else {
          throw new LoadTargetPopulatedException();
        }

        if (match.getVerbatimCollection() == null ||
            match.getVerbatimCollection().trim().length() == 0) {
          match.setVerbatimCollection(verbatimCollection);
        } else {
          throw new LoadTargetPopulatedException();
        }

        if (match.getVerbatimNumbers() == null ||
            match.getVerbatimNumbers().trim().length() == 0) {
        ...
        try {
          ...
          logHistory(match,
                     "VerbatimFields:" + WorkFlowStatus.STAGE_VERBATIM + ":",
                     new Date());
        } catch (SaveFailedException e) {
          log.error(e.getMessage(), e);
          throw new LoadTargetSaveException(
  ...
  /**
   * Give a barcode number and an arbitrary set of fields in Specimen, attempt
   * to set the values for those fields for a record.
   *
   * @param barcode                     field, must match on exactly one
   *     Specimen record.
   * @param data                        map of field names and data values
   * @param questions                   value to append to this field in
   *     Specimen.
   * @param newWorkflowStatus           to set Specimen.workflowStatus to.
   * @param allowUpdateExistingVerbatim if true can load can overwrite the value
   *     in an existing verbatim field.
   * @return true if one or more fields were updated.
   * @throws LoadException on an error (particularly from inability to map keys
   *     in data to fields in Specimen.
   */
  public boolean loadFromMap(String barcode, Map<String, String> data,
    ...
    if (newWorkflowStatus == null ||
      ...
      throw new LoadException("Trying to load into unallowed new state." +
                              newWorkflowStatus);
    }

    // Retrieve existing record for update (thus not blanking existing fields,
    // and allowing for not updating fields with values, or appending comments).
    List<Specimen> matches = sls.findByBarcode(barcode);
    if (matches != null && matches.size() == 1) {
      Specimen match = matches.get(0);
      ...
      if ((newWorkflowStatus.equals(WorkFlowStatus.STAGE_VERBATIM) &&
        ...
        // The target Specimen record has moved on past the state where it can
        // be altered by a data load.
        throw new LoadTargetMovedOnException(
            barcode + " is in state " + match.getWorkFlowStatus() +
            " and can't be altered by this data load (to " + newWorkflowStatus +
            ").");
      } else {
        ...
        while (i.hasNext()) {
          ...
          if (toParseFields.contains(key) ||
              (actualCase != null && knownFields.contains(actualCase) &&
               !key.equals("barcode") &&
            ...
            try {
              ...
              if (key.equals("collectors")) {
                  ...
                  if (numberKV.trim().length() > 0) {
                    ...
                    while (ic.hasNext()) {
                      ...
                      if (c.getNumber().equals(number) ||
                          c.getNumberType().equals(numType)) {
                        exists = true;
                      }
                    }
                    if (!exists) {
                ...
                setMethod =
                    Specimen.class.getMethod("set" + actualCase, String.class);
                Method getMethod =
                    Specimen.class.getMethod("get" + actualCase, null);
                // Obtain the current value in the Specimen record for the field
                // matching the current key.
                String currentValue = (String)getMethod.invoke(match, null);
                // Assess whether changes to existing data are allowed for that
                // field, make changes only if they are allowed.
                  ...
                  if (currentValue == null ||
                    ...
                    if (key.equalsIgnoreCase("ISODate")) {
                      EventResult parseResult =
                          DateUtils.extractDateFromVerbatimER(datavalue);
                      if (parseResult.getResultState().equals(
                              EventResult.EventQCResultState.DATE) ||
                          parseResult.getResultState().equals(
                              EventResult.EventQCResultState.RANGE)) {
                        String correctISOFormat = parseResult.getResult();
                        // switch from correct ISO format to the internally
                        // stored incorrect format that switches - and /.
                        datavalue = correctISOFormat.replace(
                            "/", "^"); // change / to placeholder
                        datavalue =
                            datavalue.replace("-", "/"); // change - to /
                        datavalue = datavalue.replace(
                            "^", "-"); // change placeholder to -
                      }
                    }
                    // overwrite verbatim fields if update is allowed, otherwise
                    // no overwite of existing data.
                    log.debug("Set: " + actualCase + " = " + datavalue);
                    setMethod.invoke(match, datavalue);
                    foundData = true;
                  } else if (MetadataRetriever.isFieldVerbatim(Specimen.class,
                                                               key) &&
            ...
            log.error("knownFields.contains(actualCase): " +
                      knownFields.contains(actualCase));
            log.error("toParseFields.contains(key): " +
                      toParseFields.contains(key));
            log.error("isFieldExternallyUpdatable:" +
                      MetadataRetriever.isFieldExternallyUpdatable(
                          Specimen.class, key));
            throw new LoadException(
                "Column " + key +
                " is not an externally updateable field of Specimen.");
          }
        }
        ...
        if (foundData) {
          try {
            ...
            // with the user running the load job and the current date as last
            // update.
            match.setLastUpdatedBy(
  ...
  /**
   * Test the headers of a file for conformance with the expectations of
   * loadFromMap
   *
   * @param headers the headers to check against allowed fields in List<String>
   *     form.
   * @return a HeaderCheckResult object containing a result (true) and a
   *     message, the
   * result is true if there are no unmatched fields in the load, currently
   * exceptions are thrown instead of any false cases for result.
   * @throws LoadException if no barcode field is found, if no data fields are
   *     found, or if
   *                       one or more unknown (not mapped to DataShot specimen)
   * fields are found.
   * @see HeaderCheckResult.loadFromMap
   */
  public HeaderCheckResult checkHeaderList(List<String> headers)
      throws LoadException {
    ...
    if (!containsBarcode) {
      throw new LoadException(
          "Header does not contain a barcode field.  \nFields:" +
          result.getMessage());
    }
    if (!containsAField) {
      throw new LoadException(
          "Header contains no recognized data fields. \nFields: " +
          result.getMessage());
    }
    if (containsUnknownField) {
      throw new LoadException(
          "Header contains at least one unknown field. \nFields: " +
          result.getMessage());
    }
  ...
  protected void logHistory(Specimen match, String externalWorkflowProcess,
                            Date externalWorkflowDate) {
    try {
