...
public class FieldLoader {
	...
	/**
	 * Setup initial conditions, construct list of known fields into which data can
	 * be put.
	 */
	protected void init() {
	 ...
	 *         fields, false if not. Throws a null pointer exception if
	 *         possibleField is null.
	 */
	public boolean isFieldKnown(String possibleField) {
		return knownFields.containsKey(possibleField.toLowerCase());
	}

	public boolean load(String barcode, String verbatimUnclassifiedText, String questions, boolean overwriteExisting)
	 ...
	 *
	 * @param barcode
	 *            must match exactly one Specimen record.
	 * @param verbatimUnclassifiedText
	 *            value for this field in Specimen.
	 * @param questions
	 *            value to append to this field in Specimen.
	 * @param overwriteExisting
	 *            if true, overwrite any value of verbatimUnclassifiedText in the
	 *            matching Specimen record.
	 * @return if the new value was saved
	 * @throws LoadException
	 *             on an error
	 */
	public boolean load(String barcode, String verbatimUnclassifiedText, String verbatimClusterIdentifier,
			String questions, boolean overwriteExisting) throws LoadException {
		...
		if (matches != null && matches.size() == 1) {
			...
			if ((!overwriteExisting && !WorkFlowStatus.allowsVerbatimUpdate(match.getWorkFlowStatus()))
					|| (overwriteExisting
							&& !WorkFlowStatus.allowsVerbatimUpdateOverwrite(match.getWorkFlowStatus()))) {
				throw new LoadTargetMovedOnException();
			} else {

				if (match.getVerbatimUnclassifiedText() == null
						|| match.getVerbatimUnclassifiedText().trim().length() == 0 || overwriteExisting) {
				...
				try {
					...
					result = true;
					logHistory(match, "Load:" + WorkFlowStatus.STAGE_VERBATIM + ":VerbatimUnclassifiedText",
							new Date());
	 ...
	 *
	 * @param barcode
	 *            field, must match on exactly one Specimen record.
	 ...
	 * @throws LoadException
	 *             on an error, including any existing value for any of the verbatim
	 *             fields.
	 */
	public boolean load(String barcode, String verbatimLocality, String verbatimDate, String verbatimCollector,
			String verbatimCollection, String verbatimNumbers, String verbatimUnclassifiedText, String questions)
			throws LoadException {
		boolean result = false;
	...
	/**
	 * Give a barcode number and an arbitrary set of fields in Specimen, attempt to
	 * set the values for those fields for a record.
	 *
	 * @param barcode
	 *            field, must match on exactly one Specimen record.
	 * @param data
	 *            map of field names and data values
	 * @param questions
	 *            value to append to this field in Specimen.
	 * @param newWorkflowStatus
	 *            to set Specimen.workflowStatus to.
	 * @param allowUpdateExistingVerbatim
	 *            if true can load can overwrite the value in an existing verbatim
	 *            field.
	 * @return true if one or more fields were updated.
	 * @throws LoadException
	 *             on an error (particularly from inability to map keys in data to
	 *             fields in Specimen.
	 */
			...
			if ((newWorkflowStatus.equals(WorkFlowStatus.STAGE_VERBATIM)
						...
						try {
							...
							if (key.equals("collectors")) {
								...
								if (key.equals("questions")) {
									...
									foundData = true;
									hasChange = true;
								} else if (key.equals("externalworkflowprocess") || key.equals("externalworkflowdate")
									...
									if (currentValue == null || currentValue.trim().length() == 0) {
										...
										if (key.equalsIgnoreCase("ISODate")) {
											...
											if (parseResult.getResultState().equals(EventResult.EventQCResultState.DATE)
												...
												datavalue = correctISOFormat.replace("/", "^"); // change / to
																								// placeholder
				...
				if (foundData) {
					try {
						...
						// If we were provided
						String ewProcess = "ArbitraryFieldLoad:" + match.getWorkFlowStatus() + ":" + keys;
	 ...
	 *
	 * @param headers
	 *            the headers to check against allowed fields in List<String> form.
	 * @return a HeaderCheckResult object containing a result (true) and a message,
	 *         the result is true if there are no unmatched fields in the load,
	 *         currently exceptions are thrown instead of any false cases for
	 *         result.
	 * @throws LoadException
	 *             if no barcode field is found, if no data fields are found, or if
	 *             one or more unknown (not mapped to DataShot specimen) fields are
	 *             found.
		...
		while (i.hasNext()) {
			...
			if (key.equals("barcode")) {
				...
				if (key.startsWith("_")) {
					...
					String actualCase = knownFieldsLowerUpper.get(key);
					if (toParseFields.contains(key) || (actualCase != null && knownFields.contains(actualCase)
							&& MetadataRetriever.isFieldExternallyUpdatable(Specimen.class, key))) {
						result.addToMessage(keyOrig);
						containsAField = true;
					} else {
