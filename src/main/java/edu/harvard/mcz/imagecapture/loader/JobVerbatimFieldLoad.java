...
public class JobVerbatimFieldLoad implements RunnableJob, Runnable {
  private static final Logger log =
      LoggerFactory.getLogger(JobVerbatimFieldLoad.class);
  ...
  public JobVerbatimFieldLoad() { init(); }
      ...
      if (Singleton.getSingletonInstance()
        ...
        fileChooser.setCurrentDirectory(new File(
            Singleton.getSingletonInstance()
                .getProperties()
                .getProperties()
                .getProperty(ImageCaptureProperties.KEY_LASTLOADPATH)));
      }
    ...
    if (file != null) {
      ...
      if (file.exists() && file.isFile() && file.canRead()) {
        ...
        Singleton.getSingletonInstance()
            .getProperties()
            .getProperties()
            .setProperty(ImageCaptureProperties.KEY_LASTLOADPATH,
                         file.getPath());
        ...
        try {
          ...
          JOptionPane.showMessageDialog(
              Singleton.getSingletonInstance().getMainFrame(),
              "Unable to load data, file not found: " + e.getMessage(),
              "Error: File Not Found", JOptionPane.OK_OPTION);
        ...
        try {
          ...
          if (!headerList.contains("barcode")) {
            ...
            errors
                .append(
                    "Field \"barcode\" not found in csv file headers.  Unable to load data.")
                .append("\n");
            okToRun = false;
          }
          ...
          if (okToRun) {
            ...
            if (headerList.size() == 3 &&
                headerList.contains("verbatimUnclassifiedText") &&
                headerList.contains("questions") &&
                headerList.contains("barcode")) {
              ...
              int confirm = JOptionPane.showConfirmDialog(
                  ...
                  "Verbatim unclassified Field found for load",
              ...
              if (confirm == JOptionPane.OK_OPTION) {
                ...
                while (iterator.hasNext()) {
                  ...
                  try {
                    String verbatimUnclassifiedText =
                    ...
                    RunnableJobError error = new RunnableJobError(
                        file.getName(), barcode, Integer.toString(lineNumber),
              ...
              // Allowed case 1b: unclassified text only (including cluster
              // identifier)
              ...
              if (confirm == JOptionPane.OK_OPTION) {
                ...
                while (iterator.hasNext()) {
                  ...
                  try {
                    ...
                    fl.load(barcode, verbatimUnclassifiedText,
                            verbatimClusterIdentifier, questions, true);
                    counter.incrementSpecimensUpdated();
                  } catch (IllegalArgumentException e) {
                    RunnableJobError error = new RunnableJobError(
                        file.getName(), barcode, Integer.toString(lineNumber),
                ...
                errors.append("Load canceled by user.").append("\n");
                       ...
                       headerList.contains("questions") &&
                       headerList.contains("barcode") &&
                       headerList.contains("verbatimLocality") &&
                       headerList.contains("verbatimDate") &&
                       headerList.contains("verbatimNumbers") &&
                       headerList.contains("verbatimCollector") &&
                       headerList.contains("verbatimCollection")) {
              // Allowed case two, transcription into verbatim fields, must be
              // exact list of all verbatim fields, not including cluster
              // identifier or other metadata.
              log.debug(
                  "Input file matches case 2: Full list of verbatim fields.");

              int confirm = JOptionPane.showConfirmDialog(
                  Singleton.getSingletonInstance().getMainFrame(),
                  "Confirm load from file " + selectedFilename + " (" + rows +
                      " rows) with just barcode and verbatim fields.",
                  "Verbatim Fields found for load",
                  JOptionPane.OK_CANCEL_OPTION);
              if (confirm == JOptionPane.OK_OPTION) {
                ...
                while (iterator.hasNext()) {
                  ...
                  try {
                    ...
                    String verbatimCollection =
                        record.get("verbatimCollection");
                    String verbatimNumbers = record.get("verbatimNumbers");
                    String verbatimUnclasifiedText =
                        record.get("verbatimUnclassifiedText");
                    barcode = record.get("barcode");
                    String questions = record.get("questions");

                    fl.load(barcode, verbatimLocality, verbatimDate,
                            verbatimCollector, verbatimCollection,
                            verbatimNumbers, verbatimUnclasifiedText,
                            questions);
                    counter.incrementSpecimensUpdated();
                  } catch (IllegalArgumentException e) {
                    RunnableJobError error = new RunnableJobError(
              ...
              // allowed case three, transcription into arbitrary sets verbatim
              // or other fields
              ...
              try {
                ...
                if (headerCheck.isResult()) {
                  int confirm = JOptionPane.showConfirmDialog(
                      Singleton.getSingletonInstance().getMainFrame(),
                      "Confirm load from file " + selectedFilename + " (" +
                          rows + " rows) with headers: \n" +
                    ...
                    if (!header.equals("barcode") && !header.startsWith("_")) {
                      ...
                      if (!header.equals("questions") &&
                          MetadataRetriever.isFieldExternallyUpdatable(
                              Specimen.class, header) &&
                          !MetadataRetriever.isFieldVerbatim(Specimen.class,
                                                             header)) {
                  ...
                  if (data.size() > 0) {
                    try {
                      ...
                      if (containsNonVerbatim) {
                        updated = fl.loadFromMap(
                            barcode, data, WorkFlowStatus.STAGE_CLASSIFIED,
                            true);
                      } else {
                        updated = fl.loadFromMap(
                            barcode, data, WorkFlowStatus.STAGE_VERBATIM, true);
                      }
                      counter.incrementSpecimenDatabased();
                      if (updated) {
                        counter.incrementSpecimensUpdated();
                      ...
                      // Catch (should just be development) problems with the
                      // underlying query
                      StringBuilder message = new StringBuilder();
                      message.append("Query Error loading row (")
                          .append(lineNumber)
                          .append(")[")
                          .append(barcode)
                          .append("]")
                      ...
                      StringBuilder message = new StringBuilder();
                      message.append("Error loading row (")
                          .append(lineNumber)
                          .append(")[")
                          .append(barcode)
                          .append("]")
   ...
   */
  protected int readRows(File file, CSVFormat formatToTry)
