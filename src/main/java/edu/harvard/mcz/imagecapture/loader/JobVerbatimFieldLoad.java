...
public class JobVerbatimFieldLoad implements RunnableJob, Runnable {
	private static final Logger log = LoggerFactory.getLogger(JobVerbatimFieldLoad.class);
			...
			if (file.exists() && file.isFile() && file.canRead()) {
				...
				try {
					...
					if (!headerList.contains("barcode")) {
						log.error(
								"Input file " + file.getName() + " header does not contain required field 'barcode'.");
							...
							if (confirm == JOptionPane.OK_OPTION) {
								...
								while (iterator.hasNext()) {
									...
									try {
										...
										fl.load(barcode, verbatimUnclassifiedText, verbatimClusterIdentifier, questions,
												true);
										counter.incrementSpecimensUpdated();
									} catch (IllegalArgumentException e) {
										RunnableJobError error = new RunnableJobError(file.getName(), barcode,
								...
								&& headerList.contains("verbatimNumbers") && headerList.contains("verbatimCollector")
									...
									if (data.size() > 0) {
										try {
											...
											if (containsNonVerbatim) {
												updated = fl.loadFromMap(barcode, data, WorkFlowStatus.STAGE_CLASSIFIED,
														true);
											} else {
												updated = fl.loadFromMap(barcode, data, WorkFlowStatus.STAGE_VERBATIM,
														true);
											}
											counter.incrementSpecimenDatabased();
											if (updated) {
											...
											message.append("Query Error loading row (").append(lineNumber).append(")[")
													.append(barcode).append("]").append(e1.getMessage());
	...
	public boolean cancel() {
		...
		log.debug(this.getClass().getSimpleName() + " " + this + "  recieved cancel signal");
		return false;
	}
	 ...
	 *
	 * @see
	 * edu.harvard.mcz.imagecapture.interfaces.RunnableJob#registerListener(edu.
	 * harvard.mcz.imagecapture.interfaces.RunnerListener)
	 */
	@Override
	...
	private void report(String selectedFilename) {
		...
		RunnableJobReportDialog errorReportDialog = new RunnableJobReportDialog(
				...
				RunnableJobErrorTableModel.TYPE_LOAD, "Load Data from file Report");
	...
	/**
	 * Attempt to read file with a given CSV format, and if successful, return the
	 * number of rows in the file.
	 *
	 * @param file
	 *            to check for csv rows.
	 * @param formatToTry
	 *            the CSV format to try to read the file with.
	 * @return number of rows in the file.
	 * @throws IOException
	 *             on a problem reading the header.
	 * @throws FileNotFoundException
	 *             on not finding the file.
	 */
