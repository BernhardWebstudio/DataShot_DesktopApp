...
public class Specimen2JSONSerializer implements ToJSONSerializerInterface {
	...
	public JSONObject serialize2JSON(Object target, JSONObject existing) throws SkipSpecimenException {
			...
			Map<String, Object> otherNrMap = new HashMap<>() {
				{
					...
					// put("bemerkung", String.join(" ", otherNr.getNumber(),
					// otherNr.getNumberType()));
				}
			};
		...
		mainReverseNestedDetermination = nahimaManager.addDefaultValuesForCreation(mainReverseNestedDetermination,
				existing == null
						? null
						: existing.getJSONArray("_reverse_nested:bestimmung:entomologie").getJSONObject(0));

		// resolutions
		tryNonUserSkippableResolve(mainReverseNestedDetermination, "art",
				() -> nahimaManager.resolveSpecificEpithet(toSerialize.getSpecificEpithet()));
		tryNonUserSkippableResolve(mainReverseNestedDetermination, "autor",
				() -> nahimaManager.resolveAuthorship(toSerialize.getAuthorship()));
		tryNonUserSkippableResolve(mainReverseNestedDetermination, "familie",
				() -> nahimaManager.resolveFamily(toSerialize.getFamily()));
		tryNonUserSkippableResolve(mainReverseNestedDetermination, "genus",
				() -> nahimaManager.resolveGenus(toSerialize.getGenus()));
		tryNonUserSkippableResolve(mainReverseNestedDetermination, "infraspezifischerrang",
				() -> nahimaManager.resolveInfraspecificRank(toSerialize.getInfraspecificRank()));
		tryNonUserSkippableResolve(mainReverseNestedDetermination, "infraspezifischestaxon",
				() -> nahimaManager.resolveInfraspecificEpithet(toSerialize.getInfraspecificEpithet()));
		tryNonUserSkippableResolve(mainReverseNestedDetermination, "subspezifischeart",
				() -> nahimaManager.resolveSubSpecificEpithet(toSerialize.getSubspecificEpithet()));
		tryNonUserSkippableResolve(mainReverseNestedDetermination, "unterfamilie",
				() -> nahimaManager.resolveSubFamily(toSerialize.getSubfamily()));
		tryNonUserSkippableResolve(mainReverseNestedDetermination, "tribus",
				() -> nahimaManager.resolveTribe(toSerialize.getTribe()));
		tryNonUserSkippableResolve(mainReverseNestedDetermination, "ordnung",
				() -> nahimaManager.resolveOrder(toSerialize.getHigherOrder()));

		String taxonName = NullHandlingUtility.joinNonNull(" ", toSerialize.getGenus(),
				toSerialize.getSpecificEpithet(), toSerialize.getSubspecificEpithet(),
				toSerialize.getInfraspecificRank(), toSerialize.getInfraspecificEpithet()).trim().replace("  ", " ");
		if (!taxonName.isEmpty()) {
			tryNonUserSkippableResolve(mainReverseNestedDetermination, "taxonname",
					() -> nahimaManager.resolveTaxon(taxonName));
		}
		...
		if (authorship != null && authorship.contains(":")
				&& ((toSerialize.getCitedInPublication() != null && !toSerialize.getCitedInPublication().isEmpty())
						|| (toSerialize.getCitedInPublicationLink() != null
								&& !toSerialize.getCitedInPublicationLink().isEmpty())
						|| (toSerialize.getCitedInPublicationYear() != null
								&& !toSerialize.getCitedInPublicationYear().isEmpty())
						|| (toSerialize.getCitedInPublicationComment() != null
								&& !toSerialize.getCitedInPublicationComment().isEmpty()))) {
			mainReverseNestedDetermination.put("_nested:bestimmung__bestimmungsreferenzen",
					new JSONArray(new JSONObject[]{new JSONObject(new HashMap<String, String>() {
						{
							// comma separated components of the publication reference
							put("bestimmungsreferenz",
									String.join(", ",
											Arrays.stream(new String[]{toSerialize.getCitedInPublication(),
		...
		if (toSerialize.getIdentificationRemarks() != null && !toSerialize.getIdentificationRemarks().isEmpty()) {
			mainReverseNestedDetermination.put("_nested:bestimmung__kommentarezurbestimmung",
					new JSONArray(new String[]{toSerialize.getIdentificationRemarks()}));
		}
		// try to parse and set the date correctly
		tryNonUserSkippableResolve(mainReverseNestedDetermination, "bestimmungsdatum",
				() -> this.dateToNahima(toSerialize.getDateIdentified(), true));

		JSONArray identifierPersons = new JSONArray();
		tryUserSkippableResolve(identifierPersons, () -> nahimaManager.resolvePerson(toSerialize.getIdentifiedBy()),
				"person");
		...
		if (!Objects.equals(toSerialize.getTypeStatus(), "Not a Type")) {
			tryUserSkippableResolve(mainReverseNestedDetermination, "typusstatus",
					() -> nahimaManager.resolveTypeStatus(toSerialize.getTypeStatus()));
		...
		for (Determination det : toSerialize.getDeterminations()) {
			...
			reverseNestedDetermination = nahimaManager.addDefaultValuesForCreation(reverseNestedDetermination,
					NahimaManager.entomologyPool);
			// reverseNestedDetermination.put("_nested:bestimmung__kommentarezurbestimmung",
			// new JSONArray(new String[]{toSerialize.getIdentificationRemarks()}));

			if (det.getRemarks() != null && !det.getRemarks().equals("")) {
				reverseNestedDetermination.put("_nested:bestimmung__kommentarezurbestimmung",
						new JSONArray(new String[]{det.getRemarks()}));
			}

			// try to parse and set the date correctly
			// tryNonUserSkippableResolve(reverseNestedDetermination, "bestimmungsdatum", ()
			// -> this.dateToNahima(det.getDateIdentified(), false));

			// tryUserSkippableResolve(reverseNestedDetermination, "typusstatus", () ->
			// nahimaManager.resolveTypeStatus(det.getTypeStatus()));
			// finally,
		...
		JSONObject[] comments = {new JSONObject(new HashMap<>() {
			{
		...
		Map<String, Object> reverseNestedCollectionMap = new HashMap<>() {
			{
				// put("sammlertrans",
				// toSerialize.getCollectors().stream().map(Collector::getCollectorName).collect(Collectors.joining(",
				// ")));
				...
				put("breitengrad",
						(georef == null || georef.getLatDeg() == null)
								? JSONObject.NULL
								: georef.getLatDeg().toString());
				put("laengengrad",
						(georef == null || georef.getLongDeg() == null)
								? JSONObject.NULL
								: georef.getLongDeg().toString());
				put("fehlerradius",
						(georef == null || georef.getMaxErrorDistance() == null)
								? JSONObject.NULL
								: georef.getMaxErrorDistance().toString());
				put("hoehemin",
						toSerialize.getMinimum_elevation() == null
								? JSONObject.NULL
								: toSerialize.getMinimum_elevation().toString());
				put("hoehemax",
						toSerialize.getMaximum_elevation() == null
								? JSONObject.NULL
								: toSerialize.getMaximum_elevation().toString());
				put("lokalitaettrans", toSerialize.getVerbatimLocality());
				put("__idx", 0);
				put("_version",
						finalExisting == null
								? 1
								: finalExisting.getJSONArray("_reverse_nested:aufsammlung:entomologie").getJSONObject(0)
										.getInt("_version") + 1);
				put("_id",
						finalExisting == null
								? JSONObject.NULL
								: NahimaManager.resolveId(finalExisting
										.getJSONArray("_reverse_nested:aufsammlung:entomologie").getJSONObject(0)));
		...
		if (toSerialize.getDateCollectedIndicator() != null && !toSerialize.getDateCollectedIndicator().equals("")) {
			JSONArray indicators = new JSONArray();
			JSONObject indicator = new JSONObject();
			tryUserSkippableResolve(indicator, "indikatorfuersammeldatum",
					() -> nahimaManager.resolveCollectionDateIndicator(toSerialize.getDateCollectedIndicator()));
		...
		tryUserSkippableResolve(reverseNestedCollection, "einheitderhoehe",
		...
		if (georef != null && !georef.equals(new LatLong())) {
			...
			if (georef.getDatum() != null && !georef.getDatum().equals("")) {
				tryUserSkippableResolve(reverseNestedCollection, "datumsformatgeodaeischeskooordinatensystem",
						() -> nahimaManager.resolveDatumFormat(georef.getDatum()));
			}
		}

		if (toSerialize.getCollectingMethod() != null && !toSerialize.getCollectingMethod().equals("")) {
			tryNonUserSkippableResolve(reverseNestedCollection, "_nested:aufsammlung__sammelmethoden",
					() -> new JSONArray(new JSONObject[]{new JSONObject(new HashMap<>() {
						{
							put("sammlungsmethode",
									nahimaManager.resolveCollectionMethod(toSerialize.getCollectingMethod()));
						}
					})}));
		}
		...
		try {
			if (toSerialize.getDateNos() != null && !toSerialize.getDateNos().equals("")) {
				...
				if (isoDateIsPeriod) {
					...
					reverseNestedCollection.put("zeitraum",
							dateRangeToNahima(dateSplit[0].trim(), dateSplit[1].trim()));
				} else {
				...
				if (Objects.equals(toSerialize.getDateNos(), toSerialize.getDateCollected())) {
					if (isoDateIsPeriod) {
						reverseNestedCollection.put("aufsammlungszeitraum",
								dateRangeToNahima(dateSplit[0].trim(), dateSplit[1].trim(), false));
					} else {
		...
		for (SpecimenPart part : toSerialize.getSpecimenParts()) {
			...
			tryUserSkippableResolve(partJson, "praeparatteil",
					() -> nahimaManager.resolvePreparationPart(part.getPartName()));
			tryUserSkippableResolve(partJson, "montierungsmethode",
					() -> nahimaManager.resolveMountingMethod(part.getPreserveMethod()));
			...
			if (part.getSpecimenPartAttributes().size() > 0) {
				for (SpecimenPartAttribute attribute : part.getSpecimenPartAttributes()) {
					...
					// tryUserSkippableResolve(partJsonClone, "attribute", () ->
					// nahimaManager.resolvePreparationPartAttribute(NullHandlingUtility.joinNonNull("
					// ", attribute.getAttributeRemark(), attribute.getAttributeType(),
					// attribute.getAttributeValue(), attribute.getAttributeDate() != null ?
					// nahimaDateFormat.format(attribute.getAttributeDate()) : "",
					// attribute.getAttributeDeterminer()), attribute.getAttributeUnits()));
					// tryUserSkippableResolve(partJsonClone, "attribute", () ->
					// nahimaManager.resolvePreparationPartAttribute(attribute.getAttributeValue(),
					// attribute.getAttributeUnits()));
					tryUserSkippableResolve(partJsonClone, "attribute",
							() -> nahimaManager.resolvePreparationPartAttribute(NullHandlingUtility.joinNonNull(": ",
									attribute.getAttributeType(), attribute.getAttributeValue()),
									attribute.getAttributeUnits()));
		...
		if (toSerialize.getCitedInPublication() != null && !toSerialize.getCitedInPublication().isBlank()) {
			...
			tryUserSkippableResolve(publication, "publikation",
					() -> nahimaManager.resolvePublication(toSerialize.getCitedInPublication(),
							toSerialize.getCitedInPublicationLink(), toSerialize.getCitedInPublicationYear(),
							toSerialize.getCitedInPublicationComment()));
		...
		tryUserSkippableResolve(result, "lebensabschnitt",
				() -> nahimaManager.resolveLifeStage(toSerialize.getLifeStage()));

		// finally, wrap everything in the pool (might want to do somewhere else)
		JSONObject wrapper = nahimaManager.wrapForCreation(result, existing, "entomologie", "entomologie__complete",
				NahimaManager.entomologyPool);

		// and add tags if needed
		tryUserSkippableResolve(wrapper, "_tags",
				() -> nahimaManager.resolveAppropriateTags(toSerialize.getWorkFlowStatus(), parentExisting));
	...
	/**
	 * Parse all "allowed" date formats, to possibly get a good, parsed, date Wrap
	 * it in whatever format Nahima requires
	 *
	 * @param date
	 *            the date string to parse
	 * @param allowInvalid
	 *            whether or not to allow invalid dates. Returns the input string,
	 *            wrapped, if true.
	 * @return the JSON Object Nahima expects
	 * @throws ParseException
	 *             if invalid dates are not allowed.
	 */
	 ...
	 *            the resolver function
	 * @param debugHint
	 *            the useful info on what worked/did not work
	 */
	...
	/**
	 * Try to run a resolve function on a certain property, do nothing if it fails,
	 * except when a SkipSpecimenException is thrown, i.e., when the resolver
	 * allowed user input "skip specimen"
	 *
	 * @param target
	 *            the array to append data to
	 * @param key
	 ...
	 *            the resolver function
	 */
	protected void tryUserSkippableResolve(JSONObject target, String key, ResolverMethodInterface resolver)
	...
	protected void tryUserSkippableResolve(JSONArray target, ResolverMethodInterface resolver, String debugHint)
			throws SkipSpecimenException {
	 ...
	 */
	protected void tryUserSkippableResolve(JSONArray target, ResolverMethodInterface resolver, String debugHint,
			boolean reduceJSONObject) throws SkipSpecimenException {
		// try to parse and set correctly
		try {
			Object returnValue = resolver.doResolve();
			if (returnValue != null) {
	...
	protected interface ResolverMethodInterface {
		Object doResolve() throws IOException, InterruptedException, ParseException, RuntimeException,
				SkipSpecimenException, InvocationTargetException;
	}

	protected interface JSONObjectResolverMethodInterface extends ResolverMethodInterface {
		JSONObject doResolve() throws IOException, InterruptedException, ParseException, RuntimeException,
				SkipSpecimenException, InvocationTargetException;
	}

	protected interface JSONArrayResolverMethodInterface extends ResolverMethodInterface {
		JSONArray doResolve() throws IOException, InterruptedException, ParseException, RuntimeException,
				SkipSpecimenException, InvocationTargetException;
	}
}
