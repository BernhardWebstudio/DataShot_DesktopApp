...
public class JNumberField extends JTextField implements ActionListener {
    ...
    public JNumberField(double value, String format,
    ...
    public JNumberField(double value, DecimalFormat format,
                        ActionListener actionListener) {
        super();
        formatter = format;
        this.addActionListener(actionListener);
    ...
    public void addToAdmissibleValueIndex(int increment) {
        ...
        index =
                Math.max(0, Math.min(index + increment, admissibleValues.length - 1));
    ...
    private void updateData() {
        ...
        if (lowerBound != -Double.MAX_VALUE && upperBound != Double.MAX_VALUE)
            this.setColumns(1 + Math.max(formatter.format(lowerBound).length(),
                    formatter.format(upperBound).length()));
        else
    ...
    private int getAdmissibleValueIndex() {
        ...
        if (this.admissibleValues != null && admissibleValues.length > 0) {
            int index =
                    java.util.Arrays.binarySearch(admissibleValues, value.doubleValue());
            if (index < 0)
                index = -index - 1;
