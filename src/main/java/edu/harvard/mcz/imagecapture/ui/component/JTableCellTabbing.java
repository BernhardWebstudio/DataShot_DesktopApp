...
public class JTableCellTabbing {
	 ...
	 *
	 * @param theTable
	 *            - Jtable with NRows and MCols of cells
	 * @param startRow
	 *            - valid start row for tabbing [ 0 - (numRows-1) ]
	 * @param numRows
	 *            - Number of rows for tabbing
	 * @param startCol
	 *            - valid start col for tabbing [ 0 - (numCols-1) ]
	 * @param numCols
	 *            - Number of columns for tabbing
	 */
	@SuppressWarnings("serial")
	static public void setTabMapping(final JTable theTable, final int startRow, final int numRows, final int startCol,
			final int numCols) {
		if (theTable == null) {
		...
		if ((startRow > endRow) || (startCol > endCol)) {
			throw new IllegalArgumentException("Table Size incorrect: endRow=" + endRow + ", startRow = " + startRow
		...
		am.put(im.get(tabKey), new AbstractAction() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				// theTable.getCellEditor(row, col).getTableCellEditorComponent(theTable,
				// ).requestFocus();
			}
		});
