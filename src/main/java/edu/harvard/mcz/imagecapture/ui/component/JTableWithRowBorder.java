...
public class JTableWithRowBorder extends JTable {
  private static final Logger log =
      LoggerFactory.getLogger(JTableWithRowBorder.class);
   ...
   * @param parentPane
   */
  public void setParentPane(JPanel parentPane) { this.parentPane = parentPane; }
    ...
    if (deletableRows == false) {
          ...
          try {
            if (clickedRow >= 0) {
              int ok = JOptionPane.showConfirmDialog(
                  parentPane, "Delete the selected " + objectName + "?",
                  "Delete " + objectName, JOptionPane.OK_CANCEL_OPTION);
              if (ok == JOptionPane.OK_OPTION) {
                log.debug("deleting " + objectName + "s row " + clickedRow);
                ((AbstractDeleteableTableModel)tableWithRowBorder.getModel())
      ...
      this.addKeyListener(new java.awt.event.KeyAdapter() {
        public void keyTyped(java.awt.event.KeyEvent e) {
          tableWithRowBorder.emitEvent(
              new ActionEvent(e, e.getID(), e.toString()));
        }
      });
    }
  }
  ...
  public Component prepareRenderer(TableCellRenderer renderer, int row,
    ...
    if (!isRowSelected(row)) {
      c.setBackground(row % 2 == 0 ? getBackground()
                                   : DataShotColors.VERY_LIGHT_GRAY);
    }
