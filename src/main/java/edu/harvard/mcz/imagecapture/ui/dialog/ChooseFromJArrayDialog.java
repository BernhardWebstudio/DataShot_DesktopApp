...
public class ChooseFromJArrayDialog extends JDialog {
			...
			try {
				...
				if (jsonObject.has("_standard")) {
					...
					if (standardObj.keySet().size() == 1) {
						...
						log.debug(
								"More than 1 _standard available -> cannot simplify object: " + standardObj.toString());
					}
				} else {
	...
	public JTextPane getDescription() {
		if (descriptionPane == null) {
			...
			descriptionPane.setText(
					"Search for \"" + this.search + "\" resulted in multiple results. Which one is the correct one?");
		}
	...
	public JPanel getSelectionPane() {
		if (buttonGroupPanel == null) {
			...
			buttonGroupPanel
					.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(), "Possibilities"));
		}
