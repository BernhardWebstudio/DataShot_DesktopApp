...
public class NahimaDrawerExportDialog extends JDialog implements ProgressListener, GuiProviderForRunnable {
	private static final Logger log = LoggerFactory.getLogger(NahimaDrawerExportDialog.class);
	...
	private void initialize() throws CsvValidationException, IOException {
		...
		// TODO: loop images in folder, create Specimen for them based on the Excel
		// file.
	...
	private ArrayList<Specimen> translateFiles(File folder) {
		...
		if (!folder.canRead()) {
			JOptionPane.showMessageDialog(Singleton.getSingletonInstance().getMainFrame(),
					"Can't start scan.  Unable to read selected directory: " + folder.getPath(), "Can't Scan.",
	...
	private Specimen translateFile(File file) {
		...
		if (matcher.find()) {
			...
			List<Map<String, String>> relevantCsvRows = this.csvContent.stream()
					.filter(row -> Integer.parseInt(row.get("Drawer")) == drawerNumber).collect(Collectors.toList());
			Set<String> relevantTags = new LinkedHashSet<>();
			//
			String[] firstOrderCols = new String[]{"Suborder", "Family", "Subfamily", "Tribe",};
			...
			String[] relevantCols = new String[]{"Genus", "Species", "subspecies", "Infra. Name"};
			relevantCsvRows.forEach(row -> {
