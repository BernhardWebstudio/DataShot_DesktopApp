...
public class SearchDialog extends JDialog {
	...
	private static final Logger log = LoggerFactory.getLogger(SearchDialog.class); // @jve:decl-index=0:
		...
		if (jButton == null) {
			...
			jButton.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					// if (jTextFieldDrawerNumber.getText() != null &&
					// jTextFieldDrawerNumber.getText().length() > 0) {
					...
					if (jTextFieldOrder.getSelectedItem() != null
							&& jTextFieldOrder.getSelectedItem().toString().trim().length() > 0) {
					...
					if (jTextFieldVerbatimLocality.getText() != null
							&& jTextFieldVerbatimLocality.getText().length() > 0) {
						query.put("verbatimLocality", jTextFieldVerbatimLocality.getText());
					}
					/*
					 * if (jTextFieldPrimaryDivision.getText()!=null &&
					 * jTextFieldPrimaryDivision.getText().length() > 0) {
					...
					if (jTextFieldInterpretedDate.getText() != null
							&& jTextFieldInterpretedDate.getText().length() > 0) {
					...
					if (jTextFieldDateModified.getText() != null && jTextFieldDateModified.getText().length() > 0) {
						try {
							...
							Singleton.getSingletonInstance().getMainFrame()
									.setStatusMessage("Error: Search could not be composed.");
							JOptionPane.showMessageDialog(null, "Date for dateLastUpdated search could not be parsed.",
									"Error", JOptionPane.ERROR_MESSAGE);
							return;
						}
						// query.put("dateLastUpdated", jTextFieldDateModified.getText());
					}
					// Date fieldModifiedValue = (Date)
					// jTextFieldDateModified.convert().getDateWithDefaultZone();
					...
					if (jComboBoxCollector.getSelectedItem() != null) {
						if (!jComboBoxCollector.getSelectedItem().toString().equals("")) {
							query.put("collectors.collectorName", jComboBoxCollector.getSelectedItem().toString());
						}
					}
					if ((jTextFieldImageFilename.getText() != null && jTextFieldImageFilename.getText().length() > 0)
						...
						if (jTextFieldImageFilename.getText() != null
								&& jTextFieldImageFilename.getText().length() > 0) {
							// if filename has content, add it
							query.put("ICImages.filename", jTextFieldImageFilename.getText());
						}
						if (jComboBoxPath.getSelectedItem() != null) {
							if (!jComboBoxPath.getSelectedItem().toString().equals("")) {
								...
								query.put("ICImages.path", jComboBoxPath.getSelectedItem().toString());
							}
						}
					}
					if (jComboBoxCollection.getSelectedItem() != null) {
						if (!jComboBoxCollection.getSelectedItem().toString().equals("")) {
							query.put("collection", jComboBoxCollection.getSelectedItem().toString());
						}
					}
					...
					if (jComboBoxEntryBy.getSelectedItem() != null) {
						if (!jComboBoxEntryBy.getSelectedItem().toString().equals("")) {
							query.put("trackings.user", jComboBoxEntryBy.getSelectedItem().toString());
						}
					}
	...
	private JScrollPane getJPanelWithFields() {
		if (scrollPane == null) {
			...
			Component[] fields = {this.getImageFileJTextField(), this.getImagePathJComboBox(),
					...
					this.getBarcodeJTextField(), this.getOrderJTextField(), this.getFamilyJTextField(),
					this.getSubfamilyJTextField(), this.getTribeJTextField(), this.getGenusJTextField(),
					this.getSpeciesJTextField(), this.getSubspeciesJTextField(), this.getCountryJComboBox(),
					this.getPrimaryDivisionJComboBox(), this.getSpecificLocalityJComboBox(),
			 ...
			 * gbc_lblHigherGeography.gridx = 0; gbc_lblHigherGeography.gridy = 12;
		...
		if (jTextFieldOrder == null) {
			...
			(new Thread(() -> {
				...
				SwingUtilities.invokeLater(() -> {
					jTextFieldOrder.setModel(new DefaultComboBoxModel<>(orders));
	...
	private Component getLastUpdatedJTextField() {
		if (jTextFieldDateModified == null) {
			// DatePickerSettings datePickerSettings = new DatePickerSettings(new
			// Locale("en", "CH"));
	...
	/**
	 * This method initializes the button to close the form it might be slightly
	 * unnecessary as the behaviour is the same as the OS's window close button
	 *
	 * @return javax.swing.JButton
	 */
	...
	private JComboBox<String> getSpecificLocalityJComboBox() {
		if (jComboBoxSpecificLocality == null) {
			...
			(new Thread(() -> {
				...
				SwingUtilities.invokeLater(() -> {
					jComboBoxSpecificLocality.setModel(new DefaultComboBoxModel<>(specificLocalities));
	...
	private JComboBox<String> getQuestionJComboBox() {
		if (jComboBoxQuestions == null) {
			...
			(new Thread(() -> {
				...
				SwingUtilities.invokeLater(() -> {
					jComboBoxQuestions.setModel(new DefaultComboBoxModel<>(questions));
	 ...
	 */
	/*
	 * private JTextField getJTextFieldPrimaryDivision() { if
	 * (jTextFieldPrimaryDivision == null) { jTextFieldPrimaryDivision = new
	 * JTextField(); } return jTextFieldPrimaryDivision; }
	 */
