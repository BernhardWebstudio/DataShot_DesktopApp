...
/**
 * Dialog providing a list of specimen part attributes for a specimen part, with
...
public class SpecimenPartAttributeDialog extends JDialog {
	...
	private static final Logger log = LoggerFactory.getLogger(SpecimenPartAttributeDialog.class);
	...
	private void init() {
		...
		try {
			JLabel lblNewLabel = new JLabel(parentPart.getSpecimen().getBarcode() + ":" + parentPart.getPartName() + " "
		...
		table = new JTable(new SpecimenPartsAttrTableModel(parentPart.getAttributeCollection()));
