...
/**
 * WhatsThisImageDialog is a dialog to allow users to identify the nature
 * (specimen, drawer) of images for which barcode detection and OCR isn't able
 * to detect the nature of the image.
 */
public class WhatsThisImageDialog extends JDialog {
	...
	private static final Logger log = LoggerFactory.getLogger(WhatsThisImageDialog.class); // @jve:decl-index=0:
	...
	/**
	 * Default constructor, probably not the one to use, as image needs to be
	 * provided.
	 *
	 * @param owner
	 ...
	 *            the parent frame for this dialog.
	 * @param imageToShow
	 *            the image of unknown nature to display.
	 */
	...
	/**
	 * Constructor with an image File to display as a parameter. Will display a
	 * broken image icon if File can't be read as an image.
	 *
	 * @param owner
	 *            the parent frame for this dialog.
	 * @param imageFileToShow
	 *            the image file of unknown nature to display.
	 */
	public WhatsThisImageDialog(Frame owner, File imageFileToShow) {
		...
		try {
			...
			URL errorFilename = this.getClass()
					.getResource("/edu/harvard/mcz/imagecapture/resources/images/unableToLoadImage.jpg");
			try {
