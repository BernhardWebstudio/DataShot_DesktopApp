...
public class FilteringAgentJComboBox extends JComboBox<MCZbaseAuthAgentName> implements FocusListener {
	...
	private static final Logger log = LoggerFactory.getLogger(FilteringAgentJComboBox.class);
	...
	protected void filter(String enteredText, boolean changePopupState) {
		...
		if (enteredText != null && enteredText.length() >= lengthThreshold) {
			...
			for (int i = 0; i < super.getModel().getSize(); i++) {
				if (((AgentNameComboBoxModel) super.getModel()).getDataElementAt(i).toString().toLowerCase()
						.contains(enteredText.toLowerCase())) {
					filterArray.addElement(
							(MCZbaseAuthAgentName) ((AgentNameComboBoxModel) super.getModel()).getDataElementAt(i));
				}
				if (((AgentNameComboBoxModel) super.getModel()).getDataElementAt(i).toString()
						.equalsIgnoreCase(enteredText)) {
					isExactMatch = true;
					super.getModel().setSelectedItem(((AgentNameComboBoxModel) super.getModel()).getDataElementAt(i));
				}
			}
