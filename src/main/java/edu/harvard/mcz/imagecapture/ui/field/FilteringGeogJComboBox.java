 ...
 *
 */
public class FilteringGeogJComboBox
    extends JComboBox<MCZbaseGeogAuthRec> implements FocusListener {
  private static final long serialVersionUID = -7988464282872345110L;
  private static final Logger log =
      LoggerFactory.getLogger(FilteringGeogJComboBox.class);
    ...
    textfield.addKeyListener(new KeyAdapter() {
      public void keyReleased(KeyEvent keyEvent) {
        ...
        SwingUtilities.invokeLater(new Runnable() {
          public void run() { filter(textfield.getText(), true); }
        });
      }
    });
  }
  ...
  protected void filter(String enteredText, boolean changePopupState) {
    ...
    if (enteredText == null || enteredText.length() == 0 ||
      ...
      if (countryLimit == null && stateprovLimit == null) {
        ...
        if (cachedModel == null ||
            (enteredText == null || enteredText.length() < lastTextLength)) {
          cachedModel =
              new HigherGeographyComboBoxModel(uls.findByExample(pattern));
        }
        super.setModel(cachedModel);
      }
    }
    ...
    if (enteredText != null && enteredText.length() >= lengthThreshold) {
      ...
      HigherGeographyComboBoxModel filterArray =
          new HigherGeographyComboBoxModel();
      ...
      for (int i = 0; i < super.getModel().getSize(); i++) {
        if (((HigherGeographyComboBoxModel)super.getModel())
          ...
          filterArray.addElement(
              (MCZbaseGeogAuthRec)((HigherGeographyComboBoxModel)
                                       super.getModel())
                  .getDataElementAt(i));
        }
        if (((HigherGeographyComboBoxModel)super.getModel())
      ...
      if (filterArray.getSize() > 0) {
        HigherGeographyComboBoxModel model =
            (HigherGeographyComboBoxModel)this.getModel();
  ...
  public void setStateProvLimit(Object selectedStateProv) {
    if (selectedStateProv != null &&
        selectedStateProv.toString().length() > 0) {
