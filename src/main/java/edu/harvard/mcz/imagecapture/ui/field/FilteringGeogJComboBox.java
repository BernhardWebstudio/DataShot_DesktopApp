...
public class FilteringGeogJComboBox extends JComboBox<MCZbaseGeogAuthRec> implements FocusListener {
	...
	private static final Logger log = LoggerFactory.getLogger(FilteringGeogJComboBox.class);
	...
	protected void filter(String enteredText, boolean changePopupState) {
		...
		int lengthThreshold = Integer.valueOf(Singleton.getSingletonInstance().getProperties().getProperties()
		...
		if (enteredText != null && enteredText.length() >= lengthThreshold) {
			...
			for (int i = 0; i < super.getModel().getSize(); i++) {
				if (((HigherGeographyComboBoxModel) super.getModel()).getDataElementAt(i).toString().toLowerCase()
						.contains(enteredText.toLowerCase())) {
					filterArray.addElement(
							(MCZbaseGeogAuthRec) ((HigherGeographyComboBoxModel) super.getModel()).getDataElementAt(i));
				}
				if (((HigherGeographyComboBoxModel) super.getModel()).getDataElementAt(i).toString()
						.equalsIgnoreCase(enteredText)) {
					isExactMatch = true;
					super.getModel()
							.setSelectedItem(((HigherGeographyComboBoxModel) super.getModel()).getDataElementAt(i));
				}
			}
			if (filterArray.getSize() > 0) {
				HigherGeographyComboBoxModel model = (HigherGeographyComboBoxModel) this.getModel();
	 ...
	 *
	 * @param selectedItem
	 *            the family to limit the picklist to.
	 */
	 ...
	 *
	 * @param selectedStateProv
	 *            the genus to limit the picklist to.
	 */
