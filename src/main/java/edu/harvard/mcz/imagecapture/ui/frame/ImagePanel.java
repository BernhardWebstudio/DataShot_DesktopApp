...
/**
 * ImagePanel displays a zoomable image in a JPanel. Support class for
 * ImageZoomPanel, which includes both the controls and the image in an
 * ImagePanel.
 *
 * @see ImageZoomPanel
 */
public class ImagePanel extends JPanel {
	...
	public Dimension getPreferredSize() {
		...
		if (image != null) {
			result = new Dimension((int) (image.getWidth(this) * zoom), (int) (image.getHeight(this) * zoom));
		}
		return result;
	}
	...
	/**
	 * Scale this image to fit entirely within a rectangle defined by a width and
	 * height.
	 *
	 * @param x
	 *            width this image is to fit into
	 * @param y
	 *            height this image is to fit into.
	 */
	...
	public void center() {
		...
		if (jScrollPane != null) {
			if (this.getSize().height < jScrollPane.getHeight() && this.getSize().width < jScrollPane.getWidth()) {
	...
	/**
	 * Convenience method to get the size of the current image. Returns 0,0 if image
	 * is null or if its size isn't known.
	 *
