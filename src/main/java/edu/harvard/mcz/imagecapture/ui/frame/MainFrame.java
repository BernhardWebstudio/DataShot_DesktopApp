...
public class MainFrame extends JFrame implements RunnerListener {
	...
	public static final Color BG_COLOR_ERROR = new Color(255, 73, 43); // highlight fields with data errors //
																		// @jve:decl-index=0:
	private static final long serialVersionUID = 536567118673854270L;
	private static final Logger log = LoggerFactory.getLogger(MainFrame.class); // @jve:decl-index=0:
	...
	/**
	 * Enable/disable menu items based on current user and their login state.
	 */
	...
	private JMenuItem getJMenuItemPreprocess() {
		if (jMenuItemPreprocess == null) {
			...
			jMenuItemPreprocess.setEnabled(true);
			try {
				jMenuItemPreprocess.setIcon(new ImageIcon(Objects.requireNonNull(this.getClass()
						.getResource("/edu/harvard/mcz/imagecapture/resources/images/barcode_icon_16px.jpg"))));
			} catch (Exception e) {
			...
			jMenuItemPreprocess.addActionListener(e -> {
				int result = JOptionPane.showConfirmDialog(Singleton.getSingletonInstance().getMainFrame(),
						"Are you sure, this will check all image files and may take some time.", "Preprocess All?",
	...
	private JMenuItem getJMenuItemDelete() {
		if (jMenuItemDelete == null) {
			...
			jMenuItemDelete.setEnabled(true);
			try {
				jMenuItemDelete.setIcon(new ImageIcon(Objects.requireNonNull(this.getClass()
						.getResource("/edu/harvard/mcz/imagecapture/resources/images/red-warning-icon.png"))));
			} catch (Exception e) {
				log.error("Can't open icon file for jMenuItemScanOneBarcode.");
				log.error(e.getLocalizedMessage());
			}
			jMenuItemDelete.addActionListener(e -> {
				...
				final JComponent[] inputs = new JComponent[]{new JLabel(
						...
						barcode};
				int result = JOptionPane.showConfirmDialog(null, inputs, "Delete a specimen record",
				...
				if (result == JOptionPane.YES_OPTION) {
					...
					if (delete_result == 0) {
						JOptionPane.showConfirmDialog(null, "Error: specimen record not found.",
								"Delete specimen record", JOptionPane.DEFAULT_OPTION);
					} else if (delete_result == 1) {
						JOptionPane.showConfirmDialog(null, "Specimen has been deleted successfully.",
								"Delete specimen record", JOptionPane.DEFAULT_OPTION);
					} else {
						JOptionPane.showConfirmDialog(null, "Error: delete failed.", "Delete specimen record",
	...
	private JMenuItem getJMenuItemLoadData() {
		if (jMenuItemLoadData == null) {
			...
			try {
				jMenuItemLoadData.setIcon(new ImageIcon(Objects.requireNonNull(this.getClass()
						.getResource("/edu/harvard/mcz/imagecapture/resources/images/cycle_icon_16px.jpg"))));
			} catch (Exception e) {
	...
	private JMenuItem getJMenuItemScanOneBarcode() {
		if (jMenuItemScanOneBarcode == null) {
			...
			try {
				jMenuItemScanOneBarcode.setIcon(new ImageIcon(Objects.requireNonNull(this.getClass()
						.getResource("/edu/harvard/mcz/imagecapture/resources/images/barcode_icon_16px.jpg"))));
			} catch (Exception e) {
				System.out.println("Can't open icon file for jMenuItemScanOneBarcode.");
				System.out.println(e.getLocalizedMessage());
			}
			jMenuItemScanOneBarcode.addActionListener(e -> {
	...
	private JMenuItem getJMenuItemScanOneBarcodeSave() {
		if (jMenuItemScanOneBarcodeSave == null) {
			...
			try {
				jMenuItemScanOneBarcodeSave.setIcon(new ImageIcon(Objects.requireNonNull(this.getClass()
						.getResource("/edu/harvard/mcz/imagecapture/resources/images/barcode_icon_16px.jpg"))));
			} catch (Exception e) {
				log.error("Can't open icon file for jMenuItemScanOneBarcode.");
				log.error(e.getLocalizedMessage());
			}
			jMenuItemScanOneBarcodeSave.addActionListener(e -> {
	...
	private JMenuItem getJMenuItemUsers() {
		if (jMenuItemUsers == null) {
			...
			jMenuItemUsers.setEnabled(false);
			try {
				jMenuItemUsers.setIcon(new ImageIcon(Objects.requireNonNull(this.getClass()
						.getResource("/edu/harvard/mcz/imagecapture/resources/images/people_icon_16px.png"))));
			} catch (Exception e) {
	...
	private JMenuItem getJMenuItemBrowseSpecimens() {
		if (jMenuItemBrowseSpecimens == null) {
			...
			try {
				jMenuItemBrowseSpecimens.setIcon(new ImageIcon(Objects.requireNonNull(this.getClass()
						.getResource("/edu/harvard/mcz/imagecapture/resources/images/butterfly_icon_16px.png"))));
			} catch (Exception e) {
				log.error("Can't open icon file for jMenuItemBrowseImages.");
				log.error("Error", e);
			}
			jMenuItemBrowseSpecimens.addActionListener(e -> {
	...
	private JMenuItem getJMenuItemEditTemplates() {
		if (jMenuItemEditTemplates == null) {
			...
			jMenuItemEditTemplates.addActionListener(e -> {
	...
	private JMenuItem getJMenuItemBrowseImages() {
		if (jMenuItemBrowseImages == null) {
			...
			try {
				jMenuItemBrowseImages.setIcon(new ImageIcon(Objects.requireNonNull(this.getClass()
						.getResource("/edu/harvard/mcz/imagecapture/resources/images/image_icon_16px.png"))));
			} catch (Exception e) {
				log.error("Can't open icon file for jMenuItemBrowseImages.");
				log.error("Error", e);
			}
			jMenuItemBrowseImages.addActionListener(e -> {
	...
	public void updateForPropertiesChange() {
		this.updateTitle();
		jMenuItemPreprocessOneDir
				.setText("Preprocess A Directory (as " + LocationInCollection.getDefaultLocation() + ")");
	}
	...
	private JMenuItem getJMenuItemPreprocessOne() {
		if (jMenuItemPreprocessOneDir == null) {
			...
			jMenuItemPreprocessOneDir.setEnabled(true);
			try {
				jMenuItemPreprocessOneDir.setIcon(new ImageIcon(Objects.requireNonNull(this.getClass()
						.getResource("/edu/harvard/mcz/imagecapture/resources/images/barcode_icon_16px.jpg"))));
			} catch (Exception e) {
				log.error("Can't open icon file for getJMenuItemPreprocessOne.");
				log.error(e.getLocalizedMessage());
			}
			jMenuItemPreprocessOneDir.addActionListener(e -> {
				JobAllImageFilesScan scan = new JobAllImageFilesScan(JobAllImageFilesScan.SCAN_SELECT,
						new File(Singleton.getSingletonInstance().getProperties().getProperties()
	...
	private JMenuItem getJMenuItemVerbatimTranscription() {
		if (jMenuItemVerbatimTranscription == null) {
			...
			jMenuItemVerbatimTranscription.addActionListener(actionEvent -> {
	...
	private JMenuItem getJMenuItemValidateImageNoDB() {
		if (jMenuItemCheckForABarcode == null) {
			...
			try {
				jMenuItemCheckForABarcode.setIcon(new ImageIcon(Objects.requireNonNull(this.getClass()
						.getResource("/edu/harvard/mcz/imagecapture/resources/images/barcode_icon_16px.jpg"))));
			} catch (Exception e) {
				log.error("Can't open icon file for jMenuItemCheckForABarcode.");
				log.error("Error", e);
			}
			jMenuItemCheckForABarcode.addActionListener(actionEvent -> {
				JobSingleBarcodeScan scan = new JobSingleBarcodeScan(false);
				scan.registerListener(thisMainFrame);
				(new Thread(scan)).start();
			});
	...
	private JMenuItem getJMenuItemReconcileFiles() {
		if (jMenuItemReconcileFiles == null) {
			...
			jMenuItemReconcileFiles.addActionListener(actionEvent -> {
			...
			jMenuItemQCBarcodes.addActionListener(e -> {
				...
				if (missingBarcodes.length > 0) {
					...
					for (String missingBarcode : missingBarcodes) {
						...
						String previous = builder.makeFromNumber(matcher.extractNumber(missingBarcode) - 1);
						String previousFile = "";
						String previousPath = "";
						SpecimenLifeCycle sls = new SpecimenLifeCycle();
						...
						RunnableJobError err = new RunnableJobError(previousFile, missingBarcode, previousPath, "",
								"Barcode not found", null, null, null,
								RunnableJobError.TYPE_BARCODE_MISSING_FROM_SEQUENCE, previousFile, previousPath);
	...
	private JMenuItem getJMenuItemFindMissingImages() {
		if (jMenuItemFindMissingImages == null) {
			...
			/*
			 * try { jMenuItemFindMissingImages.setIcon(new
			 ...
			 * log.error(e.getLocalizedMessage()); }
			 */
			jMenuItemFindMissingImages.addActionListener(actionEvent -> {
				...
				final JComponent[] inputs = new JComponent[]{
						...
						pathCombo};
				int result = JOptionPane.showConfirmDialog(null, inputs, "Find missing images",
						JOptionPane.OK_CANCEL_OPTION);
				//
				if (result == JOptionPane.YES_OPTION) {
					...
					/*
					 * for(Integer cint : missingvals){ if(cint < 10000){
					 * sb.append("ETHZ_ENT01_2017_03_15_00"); }else if(cint < 100000){
					 * sb.append("ETHZ_ENT01_2017_03_15_0"); }else if(cint < 1000000){
					 * sb.append("ETHZ_ENT01_2017_03_15_"); } sb.append(cint); sb.append(".JPG");
					 * sb.append("\n"); }
					 */
					...
					/*
					 * for(ICImage im : results){ sb.append(im.getFilename()); sb.append("\n"); }
					 */
					JOptionPane.showConfirmDialog(null,
							"Found " + results.size() + " images in the database for the date " + dateEntered
	...
	private JMenuItem getJMenuItemChangePassword() {
		if (jMenuItemChangePassword == null) {
			...
			try {
				jMenuItemChangePassword.setIcon(new ImageIcon(Objects.requireNonNull(this.getClass()
						.getResource("/edu/harvard/mcz/imagecapture/resources/images/key_icon_16px.png"))));
			} catch (Exception e) {
				log.error("Can't open icon file for jMenuItemChangePassword.");
				log.error("Error", e);
			}
			jMenuItemChangePassword.addActionListener(e -> {
				if (Singleton.getSingletonInstance().getUser() != null) {
	...
	private JMenuItem getJMenuItemCreateLabels() {
		if (jMenuItemCreateLabels == null) {
			...
			jMenuItemCreateLabels.addActionListener(e -> {
	...
	private JMenuItem getJMenuItem() {
		if (jMenuItemStats == null) {
			...
			jMenuItemStats.addActionListener(e -> {
				SpecimenLifeCycle sls = new SpecimenLifeCycle();
				JOptionPane.showMessageDialog(Singleton.getSingletonInstance().getMainFrame(),
						sls.findSpecimenCount(",\n- "), "Record counts", JOptionPane.INFORMATION_MESSAGE);
			});
		}
	...
	private JMenuItem getJMenuItemRepeatOCR() {
		if (jMenuItemRepeatOCR == null) {
			...
			try {
				jMenuItemRepeatOCR.setIcon(new ImageIcon(Objects.requireNonNull(this.getClass()
						.getResource("/edu/harvard/mcz/imagecapture/resources/images/reload_icon_16px.png"))));
			} catch (Exception e) {
	...
	private JMenuItem getJMenuItemListRunningJobs() {
		if (jMenuItemListRunningJobs == null) {
			...
			try {
				jMenuItemListRunningJobs.setIcon(new ImageIcon(Objects.requireNonNull(this.getClass()
						.getResource("/edu/harvard/mcz/imagecapture/resources/images/tools_icon_16px.png"))));
			} catch (Exception e) {
				log.error("Can't open icon file for jMenuItemListRunningJobs.");
				log.error("Error", e);
			}
			jMenuItemListRunningJobs.addActionListener(e -> {
	...
	private JMenuItem getJMenuItemRedoOCROne() {
		if (jMenuItemRedoOCROne == null) {
			...
			try {
				jMenuItemRedoOCROne.setIcon(new ImageIcon(Objects.requireNonNull(this.getClass()
						.getResource("/edu/harvard/mcz/imagecapture/resources/images/reload_icon_16px.png"))));
			} catch (Exception e) {
				log.error("Can't open icon file for jMenuItemRedoOCROne.");
				log.error("Error", e);
			}
			jMenuItemRedoOCROne.addActionListener(e -> {
	...
	private JMenuItem getJMenuItemCleanupDirectory() {
		if (jMenuItemCleanupDirectory == null) {
			...
			try {
				jMenuItemCleanupDirectory.setIcon(new ImageIcon(Objects.requireNonNull(this.getClass()
						.getResource("/edu/harvard/mcz/imagecapture/resources/images/bb_trsh_icon_16px.png"))));
			} catch (Exception e) {
				System.out.println("Can't open icon file for jMenuItemCleanupDirectory.");
				System.out.println(e.getLocalizedMessage());
			}
			jMenuItemCleanupDirectory.addActionListener(e -> {
				JobCleanDirectory r = new JobCleanDirectory(JobCleanDirectory.SCAN_SELECT,
						new File(Singleton.getSingletonInstance().getProperties().getProperties()
								.getProperty(ImageCaptureProperties.KEY_IMAGEBASE)));
				(new Thread(r)).start();
			});
	...
	private JMenuItem getJMenuItemRecheckTemplates() {
		if (jMenuItemRecheckTemplates == null) {
			...
			try {
				jMenuItemRecheckTemplates.setIcon(new ImageIcon(Objects.requireNonNull(this.getClass()
						.getResource("/edu/harvard/mcz/imagecapture/resources/images/reload_icon_16px.png"))));
			} catch (Exception e) {
				log.error("Can't open icon file for jMenuItemRecheckTemplates.");
				log.error("Error", e);
			}
			jMenuItemRecheckTemplates.addActionListener(e -> {
				JobRecheckForTemplates r = new JobRecheckForTemplates(JobRecheckForTemplates.SCAN_SELECT,
						new File(Singleton.getSingletonInstance().getProperties().getProperties()
								.getProperty(ImageCaptureProperties.KEY_IMAGEBASE)));
				(new Thread(r)).start();
			});
	...
	private JMenuItem getJMenuItemRecheckAllTemplates() {
		if (jMenuItemRecheckAllTemplates == null) {
			...
			try {
				jMenuItemRecheckAllTemplates.setIcon(new ImageIcon(Objects.requireNonNull(this.getClass()
						.getResource("/edu/harvard/mcz/imagecapture/resources/images/reload_icon_16px.png"))));
			} catch (Exception e) {
				log.error("Can't open icon file for jMenuItemRecheckAllTemplates.");
				log.error("Error", e);
			}
			jMenuItemRecheckAllTemplates.addActionListener(e -> {
