...
public class AgentNameComboBoxModel implements ComboBoxModel<MCZbaseAuthAgentName> {
	...
	private static final Logger log = LoggerFactory.getLogger(AgentNameComboBoxModel.class);
		...
		if (anItem != null) {
			...
			while (i.hasNext() && !done) {
				...
				try {
					...
					// object's agent name value.
					if (higher.getAgent_name() != null && higher.getAgent_name().equals(anItem)) {
						log.debug("Selected agent is: " + higher.getAgent_name());
						selectedItem = higher;
						done = true;
						notifyListeners();
	...
	/**
	 * Obtain the agent name string for the currently selected agent name authority
	 * record object.
	 *
	 * @return a string representation of the agent name from the currently selected
	 *         item, or null if there is no currently selected item.
	 */
