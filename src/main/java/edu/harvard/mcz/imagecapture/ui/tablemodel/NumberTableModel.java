...
public class NumberTableModel extends AbstractDeleteableTableModel {
    ...
    public Object getValueAt(int rowIndex, int columnIndex) {
        if (rowIndex >= numbers.size()) {
            log.warn(
                    "Tried to get value in NumberTable at rowIndex higher than row count; rowIndex: " +
    ...
    public void setValueAt(Object value, int rowIndex, int columnIndex) {
        if (rowIndex >= numbers.size()) {
            log.warn(
                    "Tried to set value in number table at rowIndex higher than row count; rowIndex: " +
                            rowIndex + "; columnIndex: " + columnIndex + "; value: " + value);
            return;
        }
        switch (columnIndex) {
            case 0:
