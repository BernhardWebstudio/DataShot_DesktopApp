...
public class SpecimenPartsTableModel extends AbstractDeleteableTableModel {
	...
	private static final Logger log = LoggerFactory.getLogger(SpecimenPartsTableModel.class);
	...
	public Object getValueAt(int rowIndex, int columnIndex) {
		if (rowIndex >= specimenParts.size()) {
			log.warn("Cannot get value at row, col: " + rowIndex + ", " + columnIndex + " due to size "
					+ specimenParts.size() + " in SpecimenPartsTable");
			return null;
		}
		Object result = null;
		switch (columnIndex) {
	...
	public void setValueAt(Object aValue, int rowIndex, int columnIndex) {
		if (rowIndex >= specimenParts.size()) {
			log.warn("Cannot set value " + aValue.toString() + " due to size " + specimenParts.size()
					+ " in SpecimenPartsTable at row, col: " + rowIndex + ", " + columnIndex);
			return;
		}
		switch (columnIndex) {
			case 0 :
			...
			case 2 :
				if (aValue instanceof Number) {
					...
					((SpecimenPart) specimenParts.toArray()[rowIndex]).setLotCount(0);
				}
				break;
			case 3 :
				break;
