...
public abstract class AbstractRestClient {
	...
	public AbstractRestClient() {
		Methanol.Builder builder = Methanol.newBuilder().userAgent("DataShot " + ImageCaptureApp.getAppVersion())
				.requestTimeout(Duration.ofMinutes(15)) // Default request timeout, used by uploads without
														// request-level timeout
				.headersTimeout(Duration.ofMinutes(10)) // Upload processing in Fylr can exceed 2 minutes before headers
														// are returned
				.readTimeout(Duration.ofMinutes(2)); // Timeout for single reads
		httpClient = builder.executor(Executors.newFixedThreadPool(8)).connectTimeout(Duration.ofSeconds(120)).build();
	}
	...
	protected String getRequest(String url, Map<String, String> headers) throws IOException {
		...
		con.setRequestProperty("User-Agent", "Datashot/Imagecapture App " + ImageCaptureApp.getAppVersion()
		...
		if (con.getResponseCode() != 200) {
			log.error("Error on request. Response code: " + con.getResponseCode() + ", message: "
					+ con.getResponseMessage() + ", response: " + response);
			return null;
		}
	 ...
	 *             from HTTPclient
	 */
	protected String postRequest(String url, String data, Map<String, String> headers)
			throws IOException, InterruptedException {
		log.debug("Posting request to " + url + " with data " + data);
		HttpRequest.Builder builder = HttpRequest.newBuilder().POST(HttpRequest.BodyPublishers.ofString(data))
				.uri(URI.create(url)).setHeader("User-Agent", "DataShot " + ImageCaptureApp.getAppVersion()) // add
																												// request
																												// header
				.setHeader("Content-Type", "application/x-www-form-urlencoded").timeout(Duration.ofMinutes(3));
	...
	/**
	 * @param formFields
	 *            the fields to be sent as such
	 * @see #postRequest(String, String, Map)
	 */
	protected String postRequest(String url, Map<String, String> formFields, Map<String, String> headers)
			throws IOException, InterruptedException {
	 ...
	 *             from HttpClient
	 */
	protected String putRequest(String url, String data, Map<String, String> headers)
			throws IOException, InterruptedException {
		log.debug("Putting request to " + url + " with data " + data);
		HttpRequest.Builder builder = HttpRequest.newBuilder().PUT(HttpRequest.BodyPublishers.ofString(data))
				.uri(URI.create(url)).setHeader("User-Agent", "DataShot " + ImageCaptureApp.getAppVersion()) // add
																												// request
																												// header
				.setHeader("Content-Type", "application/x-www-form-urlencoded").timeout(Duration.ofMinutes(3));
