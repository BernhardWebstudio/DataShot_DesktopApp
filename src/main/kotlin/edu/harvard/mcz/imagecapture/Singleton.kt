...
private constructor() {
    ...
    var properties: ImageCaptureProperties? = null
        get() {
            if (field == null) { // load a default properties if we haven't been given one yet.
                log!!.debug("Recreating properties on request...")
                field = ImageCaptureProperties()
            }
            return field
        }
    /**
    ...
    fun setCurrentUser(user: Users?) {
        this.user = user
        mainFrame.setStatusMessage("Connected as " + this.user.getFullname())
    }
    ...
    companion object {
        private val log = LogFactory.getLog(Singleton::class.java)
        /**
