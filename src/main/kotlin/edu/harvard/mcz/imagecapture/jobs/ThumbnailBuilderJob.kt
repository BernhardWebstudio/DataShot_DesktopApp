...
internal class ThumbnailBuilderJob : Runnable, RunnableJob {
    ...
    constructor(aStartPoint: File?, counter: AtomicInteger?) {
        ...
        thumbHeight = Singleton.Companion.getSingletonInstance().getProperties().getProperties().getProperty(ImageCaptureProperties.Companion.KEY_THUMBNAIL_HEIGHT)
        thumbWidth = Singleton.Companion.getSingletonInstance().getProperties().getProperties().getProperty(ImageCaptureProperties.Companion.KEY_THUMBNAIL_WIDTH)
        thumbInit()
    }
    ...
    override fun run() {
        ...
        if (startPoint!!.isDirectory && startPoint!!.name != "thumbs") {
            ...
            if (!thumbsDir.exists()) {
                thumbsDir.mkdir()
                Singleton.Companion.getSingletonInstance().getMainFrame().setStatusMessage("Creating " + thumbsDir.path)
            }
...
// rather than passing *.JPG
            ...
            if (filesToThumbCount > 0) {
                ...
                val mogrify: String = Singleton.Companion.getSingletonInstance().getProperties().getProperties().getProperty(ImageCaptureProperties.Companion.KEY_MOGRIFY_EXECUTABLE)
                if (mogrify == null || mogrify.trim { it <= ' ' }.length > 0) {
                    ...
                    try {
                        ...
                        e.printStackTrace()
                        Singleton.Companion.getSingletonInstance().getMainFrame().setStatusMessage("Error creating thumbnails " + e.message)
                        makeWithJava = true
                        ...
                        try {
                            ...
                            while (it.hasNext() && status != RunStatus.STATUS_CANCEL_REQUESTED) {
                                ...
                                if (!file!!.isDirectory && file.exists() && file.canRead()) { // file must exist and be readable to make thumbnail
                                    if (file.name.matches(Singleton.Companion.getSingletonInstance().getProperties().getProperties().getProperty(ImageCaptureProperties.Companion.KEY_IMAGEREGEX))) { // only try to make thumbnails of files that match the image file pattern.
