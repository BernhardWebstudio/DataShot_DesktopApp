...
org.apache.commons.csv.CSVFormatimport org.apache.commons.csv.CSVPrinterimport org.apache.commons.csv.QuoteModeimport org.apache.commons.logging.Logimport org.apache.commons.logging.LogFactoryimport java.awt.Dimension
import java.awt.Frameimport

java.awt.Insetsimport java.awt.Toolkitimport java.awt.event.ActionEventimport java.lang.Exceptionimport java.util.*
 ...
 */
class RunnableJobReportDialog : JDialog {
    private var title: String? = "Job Results"
    ...
    private var jTextArea: JTextArea? = null
    private var thisDialog: JDialog? = null
    private var model: RunnableJobErrorTableModel? = null
    ...
    constructor(owner: Frame?, resultsMessage: String?, errors: MutableList<RunnableJobError?>?, title: String?) : super(owner) {
        this.title = title
        thisDialog = this
    ...
    constructor(owner: Frame?, resultsMessage: String?, errors: MutableList<RunnableJobError?>?, listType: Int, title: String?) : super(owner) {
        this.title = title
        thisDialog = this
    ...
    fun setMessage(resultsMessage: String?) {
    ...
    private fun initialize() {
        this.setPreferredSize(Dimension(1000, 400))
        this.setTitle(title)
    ...
    private fun getJPanel1(): JPanel? {
        if (jPanel1 == null) {
            ...
            val gbc_jButtonSave = GridBagConstraints()
            gbc_jButtonSave.insets = Insets(0, 0, 0, 5)
            gbc_jButtonSave.gridx = 0
            gbc_jButtonSave.gridy = 0
            jPanel1.add(jButtonSave, gbc_jButtonSave)
            val gbc_jButton = GridBagConstraints()
            gbc_jButton.gridx = 1
            gbc_jButton.gridy = 0
     ...
     */
    private fun getJTextArea(): JTextArea? {
        if (jTextArea == null) {
            jTextArea = JTextArea()
     ...
     */
    protected fun serializeTableModel() {
        ...
        try {
            ...
            val filename = "jobreport_$time.csv"
            ...
            writer.flush()
            writer.close()
            JOptionPane.showMessageDialog(Singleton.Companion.getSingletonInstance().getMainFrame(), "Saved report to file: $filename", "Report to CSV file", JOptionPane.INFORMATION_MESSAGE)
        } catch (e: IOException) {
            log.error(e)
        } finally {
            try {
    ...
    private val jButtonSave: JButton?
        private get() {
            if (btnSave == null) {
                btnSave = JButton("Save Report")
                btnSave.addActionListener(object : ActionListener {
                    override fun actionPerformed(e: ActionEvent?) {
                        serializeTableModel()
                    }
                })
    ...
    companion object {
        private const val serialVersionUID = -8151583200152856827L
        private val log: Log? = LogFactory.getLog(RunnableJobReportDialog::class.java)
    }
