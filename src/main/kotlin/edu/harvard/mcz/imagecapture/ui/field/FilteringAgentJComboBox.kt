 ...
 *
 */
class FilteringAgentJComboBox : JComboBox<MCZbaseAuthAgentName?>, FocusListener {
    private var cachedModel: AgentNameComboBoxModel? = null
     ...
     */
    constructor() {
        super.setModel(AgentNameComboBoxModel())
        init()
    }
     ...
     * @param valueList
     */
    constructor(model: AgentNameComboBoxModel?) : super(model) {
        init()
    }

    fun setHGModel(model: AgentNameComboBoxModel?) {
    ...
    fun setAgentNameModel(model: AgentNameComboBoxModel?) {
        super.setModel(model)
    }
    ...
    protected fun filter(enteredText: String?, changePopupState: Boolean) {
        ...
        if (enteredText == null || enteredText.length == 0 || enteredText.length < lastTextLength) { // If entry is blank, show full list.
            if (cachedModel == null ||
...
// textfield.setText(super.Model.SelectedItem.toString());
    ...
    companion object {
        ...
        private val log: Log = LogFactory.getLog(FilteringAgentJComboBox::class.java)
    }
}
