 ...
 * Author: mole
 */
package edu.harvard.mcz.imagecapture.ui.fieldimportimport

org.apache.commons.logging.Logimport org.apache.commons.logging.LogFactory
import java.awt.event.KeyAdapterimport
 ...
 *
 */
class FilteringGeogJComboBox : JComboBox<MCZbaseGeogAuthRec?>, FocusListener {
    private var countryLimit: String? = null
    private var stateprovLimit: String? = null
    private var cachedModel: HigherGeographyComboBoxModel? = null
     ...
     */
    constructor() {
        super.setModel(HigherGeographyComboBoxModel())
        init()
    }
     ...
     * @param valueList
     */
    constructor(model: HigherGeographyComboBoxModel?) : super(model) {
        init()
    }

    fun setHGModel(model: HigherGeographyComboBoxModel?) {
    ...
    fun setHigherGeographyModel(model: HigherGeographyComboBoxModel?) {
        super.setModel(model)
    }

    fun resetFilter(changePopupState: Boolean) {
        filter(null, changePopupState)
    }

    protected fun filter(enteredText: String?, changePopupState: Boolean) {
        ...
        if (enteredText == null || enteredText.length == 0 || lastTextLength < enteredText.length) { // If entry is blank, show full list.
            val uls = MCZbaseGeogAuthRecLifeCycle()
            if (countryLimit == null && stateprovLimit == null) {
                if (cachedModel == null || enteredText == null || enteredText.length < lastTextLength) {
                    cachedModel = HigherGeographyComboBoxModel(uls.findAll())
                }
                super.setModel(cachedModel)
            } else {
                val pattern = MCZbaseGeogAuthRec()
                if (countryLimit != null && countryLimit!!.length > 0) {
                ...
                if (cachedModel == null || enteredText == null || enteredText.length < lastTextLength) {
                    cachedModel = HigherGeographyComboBoxModel(uls.findByExample(pattern))
                }
                super.setModel(cachedModel)
            }
        }
        ...
        val lengthThreshold: Int = Integer.valueOf(Singleton.Companion.getSingletonInstance().getProperties().getProperties().getProperty(ImageCaptureProperties.Companion.KEY_FILTER_LENGTH_THRESHOLD))
        ...
        lastTextLength = enteredText?.length ?: 0
    }
...
//textfield.setText(super.getModel().getSelectedItem().toString());
     ...
     */
    fun setCountryLimit(selectedCountry: Any?) {
        var selectedCountry = selectedCountry
        if (selectedCountry != null && selectedCountry.toString().length > 0) {
            countryLimit = selectedCountry.toString()
            stateprovLimit = null
        } else {
     ...
     */
    fun setStateProvLimit(selectedStateProv: Any?) {
        if (selectedStateProv != null && selectedStateProv.toString().length > 0) {
            stateprovLimit = selectedStateProv.toString()
            countryLimit = null
        } else {
            stateprovLimit = null
        }
    ...
    companion object {
        private const val serialVersionUID = -7988464282872345110L
        private val log: Log? = LogFactory.getLog(FilteringGeogJComboBox::class.java)
    }
}