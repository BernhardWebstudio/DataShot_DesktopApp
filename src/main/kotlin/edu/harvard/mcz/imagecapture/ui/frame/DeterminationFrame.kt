 ...
 */
class DeterminationFrame : JFrame {
    private var jContentPane: JPanel? = null
    private var jScrollPane: JScrollPane? = null
    private var jTableDeterminations: JTable? = null
    private var clickedOnDetsRow = 0
    private var jPopupDets: JPopupMenu? = null
    private var determinationsModel: DeterminationTableModel?
    private var specimen: Specimen? = null
    private var jPanel: JPanel? = null
    private var jButtonAdd: JButton? = null
    private var jButtonDone: JButton? = null
    private var panel: JPanel? = null
    private var lblFillInVerbatim: JLabel? = null private get() {
        if (field == null) {
            field = JLabel("Fill in either Verbatim Text or (Genus/Species/Subspecies/Infraspecific/Rank/Authorship), but  not both.")
        }
        return field
    }
    private var thisFrame: DeterminationFrame? = null
    ...
    constructor() : super() {
        thisFrame = this
        determinationsModel = DeterminationTableModel()
        initialize()
     ...
     */
    constructor(determinationsModel: DeterminationTableModel?) : super() {
    ...
    private fun initialize() {
        ...
        this.setPreferredSize(Dimension(1350, 350))
...
//jTableDeterminations.ColumnModel.getColumn(DeterminationTableModel.ROW_IDENTIFIEDBY).setCellEditor(new ComboBoxCellEditor(field));
                ...
                mntmDeleteRow.addActionListener(object : ActionListener {
                    override fun actionPerformed(e: ActionEvent?) {
                        try {
                            ...
                            JOptionPane.showMessageDialog(thisFrame, "Failed to delete a determination row. " + ex.message)
                        }
                    }
                })
    ...
    private fun getJPanel(): JPanel? {
        if (jPanel == null) {
            ...
            jPanel.add(getJButtonAdd(), GridBagConstraints())
    ...
    private fun getJButtonAdd(): JButton? {
        if (jButtonAdd == null) {
            ...
            jButtonAdd.addActionListener(object : ActionListener {
                override fun actionPerformed(e: ActionEvent?) {
     ...
     * @return javax.swing.JButton
     */
    private fun getJButtonDone(): JButton? {
        if (jButtonDone == null) {
            jButtonDone = JButton()
            jButtonDone.setText("Done")
            jButtonDone.setMnemonic(KeyEvent.VK_D)
            jButtonDone.addActionListener(object : ActionListener {
...
// click are saved to the model.
    ...
    private fun getPanel(): JPanel? {
        if (panel == null) {
            panel = JPanel()
            panel.add(lblFillInVerbatim)
        }
        return panel
    }

    companion object {
        private val log: Log = LogFactory.getLog(DeterminationFrame::class.java)
