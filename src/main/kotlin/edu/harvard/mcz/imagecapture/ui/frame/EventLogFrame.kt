 ...
 */
package edu.harvard.mcz.imagecapture.ui.frameimport

import java.awt.Dimensionimport

java.awt.Toolkitimport java.awt.event.ActionEvent
...
class EventLogFrame : JFrame {
    ...
    private var eventLog: EventLogFrame? = null
    ...
    constructor() : super() {
        ...
        eventLog = this
        val t = TrackingLifeCycle()
        events = t.findAll()
        initialize()
    }

    constructor(eventsList: MutableList<Tracking?>?) : super() {
        this.setDefaultCloseOperation(WindowConstants.DISPOSE_ON_CLOSE)
        eventLog = this
        events = eventsList
        initialize()
    ...
    private fun getJScrollPane(): JScrollPane? {
        if (jScrollPane == null) {
            ...
            jScrollPane.setPreferredSize(Dimension(700, 300))
        }
        return jScrollPane
    }
    ...
    private fun getJTable(): JTable? {
        if (jTable == null) {
            jTable = JTable(TrackingTableModel(events))
