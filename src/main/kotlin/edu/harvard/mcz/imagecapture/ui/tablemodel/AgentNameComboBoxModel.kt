 ...
 *
 */
package edu.harvard.mcz.imagecapture.ui.tablemodelimportimport
 ...
 *
 */
class AgentNameComboBoxModel : ComboBoxModel<MCZbaseAuthAgentName?> {
    var model: ArrayList<MCZbaseAuthAgentName?>?
    var dataListeners: ArrayList<ListDataListener?>?
    var selectedItem: MCZbaseAuthAgentName? = null

    constructor() {
        model = ArrayList<MCZbaseAuthAgentName?>()
        // add a blank row
        model!!.add(MCZbaseAuthAgentName())
        dataListeners = ArrayList<ListDataListener?>()
    }

    /**
     * @param model
     */
    constructor(model: MutableList<MCZbaseAuthAgentName?>) : super() {
        var model: MutableList<MCZbaseAuthAgentName?> = model
        if (model.isEmpty()) {
            model = ArrayList<MCZbaseAuthAgentName?>()
        }
        this.model = model as ArrayList<MCZbaseAuthAgentName?>
        if (model == null) {
            model = ArrayList<MCZbaseAuthAgentName?>()
            // add a blank row.
            model.add(0, MCZbaseAuthAgentName())
        }
        dataListeners = ArrayList<ListDataListener?>()
    }
     ...
     */
    override fun getElementAt(index: Int): MCZbaseAuthAgentName? {
        return model!![index]
    }
    ...
    private fun notifyListeners() {
        val i: MutableIterator<ListDataListener?> = dataListeners!!.iterator()
        while (i.hasNext()) {
            i.next().contentsChanged(ListDataEvent(this, ListDataEvent.CONTENTS_CHANGED, 0, model!!.size))
        }
    }
    ...
    override fun setSelectedItem(anItem: Any?) {
        ...
        if (anItem != null) {
            val i: MutableIterator<MCZbaseAuthAgentName?> = model!!.iterator()
            var done = false
            while (i.hasNext() && !done) {
                val higher: MCZbaseAuthAgentName? = i.next()
                try {
                    if (higher.getAgent_name() != null
                            && higher.getAgent_name() == (anItem as MCZbaseAuthAgentName?).getAgent_name()) {
                        log.debug("Selected agent is: " + higher.getAgent_name())
                        selectedItem = higher
                        done = true
                        notifyListeners()
                    }
                } catch (e: ClassCastException) { // Selected item is a string, we need to match to the corresponding object's agent name value.
                    if (higher.getAgent_name() != null
                            && higher.getAgent_name() == anItem) {
                        log.debug("Selected agent is: " + higher.getAgent_name())
                        selectedItem = higher
                        done = true
                        notifyListeners()
            ...
            if (done == false) { // entry isn't on list
                selectedItem = try {
                    model!!.add(anItem as MCZbaseAuthAgentName?)
                    anItem as MCZbaseAuthAgentName?
                } catch (e: ClassCastException) { // provided object was a string
                    val name = MCZbaseAuthAgentName()
     ...
     */
    val selectedItemAgentName: String?
        get() = if (selectedItem == null) {
            null
        } else selectedItem.getAgent_name()
     ...
     * @return
     */
    fun getDataElementAt(i: Int): Any? {
        return model!![i]
    }
     ...
     * @param dataElementAt
     */
    fun addElement(dataElementAt: MCZbaseAuthAgentName?) {
     ...
     * @return
     */
    fun getModel(): MutableList<MCZbaseAuthAgentName?>? {
        return model
    }

    companion object {
        private const val serialVersionUID = -4856432985354519476L
        private val log: Log? = LogFactory.getLog(AgentNameComboBoxModel::class.java)
    }
}