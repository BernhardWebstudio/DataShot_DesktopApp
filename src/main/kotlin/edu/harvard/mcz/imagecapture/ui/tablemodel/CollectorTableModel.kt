...
import edu.harvard.mcz.imagecapture.entity.Collectorimport

org.apache.commons.logging.Logimport org.apache.commons.logging.LogFactoryimport java.lang.Exception
 ...
 */
class CollectorTableModel : AbstractDeleteableTableModel {
    private var collectors: MutableSet<Collector?>?

    constructor() {
        collectors = HashSet<Collector?>()
    }

    constructor(aCollectorList: MutableSet<Collector?>?) {
    ...
    val rowCount: Int
        get() =//allie fix
            if (collectors == null) 0 else collectors!!.size
    ...
    override fun setValueAt(value: Any, rowIndex: Int, columnIndex: Int) {
        ...
        try {
            (collectors!!.toTypedArray().get(rowIndex) as Collector).setCollectorName(value.toString())
        } catch (e: Exception) {
...
//((Collector)collectors.toArray()[rowIndex]).setCollectorName(((MCZbaseAuthAgentName)value).getAgent_name());
    ...
    override fun deleteRow(rowIndex: Int) {
        val toRemove: Collector = collectors!!.toTypedArray().get(rowIndex) as Collector
        val spals = CollectorLifeCycle()
        try {
    ...
    companion object {
        private const val serialVersionUID = -3022078380872976717L
        private val log: Log? = LogFactory.getLog(CollectorTableModel::class.java)
    }
}