 ...
 *
 */
package edu.harvard.mcz.imagecapture.ui.tablemodelimport
 ...
 */
class DeterminationTableModel : AbstractTableModel {
    private var determinations: MutableSet<Determination?>? = null
    // Allow display of a cell editor, intended for adding a details button.
    private var showEditableId = false

    /**
     *
     */
    constructor() {
        determinations = HashSet<Determination?>()
    }

    constructor(aDeterminationList: MutableSet<Determination?>?) {
     ...
     * column.
     */
    constructor(aDeterminationList: MutableSet<Determination?>?, enableIdEditor: Boolean) {
     ...
     */
    val columnCount: Int
        get() =// all fields in determination except identificationQualifier
            15
...
//if (columnIndex==ROW_ID || columnIndex==ROW_SPECIMEN) {
        if (columnIndex == ROW_SPECIMEN) { // turn off editing of determinationId and barcode.
            result = false
        }
        return result
    }
    ...
    override fun setValueAt(value: Any?, rowIndex: Int, columnIndex: Int) {
        when (columnIndex) {
            SPECIES_NUMBER -> (determinations!!.toTypedArray().get(rowIndex) as Determination).setSpeciesNumber(value as String?)
            1 -> (determinations!!.toTypedArray().get(rowIndex) as Determination).setGenus(value as String?)
            2 -> (determinations!!.toTypedArray().get(rowIndex) as Determination).setSpecificEpithet(value as String?)
            3 -> (determinations!!.toTypedArray().get(rowIndex) as Determination).setSubspecificEpithet(value as String?)
            4 -> (determinations!!.toTypedArray().get(rowIndex) as Determination).setInfraspecificEpithet(value as String?)
            5 -> (determinations!!.toTypedArray().get(rowIndex) as Determination).setInfraspecificRank(value as String?)
            6 -> (determinations!!.toTypedArray().get(rowIndex) as Determination).setAuthorship(value as String?)
            ROW_TYPESTATUS -> (determinations!!.toTypedArray().get(rowIndex) as Determination).setTypeStatus(value as String?)
            ROW_IDENTIFIEDBY ->  //((Determination)determinations.toArray()[rowIndex]).setIdentifiedBy(((MCZbaseAuthAgentName)value).getAgent_name());
                (determinations!!.toTypedArray().get(rowIndex) as Determination).setIdentifiedBy(value as String?)
            9 -> (determinations!!.toTypedArray().get(rowIndex) as Determination).setUnNamedForm(value as String?)
            10 -> (determinations!!.toTypedArray().get(rowIndex) as Determination).setVerbatimText(value as String?)
            ROW_NATUREOFID -> (determinations!!.toTypedArray().get(rowIndex) as Determination).setNatureOfId(value as String?)
            ROW_DATEIDENTIFIED -> (determinations!!.toTypedArray().get(rowIndex) as Determination).setDateIdentified(value as String?)
            13 -> (determinations!!.toTypedArray().get(rowIndex) as Determination).setRemarks(value as String?)
            ROW_SPECIMEN -> {
            }
        }
    }
     ...
     */
    fun addDetermination(aDetermination: Determination?) {
    ...
    override fun getColumnName(columnIndex: Int): String {
        var returnvalue = ""
        when (columnIndex) {
            SPECIES_NUMBER -> returnvalue = "Species Number"
            1 -> returnvalue = "Genus"
            2 -> returnvalue = "Species"
            3 -> returnvalue = "Subspecies"
            4 -> returnvalue = "Infraspecific"
            5 -> returnvalue = "Infra Rank"
            6 -> returnvalue = "Authorship"
            ROW_TYPESTATUS -> returnvalue = "Type Status"
            ROW_IDENTIFIEDBY -> returnvalue = "Determiner"
            9 -> returnvalue = "Un-named form"
            10 -> returnvalue = "Verbatim Text"
            ROW_NATUREOFID -> returnvalue = "Nature Of ID"
            ROW_DATEIDENTIFIED -> returnvalue = "ID Date"
            13 -> returnvalue = "Remarks"
            ROW_SPECIMEN -> returnvalue = "Barcode"
        }
        return returnvalue
    }
    ...
    fun deleteRow(rowIndex: Int) {
        val toRemove: Determination = determinations!!.toTypedArray().get(rowIndex) as Determination
        val spals = DeterminationLifeCycle()
        try {
    ...
    companion object {
        const val ROW_SPECIMEN = 14
        const val ROW_IDENTIFIEDBY = 8
        const val ROW_TYPESTATUS = 7
        const val ROW_NATUREOFID = 11
        const val ROW_DATEIDENTIFIED = 12
        private const val serialVersionUID = -671159987795807222L
        private val log: Log? = LogFactory.getLog(DeterminationTableModel::class.java)
        // private static final int ROW_ID = 0;
        private const val SPECIES_NUMBER = 0
    }
}