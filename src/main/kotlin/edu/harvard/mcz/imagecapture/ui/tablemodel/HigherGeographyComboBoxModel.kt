 ...
 *
 */
class HigherGeographyComboBoxModel : ComboBoxModel<MCZbaseGeogAuthRec?> {
    var model: ArrayList<MCZbaseGeogAuthRec?>?
    var dataListeners: ArrayList<ListDataListener?>?
    var selectedItem: MCZbaseGeogAuthRec? = null

    constructor() {
        model = ArrayList<MCZbaseGeogAuthRec?>()
        // add a blank row
        model!!.add(MCZbaseGeogAuthRec())
     ...
     * @param model
     */
    constructor(model: MutableList<MCZbaseGeogAuthRec?>) : super() {
        var model: MutableList<MCZbaseGeogAuthRec?> = model
        if (model.isEmpty()) {
            model = ArrayList<MCZbaseGeogAuthRec?>()
        }
        this.model = model as ArrayList<MCZbaseGeogAuthRec?>
        if (model == null) {
            model = ArrayList<MCZbaseGeogAuthRec?>()
            // add a blank row.
            model.add(0, MCZbaseGeogAuthRec())
        }
        dataListeners = ArrayList<ListDataListener?>()
    }
     ...
     */
    override fun getElementAt(index: Int): MCZbaseGeogAuthRec? {
        return model!![index]
    }
    ...
    override fun setSelectedItem(anItem: Any?) {
        ...
        if (anItem != null) {
            val i: MutableIterator<MCZbaseGeogAuthRec?> = model!!.iterator()
            var done = false
            while (i.hasNext() && !done) {
                val higher: MCZbaseGeogAuthRec? = i.next()
                try {
                    if (higher.getHigher_geog() != null
                            && higher.getHigher_geog() == (anItem as MCZbaseGeogAuthRec?).getHigher_geog()) {
                        log.debug("Selected higher is: " + higher.getHigher_geog())
                        selectedItem = higher
                        done = true
                        notifyListeners()
                    }
                } catch (e: ClassCastException) { // Selected item is a string, we need to match to the corresponding object's higher geography value.
                    if (higher.getHigher_geog() != null
                            && higher.getHigher_geog() == anItem) {
                        log.debug("Selected higher is: " + higher.getHigher_geog())
                        selectedItem = higher
                        done = true
                        notifyListeners()
     ...
     */
    val selectedItemHigherGeography: String?
        get() = if (selectedItem == null) {
            null
        } else selectedItem.getHigher_geog()
     ...
     * @param dataElementAt
     */
    fun addElement(dataElementAt: MCZbaseGeogAuthRec?) {
     ...
     * @return
     */
    fun getModel(): MutableList<MCZbaseGeogAuthRec?>? {
        return model
    }

    companion object {
        private const val serialVersionUID = -4856432985354519476L
        private val log: Log? = LogFactory.getLog(HigherGeographyComboBoxModel::class.java)
    }
}