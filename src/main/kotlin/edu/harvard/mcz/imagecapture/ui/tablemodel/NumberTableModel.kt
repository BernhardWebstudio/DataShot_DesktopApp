...
import edu.harvard.mcz.imagecapture.entity.Numberimport
 ...
 * JTable.
 */
class NumberTableModel : AbstractDeleteableTableModel {
    private var numbers: MutableSet<Number?>? = null

    constructor() {
        numbers = HashSet()
    }

    constructor(aNumberList: MutableSet<Number?>?) {
        numbers = aNumberList
    }

    fun addNumber(aNumber: Number?) {
        numbers!!.add(aNumber)
    ...
    override fun getColumnName(columnIndex: Int): String? {
        ...
        when (columnIndex) {
            COLUMN_NUMBER -> returnvalue = "Number"
            COLUMN_TYPE -> returnvalue = "Type"
        }
        return returnvalue
    }
    ...
    override fun setValueAt(value: Any?, rowIndex: Int, columnIndex: Int) {
        ...
        when (columnIndex) {
            0 -> numbers!!.toTypedArray()[rowIndex].setNumber(value as String?)
            COLUMN_TYPE -> numbers!!.toTypedArray()[rowIndex].setNumberType(value as String?)
        }
    }
    ...
    override fun deleteRow(rowIndex: Int) {
        val toRemove = numbers!!.toTypedArray()[rowIndex]!!
        val spals = NumberLifeCycle()
        try {
    ...
    companion object {
        ...
        private const val serialVersionUID = 2244991738218368487L
        private val log: Log? = LogFactory.getLog(NumberTableModel::class.java)
    }
}