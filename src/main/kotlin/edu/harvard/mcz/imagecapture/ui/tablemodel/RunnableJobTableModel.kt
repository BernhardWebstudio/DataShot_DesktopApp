 ...
 * JobTableModel
 */
class RunnableJobTableModel : AbstractTableModel(), RunnerListener {
    private val jobs: MutableSet<RunnableJob?>?
    /* (non-Javadoc)
     ...
     */
    override fun getColumnClass(columnIndex: Int): Class<*> { // Given current implementation of button in SpecimenBrowser,
...
// and ** Must Not ** return Long for any other column).
        ...
        when (columnIndex) {
            0 -> result = Int::class.java
            1 -> result = String::class.java
            2 -> result = String::class.java
            3 -> result = Int::class.javaPrimitiveType!!
        }
        return result
    }
    ...
    override fun getColumnName(columnIndex: Int): String? {
        ...
        when (columnIndex) {
            0 -> returnvalue = " "
            1 -> returnvalue = "Job"
            2 -> returnvalue = "Started"
            3 -> returnvalue = "Progress"
        }
        return returnvalue
    }
    ...
    fun getJobAt(rowIndex: Int?): RunnableJob? {
        var returnvalue: RunnableJob? = null
        returnvalue = jobs!!.toTypedArray().get(rowIndex) as RunnableJob
        return returnvalue
    }
     ...
     */
    fun addJob(aJob: RunnableJob) {
        jobs!!.add(aJob)
        aJob.registerListener(this)
        this.fireTableDataChanged()
    }

    fun removeJob(aJob: RunnableJob?) {
        jobs!!.remove(aJob)
        this.fireTableDataChanged()
    }

    fun registerListener(listener: RunnerListener?) {
        val i: MutableIterator<RunnableJob?> = jobs!!.iterator()
        while (i.hasNext()) {
    ...
    companion object {
        private const val serialVersionUID = 2701547555516905743L
        private val log: Log? = LogFactory.getLog(RunnableJobTableModel::class.java)
    }
