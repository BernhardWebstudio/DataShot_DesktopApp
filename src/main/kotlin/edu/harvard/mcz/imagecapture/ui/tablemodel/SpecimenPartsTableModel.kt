 ...
 *
 */
class SpecimenPartsTableModel : AbstractDeleteableTableModel {
    private var specimenParts: MutableSet<SpecimenPart?>?

    constructor() : super() {
        specimenParts = HashSet<SpecimenPart?>()
    }

    /**
     * @param specimenParts
     */
    constructor(specimenParts: MutableSet<SpecimenPart?>?) : super() {
    ...
    val rowCount: Int
        get() =// TODO Auto-generated method stub
            specimenParts!!.size// TODO Auto-generated method stub
    ...
    override fun setValueAt(aValue: Any, rowIndex: Int, columnIndex: Int) {
        ...
        when (columnIndex) {
            0 -> (specimenParts!!.toTypedArray().get(rowIndex) as SpecimenPart).setPartName(aValue as String)
            1 -> (specimenParts!!.toTypedArray().get(rowIndex) as SpecimenPart).setPreserveMethod(aValue as String)
            2 -> (specimenParts!!.toTypedArray().get(rowIndex) as SpecimenPart).setLotCount(aValue as String?. toInt ())
            3 -> {
            }
            4 -> {
            }
    ...
    override fun deleteRow(rowIndex: Int) {
        val toRemove: SpecimenPart = specimenParts!!.toTypedArray().get(rowIndex) as SpecimenPart
        val spals = SpecimenPartLifeCycle()
        try {
    ...
    companion object {
        ...
        private val log: Log? = LogFactory.getLog(SpecimenPartsTableModel::class.java)
    }
}