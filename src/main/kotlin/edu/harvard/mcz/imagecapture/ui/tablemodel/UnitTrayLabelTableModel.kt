 ...
 */
class UnitTrayLabelTableModel : AbstractTableModel {
    private var labels: MutableList<UnitTrayLabel?>? = null

    constructor(labels: MutableList<UnitTrayLabel?>?) {
        this.labels = labels
    }
    ...
    fun setLabels(labels: MutableList<UnitTrayLabel?>?) {
        this.labels = labels
    }

    fun addRow() {
        val newLabel = UnitTrayLabel()
        val uls = UnitTrayLabelLifeCycle()
        try { //TODO: Finish making this a configuration option.
            val nextOrdinalMethod = NEXT_ORDINAL_ZERO
            var nextOrdinal = 0
    ...
    override fun getColumnName(columnIndex: Int): String? {
        var returnvalue: String? = null
        when (columnIndex) {
            0 -> returnvalue = "Family"
            1 -> returnvalue = "SubFamily"
            2 -> returnvalue = "Tribe"
            3 -> returnvalue = "Genus"
            4 -> returnvalue = "Species"
            5 -> returnvalue = "Subspecies"
            6 -> returnvalue = "Infra Rank"
            7 -> returnvalue = "Infraspecific"
            8 -> returnvalue = "Authorship"
            9 -> returnvalue = "UnNamed Form"
            10 -> returnvalue = "Drawer"
            11 -> returnvalue = "Collection"
            12 -> returnvalue = "Sort"
            COLUMN_TO_PRINT -> returnvalue = "To Print"
            COLUMN_PRINTED -> returnvalue = "Printed"
        }
        return returnvalue
    }
    ...
    override fun setValueAt(value: Any, rowIndex: Int, columnIndex: Int) {
        log.debug("Object to save at $rowIndex,$columnIndex is $value")
        when (columnIndex) {
            0 -> labels!![rowIndex].setFamily(value.toString())
            1 -> labels!![rowIndex].setSubfamily(value.toString())
            2 -> labels!![rowIndex].setTribe(value.toString())
            3 -> labels!![rowIndex].setGenus(value.toString())
            4 -> labels!![rowIndex].setSpecificEpithet(value.toString())
            5 -> labels!![rowIndex].setSubspecificEpithet(value.toString())
            6 -> labels!![rowIndex].setInfraspecificRank(value.toString())
            7 -> labels!![rowIndex].setInfraspecificEpithet(value.toString())
            8 -> labels!![rowIndex].setAuthorship(value.toString())
            9 -> labels!![rowIndex].setUnNamedForm(value.toString())
            10 -> labels!![rowIndex].setDrawerNumber(value.toString())
            11 -> labels!![rowIndex].setCollection(value.toString())
            12 -> labels!![rowIndex].setOrdinal(value.toString().toInt())
            COLUMN_TO_PRINT -> labels!![rowIndex].setNumberToPrint(value.toString().toInt())
            COLUMN_PRINTED -> labels!![rowIndex].setPrinted(value.toString().toInt())
        }
        saveRow(rowIndex)
    }
    ...
    private fun saveRow(rowIndex: Int) {
        val uls = UnitTrayLabelLifeCycle()
        try {
...
// TODO: Better support for copy and paste of multiple cells.
         ...
         */
        const val NEXT_ORDINAL_MAXPLUSONE = 1
        private val log: Log? = LogFactory.getLog(UnitTrayLabelTableModel::class.java)
        private const val COLUMN_TO_PRINT = 13
        private const val COLUMN_PRINTED = 14
        private const val serialVersionUID = -8022147291895055945L
    }
}