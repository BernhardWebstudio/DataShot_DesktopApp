...
public class TestOfImageCaptureProperties extends TestCase {
	...
	protected void setUp() throws Exception {
		...
		imageBase = Singleton.getSingletonInstance().getProperties().getProperties()
	...
	protected void tearDown() throws Exception {
		...
		Singleton.getSingletonInstance().getProperties().getProperties()
				.setProperty(ImageCaptureProperties.KEY_IMAGEBASE, imageBase);
	}

	/**
	 * Test method for
	 * {@link edu.harvard.mcz.imagecapture.ImageCaptureProperties#getPathBelowBase(java.io.File)}.
	 * Tests generation of path from base given a path including base and a
	 * filename.
	 *
	...
	public void testGetPathBelowBaseForFile() {
		...
		// Create a base path from the actual path to this file, leaving out one
		// directory
		String base = (new File(full.getParent())).getParent();
		Singleton.getSingletonInstance().getProperties().getProperties()
				.setProperty(ImageCaptureProperties.KEY_IMAGEBASE, base);
		assertEquals(AllTests.FILE_VALID_BARCODE_PATH, ImageCaptureProperties.getPathBelowBase(full, "/"));
		// test round trip re-assembly
		assertEquals(fullstring, ImageCaptureProperties.assemblePathWithBase(
				ImageCaptureProperties.getPathBelowBase(full, "/"), AllTests.FILE_VALID_BARCODE_FILENAME));
	}
	...
	public void testGetPathBelowBase() {
		...
		// Uses ImageCaptureProperties.getPathBelowBase(file, separator) which is
		// intended only
		// for use here in unit testing.

		// Goal is to retrieve:
		// File imageFile = new File(IMAGEBASE + image.getPath() + image.getFilename());
		// So, given absolute path to a file, need to remove the filename and IMAGEBASE,
		// leaving just
		// a path to store in ICImage.Path.
		// IMAGEBASE should end in a separator, image.getPath should not start with a
		// separator, but end with one,
		...
		/*
		 * Note: This test won't run, as there isn't a way to tell a non-existent file
		 * from a non-existent directory. To do this, run the test
		 * testGetPathBelowBaseForFile() on both Windows and unix machines.
		 *
		 * base = "Z:\\images\\"; fullstring =
		 * "Z:\\images\\test_files\\test\\image.img"; full = new File(fullstring);
		 * Singleton.getSingletonInstance().getProperties().getProperties().setProperty(
		 * ImageCaptureProperties.KEY_IMAGEBASE, base);
		 * assertEquals("test_files\\test\\", ImageCaptureProperties.getPathBelowBase(full, "
		 * \\"));
		 */
